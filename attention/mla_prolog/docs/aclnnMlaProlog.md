...
-  参数说明
    ...
    |----------------------------|-----------|----------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|------------|---------------------------|-------------------------|
    | tokenX                     | 输入      | 公式中用于计算Query和Key的输入tensor，Device侧的aclTensor。          | 支持B=0,S=0,T=0的空Tensor                                                                                                              | BFLOAT16、INT8 | ND         | A2、A3输入维度：<br>2维：(T,He)、3维：(B,S,He)<br>A5输入维度：<br>3维：(B,S,He) | -                       |
    ...
    | ropeSin                    | 输入      | 用于计算旋转位置编码的正弦参数矩阵，Device侧的aclTensor。              |  支持B=0,S=0,T=0的空Tensor                                                  | BFLOAT16       | ND         | A2、A3输入维度：<br>2维：(T,Dr)、3维：(B,S,Dr)<br>A5输入维度：<br>3维：(B,S,Dr) | -                       |
    | ropeCos                    | 输入      | 用于计算旋转位置编码的余弦参数矩阵，Device侧的aclTensor。              |  支持B=0,S=0,T=0的空Tensor                                                | BFLOAT16       | ND         | A2、A3输入维度：<br>2维：(T,Dr)、3维：(B,S,Dr)<br>A5输入维度：<br>3维：(B,S,Dr) | -                       |
    | cacheIndex                 | 输入      | 用于存储kvCache和krCache的索引，Device侧的aclTensor。                  |  支持B=0,S=0,T=0的空Tensor <br> 取值范围需在[0,BlockNum*BlockSize)内                                                                   | INT64          | ND         | A2、A3输入维度：<br>1维：(T)、2维：(B,S)<br> A5输入维度：<br>2维：(B,S)      | -                       |
    ...
    | queryOut                   | 输出      | 公式中Query的输出tensor（对应$q^N$），Device侧的aclTensor。             | 不支持空Tensor                                                                                                                         | BFLOAT16、INT8 | ND         | A2、A3输入维度：<br>3维：(T,N,Hckv)、4维：(B,S,N,Hckv) <br> A5输入维度：<br>4维：(B,S,N,Hckv) | -                       |
    | queryRopeOut               | 输出      | 公式中Query位置编码的输出tensor（对应$q^R$），Device侧的aclTensor。      | 不支持空Tensor                                                                                                                         | BFLOAT16       | ND         | A2、A3输入维度：<br>3维：(T,N,Dr)、4维：(B,S,N,Dr) <br> A5输入维度：<br>4维：(B,S,N,Dr) | -                       |
...
-  shape 格式字段含义说明
    ...
    | He           | Head-Size（隐藏层大小）        | A2、A3取值固定为：7168 、7680<br> A5取值固定为：7168                         |
