...
-  参数说明
    ...
    | weightDq                   | 输入      | 公式中用于计算Query的下采样权重矩阵$W^{DQ}$，Device侧的aclTensor      |  不支持空Tensor                                                                                                                         | BFLOAT16、INT8 | FRACTAL_NZ（KN格式） | 2维：(He,Hcq)             | -                       |
    ...
    | weightDkvKr                | 输入      | 公式中用于计算Key的下采样权重矩阵$W^{DKV}$和位置编码权重矩阵$W^{KR}$，Device侧的aclTensor。 |  不支持空Tensor                                        | BFLOAT16、INT8 | FRACTAL_NZ（KN格式） | 2维：(He,Hckv+Dr)         | -                      |
