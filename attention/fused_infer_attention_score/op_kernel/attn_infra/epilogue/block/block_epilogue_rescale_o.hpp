...
template <
    ...
    LseMode LSE_MODE_>
class BlockEpilogue<
    EpilogueAtlasA2RescaleO<LSE_MODE_>,
    OutputType_,
    InputType_,
    UpdateType_,
    LseType_>
...
public:
    // Type aliases
    using DispatchPolicy = EpilogueAtlasA2RescaleO<LSE_MODE_>;
    ...
    void SubCoreCompute(
        ...
        uint32_t needRowLoop, uint32_t isLastRowLoop, uint32_t rowOffsetLoop, uint32_t qSThisSubBlock,
        uint32_t proTokenIdx, uint32_t proTokenNum, uint32_t epiTokenNum, uint32_t integralHeadNum)
    {
        ...
        if (isLastStackTile) {
            ...
            if constexpr (LSE_MODE_ == LseMode::OUT_ONLY) {
                if (isLastRowLoop) {
                    AscendC::PipeBarrier<PIPE_V>();
                    uint32_t lenBrust = sizeof(float);
                    AscendC::Ln<float, false>(lse32_ubuf_tensor, glUbTensor, (uint64_t)0, 1, AscendC::UnaryRepeatParams(1, 1, 8, 8));

                    AscendC::PipeBarrier<PIPE_V>();
                    AscendC::Add<float, false>(lse32_ubuf_tensor, lse32_ubuf_tensor, gmUbTensor, (uint64_t)0, 1, AscendC::BinaryRepeatParams(1, 1, 1, 8, 8, 8));
                    AscendC::PipeBarrier<PIPE_V>();
                    ...
                    if (qNThisSubBlock == 0U) {
                        AscendC::DataCopyPad(
                            gLse, tvUbTensor,
                            AscendC::DataCopyExtParams(curRowNum, lenBrust, 0, (qHeads - 1) * lenBrust, 0));
                    } else {
                        for (uint32_t qNIdx = 0; qNIdx < qNThisSubBlock; qNIdx++) {
                            AscendC::DataCopyPad(
                                gLse[qNIdx],
                                tvUbTensor[qNIdx * qSBlockSize * FLOAT_BLOCK_SIZE],
                                AscendC::DataCopyExtParams(qSBlockSize, lenBrust, 0, (qHeads - 1) * lenBrust, 0));
                        }
                    }
        ...
        uint32_t isFirstStackTile, uint32_t isLastStackTile, uint32_t curStackTileMod)
    {
        ...
        uint32_t rowTile = MAX_UB_O_ELEM_NUM / embed;
        ...
        if (inRowActualThisSubBlock > 0U) {
            uint32_t rowLoop = CeilDiv(inRowActualThisSubBlock, rowTile);
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                uint32_t rowOffsetLoop = rowLoopIdx * rowTile;
                uint32_t rowOffsetCurLoop = inRowOffsetThisSubBlock + rowOffsetLoop;
                uint32_t rowActualCurLoop =
                    (rowLoopIdx == (rowLoop - 1U)) ? inRowActualThisSubBlock - rowLoopIdx * rowTile : rowTile;

                int64_t offsetOutput =
                    static_cast<int64_t>(rowLoopIdx * rowTile / qSThisSubBlock * embed + outOffsetSubBlock);
