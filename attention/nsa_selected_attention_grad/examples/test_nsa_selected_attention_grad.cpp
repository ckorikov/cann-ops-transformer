...
/*!
 * \file test_nsa_selected_attention_grad.cpp
 * \brief
 */
...
int main()
{
    ...
    void *actualSeqKvLenOptionalDeviceAddr = nullptr;
    ...
    std::vector<aclFloat16> dqOutHostData(GetShapeSize(dqOutShape), 1);
    std::vector<aclFloat16> dkOutHostData(GetShapeSize(dkOutShape), 1);
    std::vector<aclFloat16> dvOutHostData(GetShapeSize(dvOutShape), 1);
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(
        attentionOutHostData, attentionOutShape, &attentionOutDeviceAddr, aclDataType::ACL_FLOAT16, &attentionOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(attentionOutGradHostData,
        attentionOutGradShape,
        &attentionOutGradDeviceAddr,
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(
        topkIndicesHostData, topkIndicesShape, &topkIndicesDeviceAddr, aclDataType::ACL_INT32, &topkIndices);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    int64_t tempQ[1] = {1};
    ...
    ret = aclnnNsaSelectedAttentionGradGetWorkspaceSize(query,
        key,
        value,
        attentionOut,
        attentionOutGrad,
    ...
    auto size = GetShapeSize(dqOutShape);
    ...
    aclrtFree(actualSeqKvLenOptionalDeviceAddr);
