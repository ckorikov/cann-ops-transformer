...
template <typename T, typename T2, typename MMOUTPUT_T, uint8_t mode = 0, uint32_t sOuter = 0, uint32_t sInner = 0>
__aicore__ inline void SoftmaxFlashV510NoUpdateImpl128(
    ...
    const uint32_t originN, const T scale, const T minValue, const uint16_t blockStride, float deqScaleQK, float quantScaleP)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < rows; ++i) {
            ...
            MicroAPI::Cast<int8_t, half, castTrait>(vreg_cast, vreg_muls_res, preg_all_b16);
            ...
            MicroAPI::DataCopy<int8_t, MicroAPI::DataCopyMode::DATA_BLOCK_COPY,
                    MicroAPI::PostLiteral::POST_MODE_UPDATE>(((__ubuf__ int8_t *&)expUb), vreg_res, blockStride,
                    repeatStride, preg_s8);
    ...
    const uint32_t originN, const T scale, const T minValue, const uint16_t blockStride, float deqScaleQK, float quantScaleP)
{
    // mode 1: originN = 128
    if constexpr (mode == 1) {
        SoftmaxFlashV510NoUpdateImpl128<T, T2, MMOUTPUT_T, mode, sOuter, sInner>(dstTensor,
            expSumTensor, maxTensor, expMaxTensor, inSrcTensor, inExpSumTensor, inMaxTensor, inMaskTensor, inPseTensor,
            sharedTmpBuffer, m, originN, scale, minValue, blockStride, deqScaleQK, quantScaleP);
    }
}
    ...
    const uint32_t originN, const T scale, const T minValue, const uint16_t blockStride, float deqScaleQK, float quantScaleP)
{
    ...
    __ubuf__ T * tmpMaxUb = (__ubuf__ T*)((__ubuf__ float*)sharedTmpBuffer.GetPhyAddr() + m * reduceN);
    __ubuf__ T * tmpMaxUbStart = (__ubuf__ T*)((__ubuf__ float*)sharedTmpBuffer.GetPhyAddr() + m * reduceN);
    ...
    const uint32_t originN, const T scale, const T minValue, const uint16_t blockStride, float deqScaleQK, float quantScaleP)
{
    // mode 1: originN = 128
    if constexpr (mode == 1) {
        SoftmaxFlashV510UpdateImpl128<T, T2, MMOUTPUT_T, mode, sOuter, sInner>(dstTensor,
            expSumTensor, maxTensor, expMaxTensor, inSrcTensor, inExpSumTensor, inMaxTensor, inMaskTensor, inPseTensor,
            sharedTmpBuffer, m, originN, scale, minValue, blockStride, deqScaleQK, quantScaleP);
    }
}
 ...
 */
template <typename T, typename T2, typename MMOUTPUT_T, bool isUpdate = false, uint8_t mode = 0, uint32_t sOuter = 0, uint32_t sInner = 0>
__aicore__ inline void SoftmaxFlashV510_VF(const LocalTensor<T2>& dstTensor, const LocalTensor<float>& expSumTensor,
    const LocalTensor<T>& maxTensor, const LocalTensor<float>& expMaxTensor, const LocalTensor<T>& inSrcTensor,
    const LocalTensor<float>& inExpSumTensor, const LocalTensor<T>& inMaxTensor, const LocalTensor<uint8_t>& inMaskTensor,
    const LocalTensor<T>& inPseTensor, const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t m,
    const uint32_t originN, const T scale, const T minValue, float deqScaleQK, float quantScaleP)
{
    ...
    uint16_t blockStride = m * blockN * sizeof(T2) / blockU8;

    if constexpr (!isUpdate) {
        SoftmaxFlashV510NoUpdate8<T, T2, MMOUTPUT_T, mode, sOuter, sInner>(dstTensor,
            expSumTensor, maxTensor, expMaxTensor, inSrcTensor, inExpSumTensor, inMaxTensor, inMaskTensor, inPseTensor,
            sharedTmpBuffer, m, originN, scale, minValue, blockStride, deqScaleQK, quantScaleP);
    } else {
        SoftmaxFlashV510Update8<T, T2, MMOUTPUT_T, mode, sOuter, sInner>(dstTensor,
            expSumTensor, maxTensor, expMaxTensor, inSrcTensor, inExpSumTensor, inMaxTensor, inMaskTensor, inPseTensor,
            sharedTmpBuffer, m, originN, scale, minValue, blockStride, deqScaleQK, quantScaleP);
    }
}
