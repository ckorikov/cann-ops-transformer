 ...
 */
 ...
 #ifndef FAG_COPY_CUBE_IN_ROPE_PRE_PRELOAD_H
 #define FAG_COPY_CUBE_IN_ROPE_PRE_PRELOAD_H
 ...
 class FagCopyCubeInS1S2PreRopePreload : public CopyCubeInBase<IMPL, MM_CFG, INPUT_TYPE> {
 ...
 public:
     __aicore__ inline FagCopyCubeInS1S2PreRopePreload()
     {
     }
                                                   ...
                                                   int batchNum = 1)
     {
         ...
         MATMUL_MODULE(Context)->leftMatrixEncodingTableIdx = static_cast<int32_t>(flag.flagCommon.leftMatrixEncodingTableIdx);
         MATMUL_MODULE(Context)->rightMatrixEncodingTableIdx = static_cast<int32_t>(flag.flagCommon.rightMatrixEncodingTableIdx);
  
         if constexpr (INPUT_TYPE::TAG == InputTypeTag::A) {
             tscmIndex = MM1_ENCODING_TABLE[flag.flagCommon.leftMatrixEncodingTableIdx];
         } else {
             tscmIndex = MM1_ENCODING_TABLE[flag.flagCommon.rightMatrixEncodingTableIdx];
             if (!flag.flagCommon.kvNeedCopy) {
         ...
         if constexpr (INPUT_TYPE::TAG == InputTypeTag::A) {
             if (flag.flagCommon.copyCurrent) {
                 ...
                 if (flag.mmIdx == 1) {
                     intriParams.dValue = ROPE_D_128;
                     DataCopy(l1, aGlobal, intriParams);
                     // 从flag中获取Rope地址
                     GlobalTensor<SRC_T> aRopeGlobal;
                     __gm__ SRC_T *aGmRopeAddr_ = reinterpret_cast<__gm__ SRC_T *>(flag.aRopeAddr);
                     aRopeGlobal.SetGlobalBuffer(aGmRopeAddr_);
                     intriParams.dValue = ROPE_D_64;    // left matrix: k;
                     intriParams.srcNdMatrixStride = 0;
                     intriParams.srcDValue = orgWidth >> 1;
                     DataCopy(l1[intriParams.dstNzC0Stride * ROPE_D_128], aRopeGlobal, intriParams);
                 } else {
                     DataCopy(l1, aGlobal, intriParams);
                 }
                 tscm.EnQue(l1);
             ...
             if (flag.mmIdx == 1) {
                 ...
                 // 从flag中获取Rope地址
                 GlobalTensor<SRC_T> bRopeGlobal;
                 __gm__ SRC_T *bGmRopeAddr_ = reinterpret_cast<__gm__ SRC_T *>(flag.bRopeAddr);
                 bRopeGlobal.SetGlobalBuffer(bGmRopeAddr_);
                 intriParams.dValue = ROPE_D_64;    // left matrix: k;
                 intriParams.srcNdMatrixStride = 0;
                 intriParams.srcDValue = orgWidth >> 1;
                 DataCopy(l1[intriParams.dstNzC0Stride * ROPE_D_128], bRopeGlobal, intriParams);
             } else {
                 DataCopy(l1, bGlobal, intriParams);
             }
             tscm.EnQue(l1);
     ...
     __aicore__ inline void Destroy()
     {
         if constexpr (!PhyPosIsL1(INPUT_TYPE::pos)) {
             ...
             if constexpr (INPUT_TYPE::TAG == InputTypeTag::A) {
                 FagTscmRopeFlagData flag = MATMUL_MODULE(MatmulUserDefineInfo)->GetSelfDefineData();
                 if (flag.flagCommon.copyNext) {
                     uint8_t nextTscmIndex =
                         MM1_NEXT_ENCODING_TABLE[MM1_ENCODING_TABLE[flag.flagCommon.leftMatrixEncodingTableIdx]];
                     ...
                     Nd2NzParams intriParams;
                     intriParams.ndNum = 1;
                     intriParams.nValue = flag.flagCommon.nextMorN; // left matrix: m
                     intriParams.dValue = tileWidth_;    // left matrix: k;
                     intriParams.srcNdMatrixStride = 0;
                     intriParams.srcDValue = MATMUL_MODULE(CopyCubeInParams)->GetOrgWidth();
                     // L1->L0A，L1上的src stride为C0对齐后的SingleM
                     intriParams.dstNzC0Stride = (flag.flagCommon.nextMorN + C0_SIZE - 1) >> 4 << 4;
                     intriParams.dstNzNStride = 1;
                     intriParams.dstNzMatrixStride = 0;
                     GlobalTensor<SRC_T> aGlobal;
                     aGlobal.SetGlobalBuffer(MATMUL_MODULE(MatmulTensorInfo)->GetGlobalTensor().address_);
                     uint64_t actualPreloadAddr = flag.flagCommon.offsetSign == 1 ?
                                                      (uint64_t)(aGlobal.GetPhyAddr() + flag.flagCommon.nextAddr) :
                                                      (uint64_t)(aGlobal.GetPhyAddr() - flag.flagCommon.nextAddr);
                     ...
                     if (flag.mmIdx ==  1) {
                         ...
                         // 从flag中获取Rope地址
                         GlobalTensor<SRC_T> aRopeGlobal;
                         // aRopeAddr单位为B，nextAddr是offset单位为个数，需要乘上sizeof(SRC_T)
                         uint64_t actualPreloadRopeAddr = flag.flagCommon.offsetSign == 1 ? 
                                                              flag.aRopeAddr + flag.flagCommon.nextAddr / 2 * sizeof(SRC_T) : 
                                                              flag.aRopeAddr - flag.flagCommon.nextAddr / 2 * sizeof(SRC_T);
                         __gm__ SRC_T *nextGmRopeAddr_ = reinterpret_cast<__gm__ SRC_T *>(actualPreloadRopeAddr);
                         aRopeGlobal.SetGlobalBuffer(nextGmRopeAddr_);
                         intriParams.dValue = ROPE_D_64;    // left matrix: k;
                         intriParams.srcNdMatrixStride = 0;
                         intriParams.srcDValue = intriParams.srcDValue >> 1;
                         DataCopy(l1[intriParams.dstNzC0Stride * ROPE_D_128], aRopeGlobal, intriParams);
                     }  else {
                         DataCopy(l1, aGlobal, intriParams);
                     }
