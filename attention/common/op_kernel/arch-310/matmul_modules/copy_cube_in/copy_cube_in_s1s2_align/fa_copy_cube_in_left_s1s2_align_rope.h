...
/*!
 * \file fa_copy_cube_in_left_s1s2_align_rope.h
 * \brief
 */

#ifndef FA_COPY_CUBE_IN_LEFT_S1S2_ALIGN_ROPE_H
#define FA_COPY_CUBE_IN_LEFT_S1S2_ALIGN_ROPE_H
...
class FACopyCubeInLeftS1s2AlignRope {
...
public:
    ...
    __aicore__ inline FACopyCubeInLeftS1s2AlignRope() = default;
    __aicore__ inline ~FACopyCubeInLeftS1s2AlignRope() = default;
    ...
    LoadData(int curRow, int curCol, int tileHeight, int tileWidth, int batchNum = -1) {
        ...
        if constexpr (!PhyPosIsL1(INPUT_TYPE::pos)) {
            ...
            l1 = MATMUL_MODULE(CubeInBuffer)->AllocTensor(flag.baseFlag.leftBufIdx);
            if (!flag.baseFlag.reuseLeft) {
                GlobalTensor<TransT> aGlobal;
                aGlobal.SetGlobalBuffer(srcGlobalAddr_);
                GlobalTensor<TransT> ropeGlobal;
                ropeGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ SrcT*>(flag.qRopeAddr));
                ...
                nd2nzParams.srcDValue = flag.ropeKa;
...
#endif // FA_COPY_CUBE_IN_LEFT_S1S2_ALIGN_ROPE_H
