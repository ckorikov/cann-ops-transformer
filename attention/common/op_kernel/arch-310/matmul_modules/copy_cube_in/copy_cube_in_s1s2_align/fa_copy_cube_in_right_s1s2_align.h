...
/*!
 * \file fa_copy_cube_in_right_s1s2_align.h
 * \brief
 */

#ifndef FA_COPY_CUBE_IN_RIGHT_S1S2_ALIGN_H
#define FA_COPY_CUBE_IN_RIGHT_S1S2_ALIGN_H
...
class FACopyCubeInRightS1s2Align {
...
public:
    ...
    __aicore__ inline FACopyCubeInRightS1s2Align() = default;
    __aicore__ inline ~FACopyCubeInRightS1s2Align() = default;
    ...
    LoadData(int curRow, int curCol, int tileHeight, int tileWidth, int batchNum = -1) {
        ...
        if constexpr (!PhyPosIsL1(INPUT_TYPE::pos)) {
            ...
            if (flag.copyCurrent) {
                ...
                MATMUL_MODULE(CubeInBuffer)->DeQue();
                l1 = MATMUL_MODULE(CubeInBuffer)->GetBuffer(flag.rightBufIdx);
            }
        } else {
            l1.SetAddr(srcAddr_);
        }
    ...
    __aicore__ inline void Destroy() {
        if constexpr (!PhyPosIsL1(INPUT_TYPE::pos)) {
            ...
            FAFlagData flag = MATMUL_MODULE(MatmulUserDefineInfo)->GetSelfDefineData();
            if (flag.copyNext) {
                uint8_t tscmIdx = (flag.rightBufIdx >> 1 << 2) + 1 - flag.rightBufIdx;
...
#endif // FA_COPY_CUBE_IN_RIGHT_S1S2_ALIGN_H
