...
/*!
 * \file fa_copy_cube2_in_right_pa.h
 * \brief
 */
#ifndef FA_COPY_CUBE2_IN_RIGHT_PA_H
#define FA_COPY_CUBE2_IN_RIGHT_PA_H
...
public:
    inline __aicore__ FaCopyCube2InRightPa() = default;
    inline __aicore__ ~FaCopyCube2InRightPa() = default;
    ...
    __aicore__ inline void Init()
    {
        if constexpr (IsSameType<SrcT, int8_t>::value || IsSameType<SrcT, fp8_e5m2_t>::value || 
            ...
            alignNum_ = 32; // 32: 此处对齐方式需要与matmul中L1到L0A/B逻辑保持一致,matmul中tileHeight参数在转置时按照16对齐,非转置时按照c0size对齐.
        } else if constexpr (IsSameType<SrcT, float>::value) {
            alignNum_ = 8; // 8: float32类型，32B对齐需要有8个数
        } else {
            alignNum_ = 16; // 16: FP16, BF16
        }
    ...
    __aicore__ inline LocalTensor<TransT> LoadData(int curRow, int curCol, int tileHeight, int tileWidth,
        int batchNum = 1)
    {
        FaPaFlagData flag = MATMUL_MODULE(MatmulUserDefineInfo)->GetSelfDefineData();
        ...
        while (copyFinishRowCnt < needCopyS2) {
            ...
            if (flag.isLayoutBSH == 1) {
                curOffset = (blockId * flag.blockSize + offsetInBlock) * flag.kvHeadNum * flag.vHeadSize +
                    flag.nIdx * flag.vHeadSize;
            } else {
                curOffset = blockId * flag.blockSize * flag.kvHeadNum * flag.vHeadSize +
                    flag.nIdx * flag.blockSize * flag.vHeadSize + offsetInBlock * flag.vHeadSize;
            }
            if (flag.splitD == 1) {
                curOffset += static_cast<int64_t>(curCol) * static_cast<int64_t>(MATMUL_MODULE(CopyCubeInParams)->GetBaseWidth());
            }
            src.SetGlobalBuffer((__gm__ SrcT *)flag.tensorBAddr, flag.paBlockNumSum * flag.blockSize * flag.kvHeadNum * flag.vHeadSize);
...
#endif // FA_COPY_CUBE2_IN_RIGHT_PA_H