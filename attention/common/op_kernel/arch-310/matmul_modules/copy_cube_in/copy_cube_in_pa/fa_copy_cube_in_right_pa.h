...
/*!
 * \file fa_copy_cube_in_right_pa.h
 * \brief
 */
#ifndef FA_COPY_CUBE_IN_RIGHT_PA_H
#define FA_COPY_CUBE_IN_RIGHT_PA_H
...
public:
    inline __aicore__ FaCopyCubeInRightPa() = default;
    inline __aicore__ ~FaCopyCubeInRightPa() = default;
        ...
        int batchNum = 1)
    {
        ...
        while (copyFinishRowCnt < needCopyS2) {
            ...
            if (flag.isLayoutBSH == 1) {
                curOffset = (blockId * flag.blockSize + offsetInBlock) * flag.kvHeadNum * flag.kHeadSize +
                    flag.nIdx * flag.kHeadSize;
            } else {
                curOffset = blockId * flag.blockSize * flag.kvHeadNum * flag.kHeadSize +
                    flag.nIdx * flag.blockSize * flag.kHeadSize + offsetInBlock * flag.kHeadSize;
            }
            // D = 512,splitD
            if (flag.splitD == 1) {
                curOffset += static_cast<int64_t>(curRow) * static_cast<int64_t>(MATMUL_MODULE(CopyCubeInParams)->GetBaseHeight());
            }
            src.SetGlobalBuffer((__gm__ SrcT *)flag.tensorBAddr, flag.paBlockNumSum * flag.blockSize * flag.kvHeadNum * flag.kHeadSize);
...
#endif // FA_COPY_CUBE_IN_RIGHT_PA_H