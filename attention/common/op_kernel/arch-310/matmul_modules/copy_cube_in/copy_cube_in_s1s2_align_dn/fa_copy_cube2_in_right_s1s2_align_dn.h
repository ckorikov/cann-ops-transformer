...
/*!
 * \file fa_copy_cube2_in_right_s1s2_align_dn.h
 * \brief
 */

#ifndef FA_COPY_CUBE2_IN_RIGHT_S1S2_ALIGN_DN_H
#define FA_COPY_CUBE2_IN_RIGHT_S1S2_ALIGN_DN_H
...
class FACopyCube2InRightS1s2AlignDn {
...
public:
    ...
    __aicore__ inline FACopyCube2InRightS1s2AlignDn() = default;
    __aicore__ inline ~FACopyCube2InRightS1s2AlignDn() = default;
    ...
    LoadData(int curRow, int curCol, int tileHeight, int tileWidth, int batchNum = -1) {
        ...
        l1 = MATMUL_MODULE(CubeInBuffer)->AllocTensor(RIGHT_MATRIX_TSCM_POS + (callTimes_ & 1));
        GlobalTensor<TransT> aGlobal;
        aGlobal.SetGlobalBuffer(srcGlobalAddr_ + (callTimes_ & 1) * MATMUL_MODULE(CopyCubeInParams)->GetBaseHeight() *
            MATMUL_MODULE(CopyCubeInParams)->GetOrgWidth());
...
#endif // FA_COPY_CUBE2_IN_LEFT_S1S2_ALIGN_DN_H
