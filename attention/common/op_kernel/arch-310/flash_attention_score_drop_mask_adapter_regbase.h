...
/*!
 * \file flash_attention_score_drop_mask_adapter_regbase.h
 * \brief
 */
                                         ...
                                         AscendC::TPipe *tPipe)
{
    ...
    pipe->InitBuffer(dropMaskSelResTBuf, tiling->dropmaskParamsRegbase.baseUbCalSize * sizeof(half));
}

__aicore__ inline void FlashAttentionScoreDropMaskAdapterRegbase::Process()
{
...
__aicore__ inline void FlashAttentionScoreDropMaskAdapterRegbase::CopyIn(int64_t offset, int32_t calSize)
{
                                                   ...
                                                   const AscendC::BinaryRepeatParams &binaryRepeatParams)
{
    ...
    for (int32_t idx = 0; idx < loop; ++idx) {
        Select(dropMaskSelRes[SELECT_MAX_REPEAT_MASK * idx], dropMaskUb[SELECT_MAX_OFFSET * idx], dropMaskSelSrc,
    ...
    Select(dropMaskSelRes[SELECT_MAX_REPEAT_MASK * loop], dropMaskUb[SELECT_MAX_OFFSET * loop], dropMaskSelSrc,
...
__aicore__ inline void FlashAttentionScoreDropMaskAdapterRegbase::CopyOut(int64_t offset, int32_t calSize)
{
...
__aicore__ inline void FlashAttentionScoreDropMaskAdapterRegbase::SyncAllCores()
{
    AscendC::SyncAll();
}

#endif // FLASH_ATTENTION_SCORE_DROP_MASK_ADAPTER_REGBASE_H
