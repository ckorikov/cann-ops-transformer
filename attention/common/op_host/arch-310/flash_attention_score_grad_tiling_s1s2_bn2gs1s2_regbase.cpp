...
void FlashAttentionScoreGradTilingUs1s2Bs2Regbase::CalcleTNDCausalDeterPrefix(DeterPrefixData &deterPrefixData, int64_t &m0Max, int64_t &m1Max, int64_t &m2Max) {
    int64_t N12 = fBaseParams.n1 % fBaseParams.aicNum % NUM_TWO;
    for (int64_t i = 0; i < fBaseParams.b; i++) {
        int64_t actualS1Outer =
            CeilDivideBy(fBaseParams.actualSeqQlen[i], fBaseParams.s1Inner * fBaseParams.s1CvRatio);
        int64_t actualS2Outer =
        ...
        m0Max = std::max(m0Max, NUM_TWO * actualS1Outer - actualS2Outer + 1);
        ...
        if (N12 > 0) {
            ...
            m1Max = std::max(m1Max, actualS1Outer - (actualS2Outer + 1) / NUM_TWO + 1);
            ...
            m2Max = std::max(m2Max, actualS1Outer - actualS2Outer / NUM_TWO);
        }
    }
}
...
void FlashAttentionScoreGradTilingUs1s2Bs2Regbase::CalcleTNDCausalDeterParam()
{
    ...
    int64_t N11 = fBaseParams.n1 % fBaseParams.aicNum / NUM_TWO;
    int64_t N12 = fBaseParams.n1 % fBaseParams.aicNum % NUM_TWO;
    DeterPrefixData deterPrefixData;
    ...
    int64_t prefix0Max1 = deterPrefixData.prefix0.back() / NUM_TWO * (fBaseParams.n1 / fBaseParams.aicNum);
    int64_t prefix0Max2 =
        std::max(CeilDivideBy(deterPrefixData.prefix0.back() * N11, static_cast<int64_t>(fBaseParams.aicNum)), m0Max);
    deterPrefixData.prefix0 = SliceVector(deterPrefixData.prefix0, fBaseParams.deterPrefixStep);
    ...
    if (N12 > 0) {
        ...
        deterPrefixData.prefix1.push_back(std::max(CeilDivideBy(deterPrefixData.prefix1.back(), static_cast<int64_t>(fBaseParams.aicNum)), m1Max));
        deterPrefixData.prefix2.push_back(std::max(CeilDivideBy(deterPrefixData.prefix2.back(), static_cast<int64_t>(fBaseParams.aicNum)), m2Max));
...
void FlashAttentionScoreGradTilingUs1s2Bs2Regbase::CalcleTNDBandDeterPrefix(
    DeterPrefixData &deterPrefixData, int64_t N11, int64_t N12, int64_t &mnMax)
{
    for (int64_t i = 0; i < fBaseParams.b; i++) {
...
void FlashAttentionScoreGradTilingUs1s2Bs2Regbase::CalcleTNDBandDeterParam()
{
    ...
    DeterPrefixData deterPrefixData;
    CalcleTNDBandDeterPrefix(deterPrefixData, N11, N12, mnMax);
...
uint64_t FlashAttentionScoreGradTilingUs1s2Bs2Regbase::GetTilingKey() const
{
    ...
    OP_LOGI(context_, "splitAxis[%d], inputDtype[%d], isTnd[%d], dropValue[%d], pseValue[%d], attenMaskCfg[%d], s1TemplateType[%d], s2TemplateType[%d], dTemplateType[%u], isDeterministic[%d], hasTail[%d], dNoEqual[%d], hasRope[%d], outDtype[%d], fp8OpenTscm[%d], isRegbasePlatformValue[%d]",
                    ...
                    fBaseParams.deterSparseType, hasTail, dNoEqual, static_cast<int>(fBaseParams.hasRope), static_cast<int>(fBaseParams.outDtype),  static_cast<int>(fBaseParams.fp8OpenTscm), static_cast<int>(isRegbasePlatformValue));
...
bool FlashAttentionScoreGradTilingUs1s2Bs2Regbase::CheckFuzzyArgsLegal(uint32_t s1Inner, uint32_t s2Inner)
{
    return true;
}
    ...
    std::vector<std::vector<float>> &acturalBlockInfo)
{
    ...
    for (int64_t i = 0; i < fBaseParams.b; i++) {
        ...
        OP_LOGD("FillBlockInfoLoadBalance",
                  " b idx = %d: actualS1Len = %d, actualS2Len = %d, actualCalcS1Token = %ld, actualCalcS2Token = %ld",
    ...
    for (int64_t i = 0; i < bn2g; i++) {
        ...
        for (int64_t j = 0; j < fBaseParams.s2Outer; j++) {
            ...
            if (coreIdx >= CORE_LIST_NUM) {
                ...
                OP_LOGD("GetBlockInfoOfBNS4TND", " blockIdx = %ld: acturalBlock = %ld", coreIdx, currentSum);
