...
static void SplitCore(const BaseInfo &baseInfo, const InnerSplitParams &innerSplitParams, uint32_t coreNum, OuterSplitParams outerSplitParams, FlashDecodeParams fDParams, SplitCoreRes &res) {
            ...
            for (uint32_t s2Idx = 0; s2Idx < s2BaseNum[bIdx]; s2Idx++) {
                ...
                if (accumBaseNum >= targetBaseNum) {
                    ...
                    if (s2Idx + 1 < s2BaseNum[bIdx]) {
                        ...
                        if (s1GIdx + 1 < s1GBaseNum[bIdx]) {
                            outerSplitParams.bN2End[currCoreIdx] = bN2Idx;
                            outerSplitParams.gS1End[currCoreIdx] = s1GIdx + 1;
                            outerSplitParams.s2End[currCoreIdx] = 0;
                        } else {
                            outerSplitParams.bN2End[currCoreIdx] = bN2Idx + 1;
                            outerSplitParams.gS1End[currCoreIdx] = 0;
                            outerSplitParams.s2End[currCoreIdx] = 0;
                        }
                    }
                    currCoreIdx += 1;
    ...
    if (accumBaseNum < targetBaseNum) {
        // 更新最后一个核的End分核信息
        outerSplitParams.bN2End[currCoreIdx] = baseInfo.bSize * baseInfo.n2Size;
        outerSplitParams.gS1End[currCoreIdx] = 0;
        outerSplitParams.s2End[currCoreIdx] = 0;
        currCoreIdx += 1;
    }
