...
public:
    ...
    __aicore__ inline void InitVector2GM(const GlobalTensor<MM5_OUT_T> &bmm5Res, const GlobalTensor<int32_t> &topK,
                                    GlobalTensor<OUT_T> &dKeyIndex);

private:
    // =============== vector 0 functions ==============
    __aicore__ inline void Gather(const GlobalTensor<KV_T> &gatherRes, GlobalTensor<KV_T> &srcTensor,
        GlobalTensor<KV_T> &srcRopeTensor, const GatherParams &gatherParams, const SLIGradKLLossRunInfo &runInfo);
    __aicore__ inline void Gather(const GlobalTensor<KV_T> &gatherRes, GlobalTensor<KV_T> &srcTensor,
        const GatherParams &gatherParams, const SLIGradKLLossRunInfo &runInfo);

    template <event_t IdStart>
    ...
    __aicore__ inline void DataCopyReluRes(LocalTensor<T> &reluResTensor, GlobalTensor<T> &reluGm, SLIGradKLLossRunInfo &runInfo, int64_t reluResOffset);
    __aicore__ inline void VectorDwDqDk(SLIGradKLLossRunInfo &runInfo);
    __aicore__ inline void VectorLoss(SLIGradKLLossRunInfo &runInfo);
    // =============== vector 2 functions ==============
    ...
    // =============== vector1 variable ==============
    LocalTensor<Q_T> weightTmpBuffer;
...
__aicore__ inline void SLIKLLossVectorService<SLIT>::InitVector2GM(const GlobalTensor<MM5_OUT_T> &bmm5Res,
                                                        const GlobalTensor<int32_t> &topK,
                                                        GlobalTensor<OUT_T> &dKeyIndex)
{
    this->bmm5ResGm = bmm5Res;
    this->topKGm = topK;
    this->dKeyIndexGm = dKeyIndex;
}
...
__aicore__ inline void SLIKLLossVectorService<SLIT>::InitBuffers(TPipe *pipe)
{
    ...
    pipe->InitBuffer(this->gatherTbuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_11K * 2);  // 2:pingpong
    pipe->InitBuffer(this->mm1Tbuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_64K);
    pipe->InitBuffer(this->mm2TBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_32K);
    pipe->InitBuffer(this->sharedTBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_32K);
    pipe->InitBuffer(this->reduceSumPTBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_8K);
    pipe->InitBuffer(this->reduceSumYTBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_8K);
    pipe->InitBuffer(this->v1TmpTBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_8K);
    pipe->InitBuffer(this->reluGradTBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_4K);
    pipe->InitBuffer(this->lossSumTBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_64);
    pipe->InitBuffer(this->weightTBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_512);
    pipe->InitBuffer(this->dwTBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_4K);
    pipe->InitBuffer(this->maskBuf, SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_256);

    gatherKeyUb = gatherTbuf.Get<KV_T>();
    gatherKeyIndexUb = gatherKeyUb[SLIGradKLLossConstInfo::BUFFER_SIZE_BYTE_9K * 2];

    weightTmpBuffer = sharedTBuf.GetWithOffset<Q_T>(AlignTo(constInfo.gSizeQueryIndex, static_cast<uint32_t>(16)), 8*1024);
    weightUb = weightTBuf.Get<T>();
    reluResUb = mm2TBuf.Get<T>();
    reduceSumTmpBuffer = sharedTBuf.GetWithOffset<uint8_t>(8*1024, 8*1024);
    reduceSumYResTmpBuffer = sharedTBuf.GetWithOffset<T>(constInfo.kSize, 3*8*1024);
    reduceSumYResUb = reduceSumYTBuf.Get<T>();
...
__aicore__ inline void SLIKLLossVectorService<SLIT>::ProcessVector0(SLIGradKLLossRunInfo &runInfo)
{
    ...
    for (int32_t i = 0; i < runInfo.s2LoopTimes; ++i) {
        ...
        Gather(gatherPResGm, keyGm, keyRopeGm, gatherParams, runInfo);
        CrossCoreSetFlag<2, PIPE_MTE3>(SYNC_V0_TO_C1_P_FLAG[runInfo.taskIdMod2][i & 1]);
    }
    ...
    for (int32_t i = 0; i < runInfo.s2LoopTimes; ++i) {
        ...
        Gather(gatherSYResGm, keyIndexGm, gatherParams, runInfo);
        CrossCoreSetFlag<2, PIPE_MTE3>(SYNC_V0_TO_C1_SY_FLAG[runInfo.taskIdMod2][i & 1]);
    }
}

template <typename SLIT>
__aicore__ inline void
SLIKLLossVectorService<SLIT>::Gather(const GlobalTensor<KV_T> &gatherRes, GlobalTensor<KV_T> &srcTensor,
    GlobalTensor<KV_T> &srcRopeTensor, const GatherParams &gatherParams, const SLIGradKLLossRunInfo &runInfo)
{
    this->kvMergeGm_ = gatherRes;
    this->srcTensor = srcTensor;
    this->srcRopeTensor = srcRopeTensor;
    this->gatherParams = gatherParams;

    this->kvMergUb_ = gatherKeyUb;
    this->ropeMergUb_ = gatherKeyUb[UB_ROW_SIZE * gatherParams.dValue * 2];

    needGatherRope = SLIT::hasRope;
    gatherResGmDb = constInfo.gatherKeyDbNum;
    gatherResGmEleSize = constInfo.gatherKeySize;

    MergeKv<EVENT_ID0>(runInfo);
}
    ...
    const GatherParams &gatherParams, const SLIGradKLLossRunInfo &runInfo)
{
    ...
    needGatherRope = false;
    gatherResGmDb = constInfo.gatherKeyIndexDbNum;
    gatherResGmEleSize = constInfo.gatherKeyIndexSize;

    MergeKv<EVENT_ID2>(runInfo);
}
                                       ...
                                       int64_t keyBNBOffset, int64_t s2IdLimit, const SLIGradKLLossRunInfo &runInfo)
{
    ...
    DataCopyPad(kvMergUb_[mergeMte3Idx % 2 * UB_ROW_SIZE * gatherParams.dValue + (mte2Size - mte3Size) * gatherParams.dValue],
    ...
    if (needGatherRope) {
        ...
        DataCopyPad(ropeMergUb_[mergeMte3Idx % 2 * UB_ROW_SIZE * gatherParams.dRopeValue + (mte2Size - mte3Size) * gatherParams.dRopeValue],
    ...
    const SLIGradKLLossRunInfo &runInfo, int64_t s2IdLimit)
{
    ...
    int64_t tensorBRopeOffset;
    if constexpr (KV_LAYOUT_T == SLILayout::TND) {
        actualSeqKVPrefixSum = (runInfo.bIdx <= 0) ? 0 : actualSeqLengthsKVGm.GetValue(runInfo.bIdx - 1);
    } else {
        actualSeqKVPrefixSum = (runInfo.bIdx <= 0) ? 0 : runInfo.bIdx * constInfo.s2Size;
    }
    tensorBRopeOffset = actualSeqKVPrefixSum * gatherParams.dRopeValue;

    realKeyRopeGmOffset = (tensorBRopeOffset + realS2Idx * gatherParams.dRopeValue) / gatherParams.dRopeValue;
    ...
    const SLIGradKLLossRunInfo &runInfo, int64_t s2IdLimit)
{
    ...
    tensorBOffset = actualSeqKVPrefixSum * gatherParams.dValue;
    realKeyGmOffset = (tensorBOffset + realS2Idx * gatherParams.dValue) / gatherParams.dValue;
    ...
    int64_t s2GmStartOffset, int64_t mergeMte3Idx, const SLIGradKLLossRunInfo &runInfo)
{
    ...
    SetFlag<AscendC::HardEvent::MTE2_MTE3>(mergeMte3Idx % 2 + IdStart);
    WaitFlag<AscendC::HardEvent::MTE2_MTE3>(mergeMte3Idx % 2 + IdStart);
    ...
    int64_t gmStartOffset = runInfo.taskId % gatherResGmDb * gatherResGmEleSize +
    ...
    DataCopyPad(kvMergeGm_[gmStartOffset + (s2GmStartOffset + mte3Size) * gatherParams.dValue],
                kvMergUb_[mergeMte3Idx % 2 * UB_ROW_SIZE * gatherParams.dValue], dataCopyParams);
    ...
    if (needGatherRope) {
        ...
        DataCopyPad(kvMergeGm_[gmStartOffset + constInfo.s2BaseSize * gatherParams.dValue + (s2GmStartOffset + mte3Size) *
            gatherParams.dRopeValue], ropeMergUb_[mergeMte3Idx % 2 * UB_ROW_SIZE * gatherParams.dRopeValue], dataCopyParams);
    }
}

template <typename SLIT>
__aicore__ inline void SLIKLLossVectorService<SLIT>::GetRealS2Idx(int64_t s2GmOffset, int64_t &realS2Idx,
    int64_t topkGmBaseOffset, const SLIGradKLLossRunInfo &runInfo)
{
    ...
    realS2Idx = topKGm.GetValue(topkGmBaseOffset + topkGmIdx) * static_cast<int64_t>(constInfo.sparseBlockSize) +
...
__aicore__ inline void SLIKLLossVectorService<SLIT>::MergeKv(const SLIGradKLLossRunInfo &runInfo)
{
    ...
    if constexpr (LAYOUT_T == SLILayout::TND) {
        ...
        topkGmBaseOffset += (actualSeqQPrefixSum + runInfo.s1Idx) * constInfo.kSize;
    } else {
        topkGmBaseOffset += runInfo.bIdx * constInfo.s1Size * constInfo.kSize + runInfo.s1Idx * constInfo.kSize;
    }
    ...
    SetFlag<AscendC::HardEvent::MTE3_MTE2>(IdStart);
    SetFlag<AscendC::HardEvent::MTE3_MTE2>(IdStart + 1);
    ...
    for (int64_t s2GmOffsetArray = s2GmStartOffset; s2GmOffsetArray < s2GmLimit; s2GmOffsetArray += 2 * constInfo.sparseBlockSize) {
        if (needWaitMte3ToMte2) {
            WaitFlag<AscendC::HardEvent::MTE3_MTE2>(mergeMte3Idx % 2 + IdStart);
            needWaitMte3ToMte2 = false;
        }
        GetRealS2Idx(s2GmOffsetArray, s2IdxArray0, topkGmBaseOffset, runInfo);
        if (unlikely(s2IdxArray0 < 0)) {
            CopyOutMrgeResult<IdStart>(mte2Size, mte3Size, s2GmStartOffset, mergeMte3Idx, runInfo);
            SetFlag<AscendC::HardEvent::MTE3_MTE2>(mergeMte3Idx % 2 + IdStart);
            mergeMte3Idx++;
            break;
        }
    ...
    WaitFlag<AscendC::HardEvent::MTE3_MTE2>(IdStart);
    WaitFlag<AscendC::HardEvent::MTE3_MTE2>(IdStart + 1);
    return;
}
...
{
    int32_t pingpong = runInfo.taskId % 2;
    ...
    if ((pingpong == 0 && constInfo.subBlockIdx == 0) || (!(pingpong == 0) && constInfo.subBlockIdx != 0)) {
        VectorP(runInfo);
    } else {
    ...
    VectorDwDqDk(runInfo); // Sub + Mul + Mul + ReluGrad
    VectorLoss(runInfo);
    AscendC::SetFlag<AscendC::HardEvent::MTE3_V>(eventIdMTE3ToVTmp);
    ...
    GetTPipePtr()->ReleaseEventID<AscendC::HardEvent::V_MTE2>(eventIdVToMte2Ping);
}

template <typename SLIT> 
__aicore__ inline void SLIKLLossVectorService<SLIT>::VectorP(SLIGradKLLossRunInfo &runInfo)
{
    int64_t calcPBaseSize = runInfo.nBaseSizeP * runInfo.kBaseSize;
    int64_t bmm1ResSize = constInfo.gSizeQuery * runInfo.kBaseSize;
    int64_t nLoops = CeilDiv(constInfo.gSizeQuery, runInfo.nBaseSizeP); // 128 / 32 = 4
    int64_t kLoops = CeilDiv(runInfo.kRealSize, runInfo.kBaseSize); // 2048 / 1024 = 2
    ...
    reduceSumPUb = reduceSumPTBuf.template Get<T>();
    LocalTensor<T> mulsResUb = mm1Tbuf.Get<T>();
    LocalTensor<uint8_t> sharedTmpBuffer = sharedTBuf.GetWithOffset<uint8_t>(16 * 1024, 0);
    LocalTensor<T> reduceSumPTmpBuffer = sharedTBuf.GetWithOffset<T>(2048, 16 * 1024);

    AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(eventIdVToMte2Ping);
    for (int64_t n = 0; n < nLoops; n++) { // nLoops=4
        ...
        int64_t bmm1NOffset = n * runInfo.nBaseSizeP * runInfo.kBaseSize;
        AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(eventIdVToMte2Ping);
        DataCopyExtParams copyParams = {static_cast<uint16_t>(curNSize),
                                        static_cast<uint32_t>(runInfo.kRealSizeAlign8 * sizeof(T)),
                                        static_cast<uint32_t>((runInfo.kBaseSize - runInfo.kRealSizeAlign8) * sizeof(T)), 0, 0};
        DataCopyPadExtParams<T> copyPadParams = {false, 0, 0, 0};
        AscendC::DataCopyPad(mulsResUb, bmm1ResGm[bmm1Offset + bmm1NOffset], copyParams, copyPadParams);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(eventIdMte2ToV);
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(eventIdMte2ToV);
        AscendC::Muls(mulsResUb, mulsResUb, constInfo.scaleValue, curNSize * runInfo.kRealSizeAlign8);
        PipeBarrier<PIPE_V>();
        LocalTensor<T> softmaxMaxTmp = sharedTBuf.GetWithOffset<T>(runInfo.nBaseSizeP, 0);
        LocalTensor<T> softmaxSumTmp = sharedTBuf.GetWithOffset<T>(runInfo.nBaseSizeP, runInfo.nBaseSizeP * sizeof(T));
        LocalTensor<T> softmaxMaxUb = sharedTBuf.GetWithOffset<T>(runInfo.nBaseSizeP * 8, 1024);
        LocalTensor<T> softmaxSumUb = sharedTBuf.GetWithOffset<T>(runInfo.nBaseSizeP * 8, 2048);
        ...
        AscendC::DataCopyPad(softmaxMaxTmp, softmaxMaxGm[softmaxInputOffset], softmaxCopyParams, softmaxCopyPadParams);
        AscendC::DataCopyPad(softmaxSumTmp, softmaxSumGm[softmaxInputOffset], softmaxCopyParams, softmaxCopyPadParams);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(eventIdMte2ToV);
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(eventIdMte2ToV);
        AscendC::Brcb(softmaxMaxUb, softmaxMaxTmp, curNSize, {1, 8});
        AscendC::Brcb(softmaxSumUb, softmaxSumTmp, curNSize, {1, 8});
        ...
        AscendC::SimpleSoftMax<T>(mulsResUb, softmaxSumUb, softmaxMaxUb, mulsResUb,
                                  softmaxTmpUb, simpleSoftMaxTilingInfo, 
                                  {static_cast<uint32_t>(curNSize), static_cast<uint32_t>(runInfo.kRealSizeAlign8),
                                  static_cast<uint32_t>(curNSize), static_cast<uint32_t>(runInfo.kRealSize)});
        PipeBarrier<PIPE_V>();
        uint32_t reduceShape[] = {static_cast<uint32_t>(runInfo.nBaseSizeP), static_cast<uint32_t>(runInfo.kRealSizeAlign8)};
        AscendC::ReduceSum<T, AscendC::Pattern::Reduce::RA, true>(reduceSumPTmpBuffer, mulsResUb, sharedTmpBuffer, reduceShape, true);
        PipeBarrier<PIPE_V>();
        
        AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(eventIdVToMte2Ping);
        if (n == 0) {
            AscendC::DataCopy(reduceSumPUb, reduceSumPTmpBuffer, runInfo.kRealSizeAlign8);
        } else {
            AscendC::Add(reduceSumPUb, reduceSumPUb, reduceSumPTmpBuffer, runInfo.kRealSizeAlign8);
        }
        PipeBarrier<PIPE_V>();
    }
    PipeBarrier<PIPE_V>();
    float gRec = 1.0f / static_cast<float>(static_cast<int64_t>(constInfo.gSizeQuery));
    int64_t realKSize = 2048;
    AscendC::Muls(reduceSumPUb, reduceSumPUb, gRec, realKSize);
    PipeBarrier<PIPE_V>();
    AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(eventIdVToMte2Ping);
    ...
    AscendC::DataCopy(psySyncGm, reduceSumPUb, runInfo.kRealSizeAlign8);
...
__aicore__ inline void SLIKLLossVectorService<SLIT>::PreloadWeight(SLIGradKLLossRunInfo &runInfo)
{
    ...
    int64_t weightDBOffset = pingpong * AlignTo(constInfo.gSizeQueryIndex, static_cast<uint32_t>(16));
    // weight 可以常驻, 所以直接搬运, 减少搬运切片
    AscendC::DataCopy(weightTmpBuffer[weightDBOffset], weightGm[weightOffset], AlignTo(constInfo.gSizeQueryIndex, static_cast<uint32_t>(16)));
    AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(eventIdMte2ToV4SY);
    AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(eventIdMte2ToV4SY);
    AscendC::Cast(weightUb[weightDBOffset], weightTmpBuffer[weightDBOffset], AscendC::RoundMode::CAST_NONE, constInfo.gSizeQueryIndex);
    PipeBarrier<PIPE_V>();
}
...
__aicore__ inline void SLIKLLossVectorService<SLIT>::VectorSy(SLIGradKLLossRunInfo &runInfo)
{
    ...
    int64_t pingpong = runInfo.taskIdMod2;
    // 切B, S1做循环, bmm2 单次存储 [N1_INDEX, TopK] 大小的数据块
    // 为了softmax K轴尽量不切分, 只能在N1方向上切
    int64_t bmm2ResSize = constInfo.gSizeQueryIndex * constInfo.kSize; 
    int64_t bmm2ResOffset = pingpong * bmm2ResSize;
    // relu无法复用LocalTensor 所以再 8/ 2 = 4行
    int64_t nSplitSize = (64*1024) / constInfo.kSize / sizeof(float) / 2;
    ...
    for (int64_t n = 0; n < nLoopSize; n++) {
        ...
        DataCopyExtParams copyBmm2Params = {static_cast<uint16_t>(nSplitSize), static_cast<uint32_t>(realKSize * sizeof(T)), static_cast<uint32_t>((2048 - realKSize) * sizeof(T)), 0, 0};
        ...
        AscendC::ReduceSum<T, AscendC::Pattern::Reduce::RA, isReuse>(reduceSumYResTmpBuffer, reluResUb, reduceSumTmpBuffer, reduceShape, true);
        AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(eventIdVToMte24SY);
        PipeBarrier<PIPE_V>();

        if (n == 0) {
            AscendC::DataCopy(reduceSumYResUb, reduceSumYResTmpBuffer, realKSizeAlign);
        } else {
            AscendC::Add(reduceSumYResUb, reduceSumYResUb, reduceSumYResTmpBuffer, realKSizeAlign);
        }
        PipeBarrier<PIPE_V>();
    }
...
__aicore__ inline void SLIKLLossVectorService<SLIT>::ReLUGrad(LocalTensor<KV_T> &reluGradOutTensor, LocalTensor<T> &ReLuTensor,
    LocalTensor<T> &subResTensor, SLIGradKLLossRunInfo &runInfo)
{
    // maskTensor需要创建  subResTensor：mul之后的结果
    LocalTensor<T> maskUb = this->maskBuf.template Get<T>();
    CompareScalar(maskUb, subResTensor, static_cast<T>(0.0), AscendC::CMPMODE::GT, constInfo.kSize);
    PipeBarrier<PIPE_V>();
    //UB 新建了一个 maskBuffer
    Select(ReLuTensor, maskUb, ReLuTensor, static_cast<T>(0.0), AscendC::SELMODE::VSEL_TENSOR_SCALAR_MODE, runInfo.kRealSizeAlign8);
    PipeBarrier<PIPE_V>();
    Cast(reluGradOutTensor, ReLuTensor, AscendC::RoundMode::CAST_ROUND, runInfo.kRealSizeAlign8);
    PipeBarrier<PIPE_V>();
}

template <typename SLIT>
__aicore__ inline void SLIKLLossVectorService<SLIT>::DataCopyReluRes(LocalTensor<T> &reluResTensor, GlobalTensor<T> &reluGm,
    SLIGradKLLossRunInfo &runInfo, int64_t reluResOffset) {
    constexpr static int32_t blockSize = blockBytes / sizeof(T);
    if (likely(runInfo.kRealSizeAlign8 == runInfo.kRealSize)) {
        DataCopy(reluResTensor, reluGm[reluResOffset], runInfo.kRealSize);
    } else {
        ...
        dataCopyParams.dstStride = 0;
        dataCopyParams.srcStride = 0;
        dataCopyParams.blockLen = runInfo.kRealSize * sizeof(T);
        dataCopyPadParams.rightPadding = runInfo.kRealSizeAlign8 - runInfo.kRealSize;
...
template <typename SLIT> 
__aicore__ inline void SLIKLLossVectorService<SLIT>::VectorDwDqDk(SLIGradKLLossRunInfo &runInfo)
{
    LocalTensor<T> v1TmpUb = v1TmpTBuf.template Get<T>();
    LocalTensor<T> reluResUb = mm2TBuf.template Get<T>();
    LocalTensor<T> reduceSumPUb = reduceSumPTBuf.template Get<T>();
    LocalTensor<T> reduceSumYUb = reduceSumYTBuf.template Get<T>();
    LocalTensor<T> weightTensor = weightTBuf.template Get<T>();
    LocalTensor<KV_T> reluGradUb = reluGradTBuf.template Get<KV_T>();
    ...
    if (runInfo.taskId % 2 == 0) {
        if (constInfo.aivIdx % 2 == 0) {
            DataCopyReluRes(reduceSumYUb, psySyncOtherGm, runInfo, constInfo.kSize);
        } else {
            DataCopyReluRes(reduceSumPUb, psySyncOtherGm, runInfo, 0);
        }
    } else {
        if (constInfo.aivIdx % 2 == 0) {
            DataCopyReluRes(reduceSumPUb, psySyncOtherGm, runInfo, 0);
    ...
    Sub(v1TmpUb, reduceSumYUb, reduceSumPUb, runInfo.kRealSize);
    PipeBarrier<PIPE_V>();
    SumParams sumParams;
    sumParams.outter = 1;
    sumParams.inner = runInfo.kRealSizeAlign8;
    sumParams.n = runInfo.kRealSize;

    DataCopyParams dataCopyReluGradParams;
    dataCopyReluGradParams.blockLen = runInfo.kRealSizeAlign8 * sizeof(KV_T);
    dataCopyReluGradParams.srcStride = 0;
    dataCopyReluGradParams.blockCount = 1;
    dataCopyReluGradParams.dstStride = 0;

    int64_t reluGmOffset = runInfo.taskIdMod2 * constInfo.gSizeQueryIndex * constInfo.kSize;
    ...
    for (size_t nIdx = 0; nIdx < constInfo.gSizeQueryIndex; nIdx++) {
        ...
        DataCopyReluRes(reluResUb, reluGm, runInfo, reluResOffset);

        SetFlag<HardEvent::MTE2_V>(mte2ToV);
        WaitFlag<HardEvent::MTE2_V>(mte2ToV);
        Mul(mulLeftUb, v1TmpUb, reluResUb, runInfo.kRealSizeAlign8);
        PipeBarrier<PIPE_V>();

        AscendC::Sum(reduceSumResTensor[nIdx * 16], mulLeftUb, tmpUb, sumParams);
        PipeBarrier<PIPE_V>();

        Muls(mulLeftUb, v1TmpUb, weightTensor.GetValue(nIdx + weightOffset), runInfo.kRealSizeAlign8);
        PipeBarrier<PIPE_V>();
        WaitFlag<HardEvent::MTE3_V>(mte3ToV);
        ReLUGrad(reluGradUb, mulLeftUb, reluResUb, runInfo);
        SetFlag<HardEvent::V_MTE3>(vToMte3);
        WaitFlag<HardEvent::V_MTE3>(vToMte3);
        DataCopyPad(reluGradResGm[reluResOffset], reluGradUb, dataCopyReluGradParams);
        SetFlag<HardEvent::MTE3_V>(mte3ToV);
    }
    WaitFlag<HardEvent::MTE3_V>(mte3ToV);
    Cast(dwOutTensor, reduceSumResTensor, AscendC::RoundMode::CAST_ROUND, constInfo.gSizeQueryIndex * 16);
    SetFlag<HardEvent::V_MTE3>(vToMte3);
    WaitFlag<HardEvent::V_MTE3>(vToMte3);
    ...
    dataCopyParams.srcStride = 0;
    dataCopyParams.blockCount = constInfo.gSizeQueryIndex;
    dataCopyParams.dstStride = 0;
    if (constInfo.aivIdx % 2 == 0) {
        if constexpr (LAYOUT_T == SLILayout::BSND) {
            DataCopyPad(dWeightGm[runInfo.bIdx * (constInfo.s1Size * constInfo.gSizeQueryIndex) +
                runInfo.s1Idx * constInfo.gSizeQueryIndex], dwOutTensor, dataCopyParams);
        } else if constexpr (LAYOUT_T == SLILayout::TND) {
            DataCopyPad(this->dWeightGm[runInfo.accumS1Idx * constInfo.gSizeQueryIndex], dwOutTensor, dataCopyParams);
        }
    }
...
template <typename SLIT> 
__aicore__ inline void SLIKLLossVectorService<SLIT>::VectorLoss(SLIGradKLLossRunInfo &runInfo)
{
    int32_t pingpong = runInfo.taskId % 2;
    int64_t realKSize = runInfo.kRealSize;
    ...
    // 关注下这块workspace后续是否会开pingpong，开了的话同步适配
    GlobalTensor<T> pSyncGm = psySyncOtherGm[coreOffset];
    GlobalTensor<T> sySyncGm = psySyncOtherGm[constInfo.kSize + coreOffset];
    LocalTensor<T> copyInUb, reduceSumPTmpUb, reduceSumYResTmpUb;
    event_t EVENT_ID_IN;

    if ((pingpong == 0 && constInfo.subBlockIdx == 0) || (pingpong == 1 && constInfo.subBlockIdx == 1)) {  // 当前核做的是VectorP
        copyInUb = mm2TBuf.template Get<T>();
        EVENT_ID_IN = eventIdVToMte24SY;
        SetFlag<HardEvent::V_MTE2>(EVENT_ID_IN);
        WaitFlag<HardEvent::V_MTE2>(EVENT_ID_IN);
        DataCopy(copyInUb, sySyncGm, vRealKSizeAlign);
        reduceSumPTmpUb = reduceSumPTBuf.Get<T>();
        reduceSumPTmpUb = reduceSumPTmpUb[coreOffset];
        reduceSumYResTmpUb = copyInUb;
    } else {  // 当前核做的是VectorSY
        copyInUb = mm1Tbuf.template Get<T>();
        EVENT_ID_IN = eventIdVToMte2Ping;
        SetFlag<HardEvent::V_MTE2>(EVENT_ID_IN);
        WaitFlag<HardEvent::V_MTE2>(EVENT_ID_IN);
        DataCopy(copyInUb, pSyncGm, vRealKSizeAlign);
        reduceSumPTmpUb = copyInUb;
        reduceSumYResTmpUb = reduceSumYTBuf.Get<T>();
        reduceSumYResTmpUb = reduceSumYResTmpUb[coreOffset];
    }

    SetFlag<HardEvent::MTE2_V>(EVENT_ID_IN);
    WaitFlag<HardEvent::MTE2_V>(EVENT_ID_IN);
