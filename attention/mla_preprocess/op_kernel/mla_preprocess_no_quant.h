...
/*!
 * \file mla_preprocess_no_quant.h
 * \brief mla_preprocess_no_quant.h
 */
...
public:
    __aicore__ inline RopeBf16_no_quant() : blockIdx_(AscendC::GetBlockIdx())
    {
    }
...
__aicore__ inline void ReduceSumCustomNoQuant(const AscendC::LocalTensor<float> &dst_local,
...
template <typename T, bool WITH_BETA, bool FastComputeMode = false> class RmsNormQuant_no_quant {
public:
    __aicore__ inline RmsNormQuant_no_quant()
    {
    }

    __aicore__ inline void Init(AscendC::GlobalTensor<T> gammaGmTensor, AscendC::GlobalTensor<half> betaGmTensor,
                                AscendC::GlobalTensor<half> quantScaleGmTensor,
                                AscendC::GlobalTensor<int8_t> quantOffsetGmTensor,
                                AscendC::GlobalTensor<T> inputGmTensor, AscendC::GlobalTensor<bfloat16_t> outputGmTensor,
    ...
    Launch(const AscendC::LocalTensor<int8_t> &dstTensor, const AscendC::LocalTensor<T> &srcTensor,
           const AscendC::LocalTensor<T> &gammaTensor, const AscendC::LocalTensor<half> &betaTensor,
           const AscendC::LocalTensor<half> &quantScaleTensor, const AscendC::LocalTensor<int8_t> &quantOffsetTensor,
           const AscendC::LocalTensor<float> &res1Tensor, const AscendC::LocalTensor<float> &res3Tensor)
    {
        ...
        AscendC::LocalTensor<float> sum = buf[OFFSET_WORKSPACE_NO_QUANT * num_col_align_withStride_fp32]; // 4
        ...
        while (pid < row_work_) {
            ...
            AscendC::PipeBarrier<PIPE_V>();
            ReduceSumCustomNoQuant(sum, sqx, work, num_col_ - input_stride_);
            AscendC::PipeBarrier<PIPE_V>();
            ...
            Mul(fp32_xy, fp32_xy, g, REPEAT_TIME_64, num_col_align_withStride_fp32 / REPEAT_TIME_64,
                {1, 1, 1, AscendC::DEFAULT_REPEAT_STRIDE, AscendC::DEFAULT_REPEAT_STRIDE,
                 AscendC::DEFAULT_REPEAT_STRIDE});
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::LocalTensor<bfloat16_t> tmpfp16 =
                buf.ReinterpretCast<bfloat16_t>()[OFFSET_SUM * num_col_align_withStride_fp32 * CONST_2];
            CastFrom32To16(tmpfp16, fp32_xy, num_col_align_withStride_fp32);
            AscendC::PipeBarrier<PIPE_V>();
            SET_FLAG(V, MTE3, EVENT_ID0);
            WAIT_FLAG(V, MTE3, EVENT_ID0);
            const int32_t subNum = 16;
            AscendC::DataCopy(outputGmTensor[gm_out_offset_ + outOffset], tmpfp16,
                              AscendC::DataCopyParams(1, (num_col_ - input_stride_) / subNum, 0, 0));
...
private:
    ...
    AscendC::GlobalTensor<bfloat16_t> outputGmTensor;
...
template <typename InDtype, typename ScaleDtype> class EinSumQuant_no_quant {
public:
    __aicore__ explicit EinSumQuant_no_quant()
    {
    }
                                ...
                                const MlaTilingData &tilingData)
    {
        ...
        currentIdx = (AscendC::GetBlockIdx() / CONST_2) * CONST_2 + GetSubBlockidx();
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t ubLoopIdx = 0; ubLoopIdx < ubHeadLoop; ubLoopIdx++) {
            ...
            AscendC::Brcb(scaleBrcbFp16_, scaleTensor_, headPerLoop / BMM3SPLIT, {1, 8});
            AscendC::PipeBarrier<PIPE_V>();
        ...
        if (headTail > 0) {
            ...
            WAIT_FLAG(MTE2, V, EVENT_ID1);
            // scale broadcast [H', 1] --> [H', 16]
            AscendC::Brcb(scaleBrcbFp16_, scaleTensor_, padLen / BMM3SPLIT, {1, 8});
            AscendC::PipeBarrier<PIPE_V>();
...
public:
    __aicore__ explicit PpMatmulEinSum_no_quant(){};

    __aicore__ inline void Init(GM_ADDR gmA, GM_ADDR gmB, GM_ADDR gmC, const MlaTilingData &mlaParams, uint32_t mode);

    __aicore__ inline void Process();
    __aicore__ inline void PreloadB();

private:
    __aicore__ inline void GetBaseBlockIdx(uint64_t index, MatCoordNoQuant &tidx);
...
private:
    ...
    uint32_t k0{0};
    uint32_t n0{0};
    MatCoordNoQuant tdim{0};
    MatCoordNoQuant fdim{0};
...
template <DataFormat formatB, bool transB, uint32_t swizzleDirect, uint64_t splitGapA, uint64_t splitGapC, bool ein>
__aicore__ inline void
PpMatmulEinSum_no_quant<formatB, transB, swizzleDirect, splitGapA, splitGapC, ein>::Init(GM_ADDR gmA, GM_ADDR gmB, GM_ADDR gmC,
                                                                           const MlaTilingData &mlaParams, uint32_t mode)
{
#ifdef __DAV_C220_CUBE__
    MM_mode = mode;
    if (mode == 0) {
...
template <DataFormat formatB, bool transB, uint32_t swizzleDirect, uint64_t splitGapA, uint64_t splitGapC, bool ein>
__aicore__ inline void
PpMatmulEinSum_no_quant<formatB, transB, swizzleDirect, splitGapA, splitGapC, ein>::GetBaseBlockIdx(uint64_t index, MatCoordNoQuant &tidx)
{
...
template <DataFormat formatB, bool transB, uint32_t swizzleDirect, uint64_t splitGapA, uint64_t splitGapC, bool ein>
__aicore__ inline void PpMatmulEinSum_no_quant<formatB, transB, swizzleDirect, splitGapA, splitGapC, ein>::PreloadB()
{
#ifdef __DAV_C220_CUBE__
...
template <DataFormat formatB, bool transB, uint32_t swizzleDirect, uint64_t splitGapA, uint64_t splitGapC, bool ein>
__aicore__ inline uint64_t PpMatmulEinSum_no_quant<formatB, transB, swizzleDirect, splitGapA, splitGapC, ein>::GetOffsetB(
...
template <DataFormat formatB, bool transB, uint32_t swizzleDirect, uint64_t splitGapA, uint64_t splitGapC, bool ein>
__aicore__ inline void PpMatmulEinSum_no_quant<formatB, transB, swizzleDirect, splitGapA, splitGapC, ein>::CopyTileA(
...
template <DataFormat formatB, bool transB, uint32_t swizzleDirect, uint64_t splitGapA, uint64_t splitGapC, bool ein>
__aicore__ inline void PpMatmulEinSum_no_quant<formatB, transB, swizzleDirect, splitGapA, splitGapC, ein>::CopyTileB(
...
public:
    __aicore__ PpMatmulW8a8_no_quant() {};
...
__aicore__ inline void PpMatmulW8a8_no_quant<transA, transB, withBias, swizzleDir, formatA, formatB>::Init(
...
{
    this->gm_a = gm_a;this->gm_b = gm_b;
    this->gm_c = gm_c;this->gm_bias = gm_bias;
...
PpMatmulW8a8_no_quant<transA, transB, withBias, swizzleDir, formatA, formatB>::GetOffsetA(const uint64_t batch_idx,
...
template <bool transA, bool transB, bool withBias, uint32_t swizzleDir, DataFormat formatA, DataFormat formatB>
__aicore__ inline uint64_t
PpMatmulW8a8_no_quant<transA, transB, withBias, swizzleDir, formatA, formatB>::GetOffsetB(const uint64_t batch_idx,
...
template <bool transA, bool transB, bool withBias, uint32_t swizzleDir, DataFormat formatA, DataFormat formatB>
__aicore__ inline void PpMatmulW8a8_no_quant<transA, transB, withBias, swizzleDir, formatA, formatB>::CopyTileA(
...
template <bool transA, bool transB, bool withBias, uint32_t swizzleDir, DataFormat formatA, DataFormat formatB>
__aicore__ inline void PpMatmulW8a8_no_quant<transA, transB, withBias, swizzleDir, formatA, formatB>::CopyTileB(
    const AscendC::LocalTensor<InDtype> &dstTensor, const AscendC::GlobalTensor<InDtype> &srcTensor,
...
template <bool transA, bool transB, bool withBias, uint32_t swizzleDir, DataFormat formatA, DataFormat formatB>
__aicore__ inline void PpMatmulW8a8_no_quant<transA, transB, withBias, swizzleDir, formatA, formatB>::InitBuffer()
{
    AsdopsBuffer<ArchType::ASCEND_V220> buf;
...
template <bool transA, bool transB, bool withBias, uint32_t swizzleDir, DataFormat formatA, DataFormat formatB>
__aicore__ inline void
PpMatmulW8a8_no_quant<transA, transB, withBias, swizzleDir, formatA, formatB>::GetBaseBlockIdx(uint64_t index, uint64_t &m_idx,
                                                                                      uint64_t &n_idx)
{
...
template <bool transA, bool transB, bool withBias, uint32_t swizzleDir, DataFormat formatA, DataFormat formatB>
__aicore__ inline void PpMatmulW8a8_no_quant<transA, transB, withBias, swizzleDir, formatA, formatB>::PreloadDoubleWeight()
{
#ifdef __DAV_C220_CUBE__
...
template <bool transA, bool transB, bool withBias, uint32_t swizzleDir, DataFormat formatA, DataFormat formatB>
__aicore__ inline void PpMatmulW8a8_no_quant<transA, transB, withBias, swizzleDir, formatA, formatB>::Process()
{
    using LocalTensor = AscendC::LocalTensor<InDtype>;
    if (core_idx >= core_num) {
        return;
...
class MLAOperation_no_quant {
    using qOutDtype = typename std::conditional_t<cacheMode == CACHE_MODE_INT8_NZCACHE, int8_t, bfloat16_t>;
    using kNopeDtype = typename std::conditional_t<cacheMode == CACHE_MODE_INT8_NZCACHE, int8_t, bfloat16_t>;

public:
    __aicore__ inline MLAOperation_no_quant(const MlaTilingData &mlaParams_)
    {
        blockIdx = AscendC::GetBlockIdx();
#ifdef __DAV_C220_VEC__
        sub_block_idx = static_cast<uint64_t>(GetSubBlockidx());
#endif
        vectorBlockIdx = (blockIdx / CONST_2) * CONST_2 + sub_block_idx;
        this->n = mlaParams_.n;
                                ...
                                GM_ADDR s2Gm, GM_ADDR s3Gm)
    {
        ...
        gamma2GmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(gamma2Gm));
        ...
        gamma3GmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(gamma3Gm));
        sin1GmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(sin1Gm));
        cos1GmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(cos1Gm));
        sin2GmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(sin2Gm));
        cos2GmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(cos2Gm));
        keycacheGmTensor1.SetGlobalBuffer(reinterpret_cast<__gm__ kNopeDtype *>(keycacheOutGm));
        keycacheGmTensor2.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(keycacheOutGm2));
        slotMappingGmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(slotMappingGm));
        wuqGmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(wuqGm));
        wukGmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(wukGm));
        descale2gmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ uint64_t *>(descale2Gm));
        s2GmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ bfloat16_t *>(s2Gm));
        ...
        if constexpr (cacheMode == CACHE_MODE_INT8_NZCACHE) {
            ...
            mm_ein_sum.Init(s2Gm, wukGm, qGm, mlaParams, CONST_2);
        }
#endif

#ifdef __DAV_C220_VEC__
        // RmsNormQuant_no_quant
...
private:
        ...
        AscendC::LocalTensor<half> &tmpfp16, AscendC::LocalTensor<int8_t> &int8OutTensor, float quantScale3)
    {
        ...
        for (uint64_t loop = 0; loop < sN; ++loop) {
            ...
            AscendC::PipeBarrier<PIPE_V>();
            ReduceSumCustomNoQuant(calTensor[SPLIT_RMSNRORM_SIZE_ONE], calTensor, calTensor[SPLIT_RMSNRORM_SIZE_ONE * 2],
...
private:
    ...
    AscendC::GlobalTensor<bfloat16_t> wdqkvGmTensor;
    AscendC::GlobalTensor<bfloat16_t> gamma2GmTensor;
    ...
    AscendC::GlobalTensor<bfloat16_t> gamma3GmTensor;
    AscendC::GlobalTensor<bfloat16_t> sin1GmTensor;
    AscendC::GlobalTensor<bfloat16_t> cos1GmTensor;
    AscendC::GlobalTensor<bfloat16_t> sin2GmTensor;
    AscendC::GlobalTensor<bfloat16_t> cos2GmTensor;
    AscendC::GlobalTensor<kNopeDtype> keycacheGmTensor1;
    AscendC::GlobalTensor<bfloat16_t> keycacheGmTensor2;
    AscendC::GlobalTensor<int32_t> slotMappingGmTensor;
    AscendC::GlobalTensor<bfloat16_t> wuqGmTensor;
    AscendC::GlobalTensor<bfloat16_t> wukGmTensor;

    AscendC::GlobalTensor<qOutDtype> qGmTensor;
    AscendC::GlobalTensor<bfloat16_t> qGmTensor2;
    AscendC::GlobalTensor<bfloat16_t> s1GmTensor;
    AscendC::GlobalTensor<bfloat16_t> s2GmTensor;
    AscendC::GlobalTensor<bfloat16_t> s3GmTensor;
    ...
    PpMatmulEinSum_no_quant<weightFormat2, true, 1, 0, 0, false> mm_2;
    static constexpr uint64_t splitGapC = cacheMode == CACHE_MODE_KVCACHE ? CONST_64 : CONST_0;
    PpMatmulEinSum_no_quant<weightFormat3, false, 0, CONST_64, splitGapC, true> mm_ein_sum;
#endif

#ifdef __DAV_C220_VEC__
    RmsNormQuant_no_quant<bfloat16_t, true, false> rmsNormQuant2;
    RopeBf16_no_quant<bfloat16_t, bfloat16_t, qOutDtype, cacheMode> ropeBf16;
#endif
};

template <int8_t cacheMode, DataFormat weightFormat1, DataFormat weightFormat2, DataFormat weightFormat3>
__aicore__ inline void MLAOperation_no_quant<cacheMode, weightFormat1, weightFormat2, weightFormat3>::ProcessCube()
{
#ifdef __DAV_C220_CUBE__
    mm_1.Process();
...
template <int8_t cacheMode, DataFormat weightFormat1, DataFormat weightFormat2, DataFormat weightFormat3>
__aicore__ inline void MLAOperation_no_quant<cacheMode, weightFormat1, weightFormat2, weightFormat3>::ProcessVector()
{
#ifdef __DAV_C220_VEC__
    ...
    if (row_work_ != 0) {
        ...
        AscendC::LocalTensor<bfloat16_t> input_tensor = buf.GetBuffer<BufferType::ASCEND_UB, bfloat16_t>(0);
        AscendC::LocalTensor<bfloat16_t> gamma_tensor = buf.GetBuffer<BufferType::ASCEND_UB, bfloat16_t>(MM1_OUT_SIZE * 2);
    ...
    if (row_work_ != 0) {
        AscendC::LocalTensor<bfloat16_t> input_tensor = buf.GetBuffer<BufferType::ASCEND_UB, bfloat16_t>(0);
        AscendC::LocalTensor<bfloat16_t> gamma_tensor = buf.GetBuffer<BufferType::ASCEND_UB, bfloat16_t>(MM1_OUT_SIZE * 2);
        AscendC::LocalTensor<bfloat16_t> sin_tensor =
            buf.GetBuffer<BufferType::ASCEND_UB, bfloat16_t>(MM1_OUT_SIZE * 2 + SPLIT_RMSNRORM_SIZE_ONE * 2);
        AscendC::LocalTensor<bfloat16_t> cos_tensor = buf.GetBuffer<BufferType::ASCEND_UB, bfloat16_t>(
        ...
        AscendC::LocalTensor<bfloat16_t> temp_tensor = buf.GetBuffer<BufferType::ASCEND_UB, bfloat16_t>(out_ub_offset);

        AscendC::LocalTensor<half> tmpfp16;
        AscendC::LocalTensor<int8_t> int8OutTensor;
        float scale3 = 0;

        RmsNormAndRopeConvergence1<bfloat16_t>(
            input_tensor,       // n * 576
            gamma_tensor,       // gamma
            sin_tensor,         // sin
            cos_tensor,         // cons
