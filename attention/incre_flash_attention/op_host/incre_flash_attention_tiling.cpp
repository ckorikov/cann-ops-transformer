...
ge::graphStatus IFATiling::ProcessPageAttentionFlag()
{
    ...
    if (inputQType_ == ge::DT_INT8 && inputKvType_ == ge::DT_INT8 && context_->keyRope.tensor != nullptr && context_->queryRope.tensor != nullptr && headDim_ == 512) { // 512 : for MLA
        OP_CHECK_IF((kDimNum == DIM_BNSD),
                    OP_LOGE(context_->opName, "when the dtype of query is int8 in MLA, KV layout must be NZ"),
                    return ge::GRAPH_FAILED);        
    } else {
...
ge::graphStatus PFAConvertContext(ContextParamsForPFATiling &contextKeyParams, gert::TilingContext *context)
{
    ...
    PFAConvertOptionalInputTensor(contextKeyParams, context);
    ...
    PFAConvertDataTypes(contextKeyParams, context);
    PFAConvertInputShapes(contextKeyParams, context);
    contextKeyParams.outputShape = context->GetOutputShape(0);
    auto attrs = context->GetAttrs();
    PFAConvertAttrs(contextKeyParams, attrs);
