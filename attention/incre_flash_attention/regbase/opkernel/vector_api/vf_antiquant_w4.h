...
template <typename Q_T, typename KV_T, typename ANTIQ_PARAMS_T, uint32_t baseSize, bool hasOffset = false>
__aicore__ inline void AntiquantVFImplW4PerTokenD64(LocalTensor<KV_T>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
                                            LocalTensor<ANTIQ_PARAMS_T>& antiqOffsetUb, LocalTensor<ANTIQ_PARAMS_T>& antiqScaleUb,
                                            uint32_t dealRowCount) {
  static_assert(baseSize == 64);
  __VEC_SCOPE__ {
...
template <typename Q_T, typename KV_T, typename ANTIQ_PARAMS_T, uint32_t baseSize, bool hasOffset = false>
__aicore__ inline void AntiquantVFImplW4Norm(LocalTensor<KV_T>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
                                             LocalTensor<ANTIQ_PARAMS_T>& antiqOffsetUb, LocalTensor<ANTIQ_PARAMS_T>& antiqScaleUb,
                                             uint32_t dealRowCount) {
  ...
  __VEC_SCOPE__ {
    ...
    for (uint16_t colLoopIdx = 0; colLoopIdx < colLoopCnt; colLoopIdx++) {
      ...
      if constexpr (hasOffset) {
        MicroAPI::DataCopy<Q_T, MicroAPI::LoadDist::DIST_NORM>(vOffset, ubOffsetAddr + colLoopIdx * 128);
      }
      MicroAPI::DataCopy<Q_T, MicroAPI::LoadDist::DIST_NORM>(vScale, ubScaleAddr + colLoopIdx * 128);
...
template <typename Q_T, typename KV_T, typename ANTIQ_PARAMS_T, uint32_t baseSize, bool hasOffset = false>
__aicore__ inline void AntiquantVFImplW4PerTokenD128(LocalTensor<KV_T>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
                                                     LocalTensor<ANTIQ_PARAMS_T>& antiqOffsetUb, LocalTensor<ANTIQ_PARAMS_T>& antiqScaleUb,
                                                     uint32_t dealRowCount) {
...
template <typename Q_T, typename KV_T, typename ANTIQ_PARAMS_T, uint32_t baseSize, bool hasOffset = false>
__aicore__ inline void AntiquantVFImplW4PerTokenD256(LocalTensor<KV_T>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
                                                     LocalTensor<ANTIQ_PARAMS_T>& antiqOffsetUb, LocalTensor<ANTIQ_PARAMS_T>& antiqScaleUb,
                                                     uint32_t dealRowCount) {
...
template <typename Q_T, typename KV_T, typename ANTIQ_PARAMS_T, uint32_t baseSize, bool hasOffset = false>
__aicore__ inline void AntiquantVFImplW4PerTokenD512(LocalTensor<KV_T>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
                                                     LocalTensor<ANTIQ_PARAMS_T>& antiqOffsetUb, LocalTensor<ANTIQ_PARAMS_T>& antiqScaleUb,
                                                     uint32_t dealRowCount) {
...
__aicore__ inline void AntiquantVFImplFP4_d64(LocalTensor<KV_T>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
...
__VEC_SCOPE__{
    ...
    MicroAPI::MaskXor(preg_next_64, preg_prev_64, preg_tmp, preg_0);
    ...
    for (uint16_t i = 0; i < loopCnt; i++) {
      ...
      // 加载 scale
      MicroAPI::DataCopy<bfloat16_t, MicroAPI::PostLiteral::POST_MODE_UPDATE, 
      ...
      if constexpr (std::is_same<Q_T, bfloat16_t>::value) {
        // 将输出结果copy到UB
        MicroAPI::DataCopy<Q_T, MicroAPI::DataCopyMode::DATA_BLOCK_COPY, 
        ...
        // 将输出结果copy到UB
        MicroAPI::DataCopy<half, MicroAPI::DataCopyMode::DATA_BLOCK_COPY, 
...
template <typename Q_T, typename KV_T, uint32_t baseSize, bool hasOffset = false>
__aicore__ inline void AntiquantVFImplFP4_norm(LocalTensor<KV_T>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
                                         LocalTensor<Q_T>& antiqOffsetUb, LocalTensor<Q_T>& antiqScaleUb,
                                         uint32_t dealRowCount) {
__VEC_SCOPE__{
    __ubuf__ uint8_t *ubSrcAddr = (__ubuf__ uint8_t *)(antiqInUb.GetPhyAddr());
...
template <typename Q_T, typename KV_T, uint32_t baseSize, bool hasOffset = false>
__aicore__ inline void AntiquantVFImplFP4_unalign(LocalTensor<KV_T>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
                                         LocalTensor<Q_T>& antiqOffsetUb, LocalTensor<Q_T>& antiqScaleUb,
...
__VEC_SCOPE__{
    ...
    MicroAPI::PrefixSum(prefixSum, preg_1);
    ...
    for (uint16_t colLoopIdx = 0; colLoopIdx < colLoopCnt; colLoopIdx++) {
      ...
      for (uint16_t i = 0; i < static_cast<uint16_t>(dealRowCount); i++) { 
          ...
          MicroAPI::Cast<bfloat16_t, KV_T, castTraitW4>(w_nd_bf16, (MicroAPI::RegTensor<KV_T>&)w_nd_f4, preg_0);
          // 加载 scale
          MicroAPI::DataCopyUnAlignPre(u0, (__ubuf__ bfloat16_t *&)ubScalerSrcAddrTemp);
          MicroAPI::DataCopyUnAlign<bfloat16_t, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
...
__aicore__ inline void AntiquantVFImpl(LocalTensor<fp4x2_e2m1_t>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
  ...
  if constexpr (baseSize == 64) {
    AntiquantVFImplFP4_d64<Q_T, fp4x2_e2m1_t, baseSize, hasOffset>(antiqInUb, antiqResUb, antiqOffsetUb, antiqScaleUb, dealRowCount);
  } else {
    if (baseSize != headDim) {
      AntiquantVFImplFP4_unalign<Q_T, fp4x2_e2m1_t, baseSize, hasOffset>(antiqInUb, antiqResUb, antiqOffsetUb, antiqScaleUb, dealRowCount, headDim);
    } else {
      AntiquantVFImplFP4_norm<Q_T, fp4x2_e2m1_t, baseSize, hasOffset>(antiqInUb, antiqResUb, antiqOffsetUb, antiqScaleUb, dealRowCount);
    }
  }
}
...
__aicore__ inline void AntiquantVFImpl(LocalTensor<fp4x2_e1m2_t>& antiqInUb, LocalTensor<Q_T>& antiqResUb,
                                       ...
                                       uint32_t dealRowCount, uint32_t headDim) {
  if constexpr (baseSize == 64) {
    AntiquantVFImplFP4_d64<Q_T, fp4x2_e1m2_t, baseSize, hasOffset>(antiqInUb, antiqResUb, antiqOffsetUb, antiqScaleUb, dealRowCount);
  } else {
    if (baseSize != headDim) {
      AntiquantVFImplFP4_unalign<Q_T, fp4x2_e1m2_t, baseSize, hasOffset>(antiqInUb, antiqResUb, antiqOffsetUb, antiqScaleUb, dealRowCount, headDim);
    } else {
      AntiquantVFImplFP4_norm<Q_T, fp4x2_e1m2_t, baseSize, hasOffset>(antiqInUb, antiqResUb, antiqOffsetUb, antiqScaleUb, dealRowCount);
    }
  }
}
