        ...
        uint32_t s1Extend, EvenvIdList &eventIdList)
{
    ...
    SetFlag<HardEvent::MTE2_V>(vWaitMte2Pingpong);
    WaitFlag<HardEvent::MTE2_V>(vWaitMte2Pingpong);
    if constexpr (!IsSameType<T1, float>::value) {
    ...
    SetFlag<HardEvent::V_MTE2>(mte2WaitVSfmg2A);
}
                                    ...
                                    EvenvIdList &eventIdList)
{
    ...
    if constexpr (TND_S1_PP == ENABLE) {
        int32_t mte2WaitVSfmg2A = static_cast<int32_t>(isPing ?
                                  eventIdList.structMte2WaitVPing : eventIdList.structMte2WaitVPong);
        WaitFlag<HardEvent::V_MTE2>(mte2WaitVSfmg2A);
    } else {
        if (curIdx > 1) {
            WaitFlag<HardEvent::MTE3_MTE2>(curEventIdMte2WaitMte3);
        }
