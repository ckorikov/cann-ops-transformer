    ...
    if constexpr (IS_DROP) {
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t i = 0; i < (uint16_t)(repeatTimes / 2); ++i) {
                ...
                preg0 = UpdateMask<float>(sreg);
                DataCopy(vreg3, (__ubuf__ float *&)srcLocalInt + (i * 2 + 1) * repeatElm);
            ...
            for (uint16_t i = 0; i < (uint16_t)(repeatTimes % 2); ++i) {
                ...
                MaskUnPack<AscendC::MicroAPI::HighLowPart::LOWEST>(preg8, preg7);
                DataCopy(vreg7, (__ubuf__ float *&)srcLocalInt + offset0);
