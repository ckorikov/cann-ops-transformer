...
/*!
 * \file fag_copy_cube_in_post.h
 * \brief
 */

#ifndef FAG_COPY_CUBE_IN_POST_H
#define FAG_COPY_CUBE_IN_POST_H
...
class FagCopyCubeInS1S2Post : public CopyCubeInBase<IMPL, MM_CFG, INPUT_TYPE> {
...
public:
    __aicore__ inline FagCopyCubeInS1S2Post()
    {
    }
                                                  ...
                                                  int batchNum = 1)
    {
        ...
        if constexpr (INPUT_TYPE::TAG == InputTypeTag::B) {
            uint8_t tscmIndex = MM2_ENCODING_TABLE[rightMatrixEncodingTableIdx];
