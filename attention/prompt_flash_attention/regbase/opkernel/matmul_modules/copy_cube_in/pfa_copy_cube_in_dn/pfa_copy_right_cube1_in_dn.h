...
/*!
 * \file pfa_copy_right_cube1_in_dn.h
 * \brief
 */

 #ifndef PFA_COPY_RIGHT_CUBE1_IN_DN_H
 #define PFA_COPY_RIGHT_CUBE1_IN_DN_H
 ...
 public:
     ...
     __aicore__ inline PFACopyRightCubeInDN() = default;
     __aicore__ inline ~PFACopyRightCubeInDN() = default;
     ...
     LoadData(int curRow, int curCol, int tileHeight, int tileWidth, int batchNum = -1) {
         ...
         if constexpr (!PhyPosIsL1(INPUT_TYPE::pos)) {
             ...
             if (!flag.reuseLeft) {
                 ...
                 nd2nzParams.ndNum = 1;
                 nd2nzParams.nValue = tileHeight - flag.mOrNAdditionalSize; // 搬运Q的实际有效行数
 ...
 #endif // PFA_COPY_RIGHT_CUBE1_IN_DN_H
