...
__aicore__ inline void PromptFlashAttentionS1s2Bns1X310Base<PFAT>::CopyND2NZOnTheFly(
    ...
    if (height % BLOCK_CUBE != 0) {
        ...
        for (int32_t i = 0; i < (totalRepeat + MAX_REPEATS_PER_BATCH - 1) / MAX_REPEATS_PER_BATCH; i++) {
            uint16_t curRepeat = (i == (totalRepeat + MAX_REPEATS_PER_BATCH - 1) / MAX_REPEATS_PER_BATCH - 1) ?
