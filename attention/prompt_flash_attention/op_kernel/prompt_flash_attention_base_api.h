...
const uint32_t PARITY_CHECK_BASE = 2;
...
constexpr int32_t SIXTY_FOUR = 64;

constexpr int32_t HEAD_STRIDE_INVALID = 0;
constexpr int32_t MASK_TYPE_NO_MASK = 2;
constexpr int32_t ELEMENT_SIZE_BYTES = 4; 
constexpr int32_t NO_SUB_BLOCK = 0;
constexpr int32_t DEFAULT_MASK_TYPE = 0;

constexpr int32_t MAX_WINDOW_BLOCKS = 3;
constexpr int32_t MASK_OFFSET_NONE = 0; 
constexpr int32_t MASK_OFFSET_1X = 1; 
constexpr int32_t MASK_OFFSET_2X = 2; 
constexpr int32_t MASK_OFFSET_3X = 3; 
...
public:
    ...
    __aicore__ __attribute__((always_inline)) inline void Run()
    {
        ...
        for (uint32_t process = GetBlockIdx(); process < processNum; process = nextProcess) {
            ...
            if (isTriuMask) {
                uint32_t currIter = process / GetBlockNum();
                nextProcess = currIter % PARITY_CHECK_BASE == 1 ? (currIter + 1) * GetBlockNum() + GetBlockIdx() : (currIter + PARITY_CHECK_BASE) * GetBlockNum() - 1 - GetBlockIdx();
            }
            ...
            for (uint32_t nIdx = nStart; nIdx < nEnd + launchDelay; nIdx += sBlockStack) {
                if (nIdx < nEnd) {
		            ...
		            uint32_t deltaIdx = mIdx - nIdx;
                    bool skipMask = windowStart > MAX_WINDOW_BLOCKS && deltaIdx > ONE && deltaIdx < windowStart - ONE;
                    if constexpr (swaCompress) {
                        maskOffset = 0;
                        if (windowStart <= MAX_WINDOW_BLOCKS) {    // window < 128*3 最多跨4个基块
                            if (mIdx < nIdx) {
                            ...
                            if (deltaIdx == MASK_OFFSET_NONE) {
                                maskOffset = MASK_OFFSET_NONE;    // m = n
                            } else if (deltaIdx == windowStart) {
                                maskOffset = MASK_OFFSET_3X * maxSeqlen * ppMScalar;
                            } else if (deltaIdx == MASK_OFFSET_1X) {
                                maskOffset = maxSeqlen * ppMScalar;
                            } else if (deltaIdx == windowStart - MASK_OFFSET_1X) {
                                maskOffset = MASK_OFFSET_2X * maxSeqlen * ppMScalar;
                            }   // delta idx in [2, window-1) do not move and add mask
                        }
                    }
                    qkRoundN = (qkN + BLOCK_SIZE - 1) / BLOCK_SIZE * BLOCK_SIZE;
                    if (qkN <= VECTOR_SIZE) {
                        if (subM > NO_SUB_BLOCK && maskType != DEFAULT_MASK_TYPE) {
                            if (alibiCoeffGm != nullptr) {
                                if (alibiLeftAlign == ZERO) {
                                    if (nIdx == nEnd - 1) {
                                        maskOffset = 0;
                                        deltaUint = 0;
                                        delta = 0;
                            ...
                            } else if (maskType == MASK_TYPE_NO_MASK && alibiCompressOffset > ZERO) {
                                if (nIdx == nEnd - 1) {
                            ...
                            if (longSeq == 0) {
                                ...
                                AscendC::PipeBarrier<PIPE_V>();
                                if (headStride == HEAD_STRIDE_INVALID && maskType != MASK_TYPE_NO_MASK) {
                                    if constexpr (!swaCompress) {
                                        AscendC::Muls<float, false>(
                                            maskUbufTensor,
                                            maskUbufTensor, (float)-3e38,
                        ...
                        if (subM > 0) {
                            ...
                            if(scaleType == ScaleType::SCALE_LOGN_FP32){
                                ...
                                AscendC::DataCopyPad(
                                    logUbufFloatTensor,
                                    logNFloatGmTensor[mIdx * ppMScalar + (uint64_t)subBlockIdx * qkM / TWO],
                                    AscendC::DataCopyExtParams(
                                             1,                   // nBurst
                                            subM * ELEMENT_SIZE_BYTES,                // lenBurst
                                            0, // srcGap byte
                                            (roundSubM - subM) * ELEMENT_SIZE_BYTES,    // dstGap block
                                            0),
                                ...
                                AscendC::DataCopyPad(
                                    ...
                                    AscendC::DataCopyExtParams(
                                            ...
                                            subM * TWO,                // lenBurst
                                            0, // srcGap byte
                                            (roundSubM - subM) * TWO,    // dstGap block
                                            0),
                                    AscendC::DataCopyPadExtParams<U_T>(
                                        false,
                                        0,
                            ...
                            if (maskType != 0) {
                                if (longSeq == 0) {
                                    ...
                                    AscendC::SetFlag<AscendC::HardEvent::V_MTE2>((EVENT_ID1));
                                } else if (ppNScalar == FLOAT_VECTOR_SIZE && sBlockStack == TWO && nIdx == nEnd - TWO) {
                                    __set_mask(qkN - FLOAT_VECTOR_SIZE);
                                    AscendC::Add<float, false>(
                            ...
                            AscendC::DataCopy(
                                ...
                                AscendC::DataCopyParams(
                                    1,                                            // nBurst
                                    subM * qkRoundN * TWO / BlockSize<int8_t>(), // lenBurst
                                    0,                                  // srcGap
                                    0                                   // dstGap
                                )
                            );
                                ...
                                if (mSplit > 0) {
                                    ...
                                    if(scaleType == ScaleType::SCALE_LOGN_FP32){
                                        ...
                                        AscendC::DataCopyPad(
                                            ...
                                            AscendC::DataCopyExtParams(
                                                    1,                          // nBurst
                                                    mSplit * ELEMENT_SIZE_BYTES,                // lenBurst
                                                    0,                          // srcGap byte
                                                    (roundMSplit - mSplit) * ELEMENT_SIZE_BYTES,   // dstGap block
                                                    0),
                                            AscendC::DataCopyPadExtParams<float>(
                                                false,
                                                0,
                                        ...
                                        AscendC::DataCopyPad(
                                            ...
                                            AscendC::DataCopyExtParams(
                                                    ...
                                                    mSplit * TWO,                // lenBurst
                                                    0,                          // srcGap byte
                                                    (roundMSplit - mSplit) * TWO,   // dstGap block
                                                    0),
                                            AscendC::DataCopyPadExtParams<U_T>(
                                                false,
                                                0,
                                    ...
                                    AscendC::DataCopy(
                                        pGmTensor[((uint64_t)GetBlockIdx() * TMP_SIZE +
                                                    ...
                                                    ((uint64_t)subBlockIdx * qkM / TWO + splitIdx * mSlice) *
                                                        qkRoundN) * TWO / sizeof(P_T)],
                                        lpUbufTensor.ReinterpretCast<P_T>(),
                                        AscendC::DataCopyParams(
                                            mSplit,                              // nBurst
                                            qkRoundN * TWO / BlockSize<int8_t>(), // lenBurst
                                            0,                                  // srcGap
                                            0                                   // dstGap
                                        )
                                    );
