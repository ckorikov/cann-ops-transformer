...
constexpr uint32_t L0B_LOAD_TOTAL_SIZE_128 = 128;

constexpr size_t LEN_BURST_TOTAL_BYTES_32 = 32; 
    ...
    __aicore__ __attribute__((always_inline)) inline void Run()
    {
        ...
        for (uint32_t process = GetBlockIdx(); process < processNum; process = nextProcess) {
            ...
            if (qkM == 1) {
                AscendC::DataCopy(l1qBufAddrTensor,
                                ...
                                AscendC::DataCopyParams(1,                                              // nBurst
                                                        CeilDiv<BLOCK_SIZE_COPY>(1 * RoundUp<uint32_t>(roundK, LEN_BURST_TOTAL_BYTES_32 / sizeof(QKV_DT))), // lenBurst
                                                        0,                                              // srcGap
                                                        0));
            } else {
            ...
            for (uint32_t nIdx = 0; nIdx < nEnd + launchDelay; nIdx += sBlockStack) {
                if (nIdx < nEnd) {
                    ...
                    for (uint32_t splitIdx = 0; splitIdx < sBlockStack && nIdx + splitIdx < nEnd; splitIdx++) {
                        ...
                        AscendC::LoadData(
                            ...
                            AscendC::LoadData2dParams(0,
                                                        NumMatrixsRoundUp<QKV_DT>(RoundUp<uint32_t>(roundK, LEN_BURST_TOTAL_BYTES_32 / sizeof(QKV_DT)) * qkRoundN),
                                                        1,
                                                        0,
                                                        0,
                                                        false,
                        ...
                        AscendC::SetFlag<AscendC::HardEvent::MTE1_MTE2>(pingpongFlag + TWO * kvPingpongFlag);
                ...
                if (nIdx >= launchDelay) {
                    uint32_t l0cPingpongFlag = (nIdx + 1) % TWO;
                    ...
                    AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(kvPingpongFlag * TWO);
                    AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(kvPingpongFlag * TWO + PINGPONG_FLAG_M_MTE1_OFFSET_ONE);
                    if (strideKv < STRIDE_LIMIT) {
                    ...
                    for (uint32_t l1KSplitIdx = 0; l1KSplitIdx < l1SplitLoop; l1KSplitIdx++) {
                        ...
                        AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1PingpongFlag + PINGPONG_FLAG_M_MTE1_OFFSET_FOUR);
                        if (qkM == 1) {
                        ...
                        for (uint32_t l0KSplitIdx = 0; l0KSplitIdx < dSplitLoop; l0KSplitIdx++) {
                            ...
                            if (l0LastSplit){
                                AscendC::SetFlag<AscendC::HardEvent::MTE1_MTE2>(l1PingpongFlag + PINGPONG_FLAG_M_MTE1_OFFSET_FOUR);
                            }
                            AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(l0PingpongFlag + PINGPONG_FLAG_M_MTE1_OFFSET_TWO);
                            for (uint32_t l0bLoadIdx = 0; l0bLoadIdx < L0B_LOAD_TOTAL_SIZE_128 / BLOCK_SIZE; ++l0bLoadIdx) {
                                AscendC::LoadData(
                            ...
                            if (l0LastSplit && l1LastSplit){
                                AscendC::SetFlag<AscendC::HardEvent::MTE1_MTE2>(kvPingpongFlag * TWO);
                                AscendC::SetFlag<AscendC::HardEvent::MTE1_MTE2>(kvPingpongFlag * TWO + PINGPONG_FLAG_M_MTE1_OFFSET_ONE);
                            }
                            AscendC::SetFlag<AscendC::HardEvent::MTE1_M>(l0PingpongFlag + PINGPONG_FLAG_M_MTE1_OFFSET_SIX);
                            AscendC::WaitFlag<AscendC::HardEvent::MTE1_M>(l0PingpongFlag + PINGPONG_FLAG_M_MTE1_OFFSET_SIX );
                            if constexpr (int8Flag) {
            ...
            for (uint32_t nIdx = 0; nIdx < nEnd + launchDelay; nIdx += sBlockStack) {
                ...
                if (nIdx >= launchDelay) {
                    ...
                    if (nIdx + sBlockStack > nEnd + launchDelay - 1)  {
                        ...
                        AscendC::DataCopyPad(
                            oGmTensor[oOffset + (uint64_t)subBlockIdx * qkM / TWO * strideQo],
                            goUbufTensor.ReinterpretCast<O_DTYPE>(),
