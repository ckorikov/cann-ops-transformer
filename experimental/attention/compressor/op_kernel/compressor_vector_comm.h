...
__aicore__ inline void AccumulateByRow(LocalTensor<float> &srcUb, uint32_t rowCount, uint32_t columnCount)
{
    for (uint32_t i = rowCount >> 1; i > 0; i >>= 1) {
...
__aicore__ inline void ColumnSum(LocalTensor<float> outputTensor, LocalTensor<float> inputTensor, uint32_t dealRowCount,
                                 uint32_t columnCount)
{
    ...
    for (uint32_t mask = MAX_R * 2; mask > 1; mask >>= 1) {
        if (dealRowCount & mask) {
            if (dealRowCount == mask) {
                mask >>= 1;
                datacopy(outputTensor, inputTensor, mask * columnCount);
                PipeBarrier<PIPE_V>();
                Add(outputTensor, outputTensor, inputTensor[mask * columnCount], mask * columnCount);
            } else {
                datacopy(outputTensor, inputTensor, mask * columnCount);
                PipeBarrier<PIPE_V>();
                Add(outputTensor, outputTensor, inputTensor[mask * columnCount], (dealRowCount - mask) * columnCount);
            }
            PipeBarrier<PIPE_V>();
            AccumulateByRow(outputTensor, mask, columnCount);
            break;
        }
    }
}

__aicore__ inline void MaxByRow(LocalTensor<float> &srcUb, uint32_t rowCount, uint32_t columnCount)
{
    for (uint32_t i = rowCount >> 1; i > 0; i >>= 1) {
        Max(srcUb, srcUb, srcUb[i * columnCount], i * columnCount);
        PipeBarrier<PIPE_V>();
    }
}

__aicore__ inline void ColumnMax(LocalTensor<float> outputTensor, LocalTensor<float> inputTensor, uint32_t dealRowCount,
                                 uint32_t columnCount)
{
    // 将srcUb的dealRowCount行累加到第一行,每行columnCount各元素
    for (uint32_t mask = MAX_R * 2; mask > 1; mask >>= 1) {
        if (dealRowCount & mask) {
            if (dealRowCount == mask) {
                ...
                datacopy(outputTensor, inputTensor, mask * columnCount);
                PipeBarrier<PIPE_V>();
                Max(outputTensor, outputTensor, inputTensor[mask * columnCount], mask * columnCount);
            } else {
                datacopy(outputTensor, inputTensor, mask * columnCount);
                PipeBarrier<PIPE_V>();
                Max(outputTensor, outputTensor, inputTensor[mask * columnCount], (dealRowCount - mask) * columnCount);
            }
            PipeBarrier<PIPE_V>();
            MaxByRow(outputTensor, mask, columnCount);
            break;
        }
    }
}

__aicore__ inline void MatSubsVec(LocalTensor<float> dstTensor, LocalTensor<float> src0Tensor, LocalTensor<float> src1Tensor,
                                 uint32_t dealRowCount, uint32_t columnCount, uint32_t dtypeMask, uint32_t dLoop,
                                 uint32_t dRemain, uint32_t repeatStride)
{
    uint32_t offset = 0;
    for (uint32_t row = 0; row < dealRowCount; row += 128) {
        uint32_t repeatTimes = min(dealRowCount - row + 128, 128);
        for (uint32_t i = 0; i < dLoop; i++) {
            Sub(dstTensor[row * columnCount + offset], src0Tensor[row * columnCount + offset], src1Tensor[offset],
                dtypeMask, repeatTimes, {1, 1, 1, repeatStride, repeatStride, 0});
            offset += dtypeMask;
        }
        if (dRemain > 0) {
            Sub(dstTensor[row * columnCount + offset], src0Tensor[row * columnCount + offset], src1Tensor[offset],
                dRemain, repeatTimes, {1, 1, 1, repeatStride, repeatStride, 0});
        }
    }
}

__aicore__ inline void MatDivsVec(LocalTensor<float> dstTensor, LocalTensor<float> src0Tensor, LocalTensor<float> src1Tensor,
                                 uint32_t dealRowCount, uint32_t columnCount, uint32_t dtypeMask, uint32_t dLoop,
                                 uint32_t dRemain, uint32_t repeatStride)
{
    uint32_t offset = 0;
    for (uint32_t row = 0; row < dealRowCount; row += 128) {
        uint32_t repeatTimes = min(dealRowCount - row + 128, 128);
        for (uint32_t i = 0; i < dLoop; i++) {
            Div(dstTensor[row * columnCount + offset], src0Tensor[row * columnCount + offset], src1Tensor[offset],
                dtypeMask, repeatTimes, {1, 1, 1, repeatStride, repeatStride, 0});
            offset += dtypeMask;
        }
        if (dRemain > 0) {
                               ...
                               const Rectangle& rectangleParams)
{
    ...
    if constexpr (std::is_same<float, half>::value) {
 ...
 */
template <typename float>
__aicore__ inline void Dequant(const LocalTensor<float> &outputLocal, const LocalTensor<float> &inputLocal, const LocalTensor<float> &scaleLocal,
