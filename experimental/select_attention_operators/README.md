...
As a temporary working environment, when on the .83 machine - run the following docker image from inside the project directory:
```bash
sudo docker run --rm -it --ipc=host --privileged  \
    --device=/dev/davinci9 --device=/dev/davinci11  \
    ...
    -v $(pwd):$(pwd) \
    -w $(pwd) \
    --name select_attn_kernel_dev \
    vllm_npu:cann_8.0.RC3.beta1 \
    sleep infinity # will keep the container running after the terminal closes
...
## Running (either in conda or in docker)
