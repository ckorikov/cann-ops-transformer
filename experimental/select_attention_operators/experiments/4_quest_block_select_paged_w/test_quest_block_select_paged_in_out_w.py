                ...
                for n in N_VALUES:
                    ...
                    if n <= h and h % n == 0:
                        for mmbpr in MMBPR_VALUES:
                            for k in K_VALUES:
                                parameter_sets.append((dtype, b, h, n, BLOCK_SIZE_DEFAULT, D_DEFAULT, mmbpr, k))
    return parameter_sets

########################### Test 1 - Basic functionality ###########################
parameter_sets = construct_quest_paged_parameter_sets(DTYPE_VALUES = [torch.float16, torch.bfloat16], 
                                                      B_VALUES = [1, 2], 
                                                      H_VALUES = [8, 16], 
                                                      N_VALUES = [4, 8], 
...
if __name__ == "__main__":
    test_quest_paged_kernel(DTYPE=torch.bfloat16, B=20, H=32, N=8, BLOCK_SIZE=128, D=128, MMBPR=1, k=8, verbose=True)  # fails bfloat16
