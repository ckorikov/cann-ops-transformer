...
__aicore__ inline void CopyTiling(TCubeTiling *tiling, RopeMatrixTiling *ropeTiling, GM_ADDR tilingGM)
{
    ...
    for (uint32_t j = 0; j < sizeof(RopeMatrixTiling) / sizeof(uint32_t); i++, ropePtr++, j++) {
        *ropePtr = *(tiling32 + i);
    }
    return;
}
...
extern "C" __global__ __aicore__ void rope_matrix_kernel_bf16(GM_ADDR x, GM_ADDR y, GM_ADDR sin, GM_ADDR cos, GM_ADDR out, GM_ADDR workspace, GM_ADDR tiling) {
    ...
    if ASCEND_IS_AIC {
        RopeMatrix::MatmulBatchKernel<bfloat16_t, bfloat16_t, bfloat16_t> mmkernel;
    ...
    if ASCEND_IS_AIV {
        ...
        RopeMatrix::RopeVec<bfloat16_t> vec;
