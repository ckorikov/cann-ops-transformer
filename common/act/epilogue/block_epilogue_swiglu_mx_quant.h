...
private:
    ...
    __aicore__ inline void CopyDequantResFromUb2Gm(uint64_t blockCount, uint64_t offset,
                                                   AscendC::LocalTensor<DataTypeIn> &src);
    ...
    __aicore__ inline void ComputeData(__ubuf__ bfloat16_t *srcAddr, __ubuf__ uint16_t *halfScaleLocalAddr,
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < loopNumScale; i++) {
            ...
            AscendC::MicroAPI::DataCopy<uint16_t, AscendC::MicroAPI::PostLiteral::POST_MODE_UPDATE,
                ...
                mxScaleLocalAddr, scaleValue, vlForHalfNumber_ / 2, preMaskScale);
...
QMM_BLOCK_EPILOGUE_SWIGLU_QUANT_CLASS_LOCAL_PARAMS
__aicore__ inline void BlockEpilogueSwigluQuant<QMM_BLOCK_EPILOGUE_DEQUANT_FUNC_LOCAL_PARAMS>::ComputeData(
...
{
    constexpr uint16_t sizePerRepeat = AscendC::VECTOR_REG_WIDTH / sizeof(DataTypeIn); // 64
    ...
    ComputeData(gluResAddr, halfScaleLocalAddr, outputDst, totalDataInUb, loopDataNum);
    return;
}
