...
public:
    ...
    ge::graphStatus DoTilingImpl(gert::TilingContext *context)
    {
        ...
        if (platformInfoPtr == nullptr) {
            auto compileInfoPtr = reinterpret_cast<const FACompileInfoCommon *>(context->GetCompileInfo());
...
public:
    ...
    template <typename T> RegisterNew &tiling(int32_t priority, int32_t soc_version)
    {
    ...
    template <typename T> RegisterNew &tiling(int32_t priority, const std::vector<int32_t>& soc_versions)
    {
...
#define REGISTER_TILING_TEMPLATE_WITH_SOCVERSION(op_type, class_name, soc_versions, priority) \
	static RegisterNew VAR_UNUSED##op_type##class_name##priority_register = \
		RegisterNew(#op_type).tiling<class_name>(priority, soc_versions)
