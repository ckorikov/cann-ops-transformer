...
constexpr static int32_t shapeDim = 2;
    ...
    int64_t srcOffset, uint32_t s1Size, uint32_t s2Size, int64_t totalS2Size, int64_t alignedSize = blockBytes)
{
    ...
    dstTensor.SetShapeInfo(ShapeInfo(shapeDim, shapeArray, DataFormat::ND));
    ...
    int64_t alignedSize = blockBytes)
{
    ...
    dataCopyParams.dstStride = 0;
    if (alignedSize != 0 && s2TotalSize / byteBitRatio % alignedSize == 0 && s2BaseSize / byteBitRatio % alignedSize == 0) {
