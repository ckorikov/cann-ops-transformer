...
#include "nn_other.h"
#include "../op_graph/interleave_rope_proto.h"
...
const int S = 16; // Sequence length
const int H = 32; // Hidden size
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                          \
    ...
    interleave_rope_op.set_input_##intputName(placeholder##intputIndex);                       \
    inputs.push_back(placeholder##intputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue)                                                  \
    interleave_rope_op.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                    \
    ...
    graph.AddOp(placeholder##intputIndex);                                                   \
    interleave_rope_op.set_input_##intputName(placeholder##intputIndex);                       \
    interleave_rope_op.update_input_desc_##intputName(placeholder##intputIndex##_desc);        \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                        \
    TensorDesc outputName##outputIndex##_desc =                                              \
        TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype);                          \
    interleave_rope_op.update_output_desc_##outputName(outputName##outputIndex##_desc);   
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto interleave_rope_op = op::InterleaveRope("test_geir_interleave_rope");
    ...
    // 添加输入（顺序严格匹配 proto.h）
    ADD_INPUT(1, x, inDtype, inputXShape);
    ADD_INPUT(2, cos, DT_FLOAT16, inputCosShape);
    ADD_INPUT(3, sin, DT_FLOAT16, inputSinShape);
    
    // 添加输出（顺序严格匹配 proto.h）
    ADD_OUTPUT(1, y, DT_FLOAT16, outYShape);

    outputs.push_back(interleave_rope_op);
    // 添加完毕
    return SUCCESS;
}
