...
template <typename T>
__aicore__ inline void InterleavedSplitBSPad<T>::InitData(const RotaryPositionEmbeddingTilingData &tiling)
{
                                                         ...
                                                         uint32_t calcLen)
{
    ...
    uint64_t startOffset = batchIdx * ubCalcBNum * seqLen * bufferNdSize + seqIdx * bufferNdSize;
    if (numHeads == 1) {
        dataCopyParams.blockCount = calcLen;
        dataCopyParams.blockLen = headDim * sizeof(T);
        dataCopyParams.srcStride = (seqLen - 1) * headDim * sizeof(T);
        dataCopyParams.dstStride = 0;
        DataCopyPad(x, xGm[startOffset], dataCopyParams, {false, 0, 0, 0});
    } else {
        for (uint32_t loopIdx = 0; loopIdx < calcLen; ++loopIdx) {
            ...
            dataCopyParams.dstStride = 0;
            DataCopyPad(x[loopIdx * numHeads * headDimAlign], xGm[startOffset + loopIdx * seqLen * bufferNdSize],
...
__aicore__ inline void InterleavedSplitBSPad<T>::CopyOut(uint32_t seqIdx, uint32_t batchIdx, uint32_t calcLen)
{
    ...
    if (numHeads == 1) {
        ...
        dataCopyParams.blockLen = headDim * sizeof(T);
        dataCopyParams.srcStride = 0;
        dataCopyParams.dstStride = (seqLen - 1) * headDim * sizeof(T);
        DataCopyPad(yGm[startOffset], y, dataCopyParams);
    } else {
        for (uint32_t loopIdx = 0; loopIdx < calcLen; ++loopIdx) {
            dataCopyParams.blockCount = numHeads;
            dataCopyParams.blockLen = headDim * sizeof(T);
            dataCopyParams.srcStride = 0;
            dataCopyParams.dstStride = 0;
            DataCopyPad(yGm[startOffset + loopIdx * seqLen * bufferNdSize], y[loopIdx * numHeads * headDimAlign],
                        dataCopyParams);
        }
    }
