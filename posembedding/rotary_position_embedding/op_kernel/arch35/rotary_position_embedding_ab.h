...
{
    uint32_t bsLoopCnt = (bsBlockCount_, tilingData_->ubFactorBS);
    uint32_t nLoopCnt = (nBlockCount_, tilingData_->ubFactorN);
    ...
    int64_t nCount)
{
    ...
    if (tilingData_->rotaryMode == static_cast<int64_t>(RotaryPosEmbeddingMode::DEEPSEEK_INTERLEAVE)) {
        inParams = {static_cast<uint16_t>(totalCount), tilingData_->D * sizeof(T), 0, 0, 0};
    }
    ...
    if (tilingData_->rotaryMode == static_cast<int64_t>(RotaryPosEmbeddingMode::HALF)) {
        HalfAlignVF(sinBuffer, cosBuffer, inBuffer, outBuffer, tilingData_->D, tilingData_->dAlign, bsCount, nCount);
    } else if (tilingData_->rotaryMode == static_cast<int64_t>(RotaryPosEmbeddingMode::INTERLEAVE)) {
        InterleaveModeVF(sinBuffer, cosBuffer, inBuffer, outBuffer, tilingData_->D, bsCount, nCount);
    } else if (tilingData_->rotaryMode == static_cast<int64_t>(RotaryPosEmbeddingMode::QUARTER)) {
        QuarterAlignVF(sinBuffer, cosBuffer, inBuffer, outBuffer, tilingData_->D, tilingData_->dAlign, bsCount, nCount);
    } else {
        DeepSeekInterleaveModeVF<T>(sinBuffer, cosBuffer, inBuffer, outBuffer, tilingData_->D, bsCount, nCount);
    }
...
#endif // ROTARY_POSITION_EMBEDDING_AB_H