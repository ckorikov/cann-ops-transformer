...
__aicore__ inline void RotaryPositionEmbeddingABAAndBA<T, IsBBoardcast>::InitAllBuffer()
{
    ...
    if (tilingData_->rotaryMode == static_cast<int64_t>(RotaryPosEmbeddingMode::DEEPSEEK_INTERLEAVE)) {
        ...
        this->ubCopyInStride =
            (this->dAlign_ * sizeof(T) - ops::CeilAlign<int64_t>(D_ * sizeof(T), BLOCK_TYPE_SIZE)) / BLOCK_TYPE_SIZE;
    }
...
{
    // 在S轴进行循环
    int64_t ubLoopCount = (sBlockLength_, ubFactorS_);
...
{
    // 在B轴进行循环
    int64_t ubLoopCount = (bBlockLength_, ubFactorB_);
    if constexpr (IsBBoardcast) {
        // cos和sin需要在B轴广播的情况
...
{
    // 循环处理Q
    int64_t qUbLoopCount = (tilingData_->N, ubFactorN_);
