...
int aclnnGroupedMatmulSwigluQuantV2Test(int32_t deviceId, void*& workspaceAddr) {
    aclrtStream stream;
    ...
    std::vector<int64_t> weightScaleShape = {{E, K / 64, N, 2}};
    std::vector<int64_t> xScaleShape = {M, K / 64, 2};
    std::vector<int64_t> groupListShape = {E};
    std::vector<int64_t> outputShape = {M, N / 2};
    std::vector<int64_t> outputScaleShape = {M, N / 64 / 2, 2};
    ...
    std::vector<float> weightScaleHostData(E * K / 32 * N, 1);
    std::vector<float> xScaleHostData(M * K / 32, 1);
    std::vector<int64_t> groupListHostData(E, 1);
    std::vector<int8_t> outputHostData(M * N / 2, 1);
    std::vector<float> outputScaleHostData(M * N / 64, 1);
    ...
    std::vector<std::vector<float>> weightScaleHostDataList = {weightScaleHostData};
    std::vector<std::vector<int64_t>> weightScaleShapeList = {weightScaleShape};
    ret = CreateAclTensorList<float>(weightScaleHostDataList, weightScaleShapeList, &weightScaleDeviceAddr, aclDataType::ACL_FLOAT8_E8M0, &weightScale);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    // 创建xScale aclTensor
    ret = CreateAclTensor<float>(xScaleHostData, xScaleShape, &xScaleDeviceAddr, aclDataType::ACL_FLOAT8_E8M0, aclFormat::ACL_FORMAT_ND, &xScale);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    // 创建y aclTensor
    std::vector<float> outputHostDataFloat = {outputHostData.begin(), outputHostData.end()};
    ret = CreateAclTensor<float>(outputHostDataFloat, outputShape, &outputDeviceAddr, aclDataType::ACL_FLOAT8_E5M2, aclFormat::ACL_FORMAT_ND, &output);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    // 创建yScale aclTensor
    ret = CreateAclTensor<float>(outputScaleHostData, outputScaleShape, &outputScaleDeviceAddr, aclDataType::ACL_FLOAT8_E8M0, aclFormat::ACL_FORMAT_ND, &outputScale);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    std::vector<float> outputScaleData(size, 0);
...
int main()
{
    ...
    return aclnnGroupedMatmulSwigluQuantV2Test(deviceId, workspaceAddr);
