...
- 计算公式：
  ...
  ----
  - MSD伪量化场景A8W4（A指激活矩阵，W指权重矩阵，8指INT8数据类型，4指INT4数据类型）：
    - **定义**：
      * **⋅** 表示矩阵乘法。
      * **⊙** 表示逐元素乘法。
      * $\left \lfloor x\right \rceil$ 表示将x四舍五入到最近的整数。
...
- `aclnnStatus aclnnGroupedMatmulSwigluQuantV2GetWorkspaceSize(const aclTensor *x, const aclTensorList *weight, const aclTensorList *weightScale, const aclTensorList *weightAsistMatrix, const aclTensor *bias, const aclTensor *xScale, const aclTensor *smoothScale, const aclTensor *groupList, int64_t dequantMode, int64_t dequantDtype, int64_t quantMode, int64_t quantDtype, int64_t groupListType, const aclIntArray *tuningConfig, aclTensor *output, aclTensor *outputScale, uint64_t *workspaceSize, aclOpExecutor **executor)`
- `aclnnStatus aclnnGroupedMatmulSwigluQuantV2(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor, aclrtStream stream)`
...
- **参数说明：**
  ...
  - weight（aclTensorList*，计算输入）：权重矩阵，公式中的$W$，Device侧的aclTensor。shape支持3维，数据类型支持INT8、INT4、INT32（INT32为适配用途，实际1个INT32会被解释为8个INT4数据），[数据格式](./common/数据格式.md)支持FRACTAL\_NZ，支持[非连续的Tensor](common/非连续的Tensor.md)。

  - weightScale（aclTensorList*，计算输入）：右矩阵的量化因子，公式中的$w\_scale$，Device侧的aclTensor。首轴长度需与`weight`的首轴维度相等，尾轴长度需要与weight还原为ND格式的尾轴相同，[数据格式](./common/数据格式.md)支持ND，支持[非连续的Tensor](common/非连续的Tensor.md)。
    - A8W4场景：shape支持2或3维，数据类型支持UINT64。
    - A8W8场景：shape支持2维，数据类型支持FLOAT、FLOAT16、BFLOAT16。

  - weightAsistMatrix（aclTensorList*，计算输入）：计算矩阵乘时的辅助矩阵，公式中的$bias$，shape支持2维，数据类型支持fp32，仅A8W4场景生效，A8W8场景需传空指针。首轴长度需与`weight`的首轴维度相等，尾轴长度需要与weight还原为ND格式的尾轴相同。

  - bias（aclTensor*，计算输入）：矩阵乘计算的偏移值。预留输入，暂不支持，需要传空指针。
  ...
  - smoothScale（aclTensor*，计算输入）：预留输入，暂不支持，需要传空指针。
  ...
  - dequantMode（int64_t，计算输入）：指定反量化计算类型，用于确定激活矩阵与权重矩阵的反量化方式。0：激活矩阵per-token，权重矩阵per-channel；1：激活矩阵per-token，权重矩阵per-group。

  - dequantDtype（int64_t，计算输入）：中间GroupedMatmul的结果数据类型。仅A5有效，A2/A3使用固定数据类型，无法选择。0：DT_FLOAT；1：DT_FLOAT16；27：DT_BF16；28：DT_UNDEFINED。

  - quantMode（int64_t，计算输入）：指定量化计算类型，用于确定swiglu结果的量化模式。0：per-token；1：per-channel。

  - quantDtype（int64_t，计算输入）：输出的数据类型。仅A5有效，A2/A3使用固定数据类型，无法选择。2：DT_INT8 ；35：DT_FLOAT8_E5M2；36：DT_FLOAT8_E4M3FN；40：DT_FLOAT4_E2M1；41：DT_FLOAT4_E1M2；28：DT_UNDEFINED。

  - groupListType（int64_t，计算输入）：指定分组的解释方式，用于确定groupList的语义。0：cumsum；1：count。

  - tuningConfig（aclIntArray*，计算输入）：用于算子预估m/e的大小，走不同的算子模板，以适配不不同场景性能要求。默认值为[]。
