...
static const std::map<DataType, std::unordered_set<DataType>> BIAS_TYPE_SUPPORT_MAP = {
...
graphStatus GroupedMatmulWeightQuantChecker::GetXAndWeightDimValue(const gert::InferShapeContext *context,
...
graphStatus GroupedMatmulWeightQuantChecker::CheckShapeForXAndWeight(const gert::InferShapeContext *context,
                                                                     const GMMAttrs &gmmAttrs) const
{
    ...
    OP_CHECK_IF(!(weightNDim_ > 0 && weightNDim_ <= GMM_MAX_INNER_AXIS), OP_LOGE(context->GetNodeName(),
                    ...
                    GMM_MAX_INNER_AXIS, weightNDim_),return GRAPH_FAILED);
    OP_CHECK_IF(!(weightKDim_ > 0 && weightKDim_ <= GMM_MAX_INNER_AXIS), OP_LOGE(context->GetNodeName(),
                        ...
                        GMM_MAX_INNER_AXIS, weightKDim_), return GRAPH_FAILED);
    if (weightDtype == ge::DT_FLOAT4_E2M1 || weightDtype == ge::DT_FLOAT) {
        OP_CHECK_IF(
            ...
            OP_LOGE(
                ...
                GMM_N_K_ALIGN_VALUE_WEIGHT_QUANT_4BIT, weightNDim_, weightKDim_), return GRAPH_FAILED);
    } else {
        OP_CHECK_IF(
            ...
            OP_LOGE(
                context->GetNodeName(),
                "The value of dim n, k should be an integer multiple of [%ld], but actual n is [%ld], k is [%ld].",
                GMM_N_K_ALIGN_VALUE_WEIGHT_QUANT, weightNDim_, weightKDim_), return GRAPH_FAILED);
    }
    auto weightShape = context->GetDynamicInputShape(GMM_INDEX_IN_WEIGHT, 0);
...
graphStatus GroupedMatmulWeightQuantChecker::CheckScenarioValidForShape(const gert::InferShapeContext *context,
                                                                        const GMMAttrs &gmmAttrs) const
{
...
graphStatus GroupedMatmulWeightQuantChecker::CheckShapeForGrouplist(const gert::InferShapeContext *context,
...
graphStatus GroupedMatmulWeightQuantChecker::CheckShapeForTensorList(const gert::InferShapeContext *context,
                                                                     size_t gmm_index,
...
graphStatus GroupedMatmulWeightQuantChecker::CheckShapeForWeightQuantParam(const gert::InferShapeContext *context) const
{
    ...
    OP_CHECK_IF(CheckShapeForTensorList(context, GMM_INDEX_IN_ANTIQUANT_SCALE, "antiquantScale") != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeForAntiquantScale failed."), return GRAPH_FAILED);

    OP_CHECK_IF(CheckShapeForTensorList(context, GMM_INDEX_IN_ANTIQUANT_OFFSET, "antiquantOffset") != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeForAntiquantOffset failed."), return GRAPH_FAILED);

    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulWeightQuantChecker::CheckGroupSize(const gert::InferShapeContext *context,
                                                            const GMMAttrs &gmmAttrs) const {
    auto antiquantScaleShape = context->GetDynamicInputShape(GMM_INDEX_IN_ANTIQUANT_SCALE, 0);
...
graphStatus GroupedMatmulWeightQuantChecker::CheckShapeValid(const gert::InferShapeContext *context,
                                                             const GMMAttrs &gmmAttrs)
{
    ...
    OP_CHECK_IF(CheckShapeForGrouplist(context, groupListShape) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeForGrouplist failed."), return GRAPH_FAILED);
    groupNum_ = groupListShape->GetDim(0);
    OP_CHECK_IF(CheckGroupSize(context, gmmAttrs) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckGroupSize failed."), return GRAPH_FAILED);
    OP_CHECK_IF(CheckShapeForXAndWeight(context, gmmAttrs) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeForXAndWeight failed."), return GRAPH_FAILED);

    OP_CHECK_IF(CheckShapeForTensorList(context, GMM_INDEX_IN_BIAS, "bias") != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeForBias failed."), return GRAPH_FAILED);

    OP_CHECK_IF(CheckShapeForWeightQuantParam(context) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeForWeightQuantParam failed."), return GRAPH_FAILED);

    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulWeightQuantChecker::CheckFormatValid(const gert::InferShapeContext *context) const
{
...
graphStatus GroupedMatmulWeightQuantChecker::CheckTransposeValid(const gert::InferShapeContext *context,
                                                                 const GMMAttrs &gmmAttrs) const {
    auto weightDesc = context->GetDynamicInputDesc(GMM_INDEX_IN_WEIGHT, 0);
    auto weightDtype = weightDesc->GetDataType();
...
graphStatus GroupedMatmulWeightQuantChecker::CheckShape(const gert::InferShapeContext *context,
    ...
    OP_CHECK_IF(CheckFormatValid(context) != GRAPH_SUCCESS, OP_LOGE(context->GetNodeName(), "CheckFormatValid failed."),
              return GRAPH_FAILED);
    OP_CHECK_IF(CheckTransposeValid(context, commonUtil.attrsInfo) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckTransposeValid failed."), return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulWeightQuantChecker::UpdateShapeY(gert::InferShapeContext *context, size_t idxY,
...
graphStatus GroupedMatmulWeightQuantChecker::InferOutShape(gert::InferShapeContext *context) const
{
...
graphStatus GroupedMatmulWeightQuantChecker::CheckDtype(const gert::InferDataTypeContext *context) const
{
...
graphStatus GroupedMatmulWeightQuantChecker::InferOutDtype(gert::InferDataTypeContext *context) const
{
