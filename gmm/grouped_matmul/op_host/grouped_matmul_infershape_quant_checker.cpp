...
static const std::unordered_set<DataType> X_TYPE_SUPPORT_SET = {ge::DT_INT8, ge::DT_FLOAT8_E4M3FN, ge::DT_FLOAT8_E5M2, ge::DT_HIFLOAT8};
static const std::unordered_set<DataType> WEIGHT_TYPE_SUPPORT_SET = {ge::DT_INT8, ge::DT_FLOAT8_E4M3FN,
                                                                     ge::DT_FLOAT8_E5M2, ge::DT_HIFLOAT8};
static const std::unordered_set<DataType> BIAS_TYPE_SUPPORT_SET = {ge::DT_FLOAT16, ge::DT_BF16, ge::DT_FLOAT,
                                                                   ge::DT_INT32};
static const std::unordered_set<DataType> SCALE_TYPE_SUPPORT_SET = {ge::DT_UINT64, ge::DT_INT64, ge::DT_FLOAT,
                                                                    ge::DT_BF16, ge::DT_FLOAT8_E8M0};
static const std::unordered_set<DataType> PERTOEKN_SCALE_TYPE_SUPPORT_SET = {ge::DT_FLOAT, ge::DT_FLOAT8_E8M0};
...
graphStatus GroupedMatmulQuantChecker::GetXAndWeightDimValue(const gert::InferShapeContext *context,
...
[%zu].", xdimNum_), return GRAPH_FAILED);
...
but it is [%zu].", weightdimNum_), return GRAPH_FAILED);
    ...
    weightKDim_ =
        gmmAttrs.transposeWeight ?
            weightShape->GetDim(weightdimNum_ - 1) : weightShape->GetDim(weightdimNum_ - PENULTIMATE_DIM);
    weightNDim_ =
        gmmAttrs.transposeWeight ?
            weightShape->GetDim(weightdimNum_ - PENULTIMATE_DIM) : weightShape->GetDim(weightdimNum_ - 1);
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::CheckDtypeValid(const gert::InferDataTypeContext *context) const
{
    ...
    OP_CHECK_IF(X_TYPE_SUPPORT_SET.find(xDtype) == X_TYPE_SUPPORT_SET.end(),
              ...
              TypeUtils::DataTypeToAscendString(xDtype).GetString()), return GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        ...
        TypeUtils::DataTypeToAscendString(weightDtype).GetString()), return GRAPH_FAILED);
    if (xDtype == DataType::DT_INT8 || weightDtype == DataType::DT_INT8) {
        OP_CHECK_IF(!(xDtype == DataType::DT_INT8 && weightDtype == DataType::DT_INT8),
...
x is [%s] and the data type of weight is [%s].",
    ...
    OP_CHECK_IF(LogicXOR((xDtype == DataType::DT_HIFLOAT8), (weightDtype == DataType::DT_HIFLOAT8)),
        OP_LOGE(context->GetNodeName(),
            "When one input dtype is HIFLOAT8, then the other input dtype must be HIFLOAT8, vice versa, actual \
x is %s, weight is %s.", TypeUtils::DataTypeToAscendString(xDtype).GetString(),
            TypeUtils::DataTypeToAscendString(weightDtype).GetString()), return GRAPH_FAILED);
    OP_CHECK_IF(LogicXOR((xDtype == DataType::DT_FLOAT8_E4M3FN || xDtype == DataType::DT_FLOAT8_E5M2),
                       (weightDtype == DataType::DT_FLOAT8_E4M3FN || weightDtype == DataType::DT_FLOAT8_E5M2)),
        OP_LOGE(context->GetNodeName(),
            "When one input dtype is FLOAT8, then the other input dtype must be FLOAT8, vice versa, actual x is %s, \
weight is %s.", TypeUtils::DataTypeToAscendString(xDtype).GetString(),
            TypeUtils::DataTypeToAscendString(weightDtype).GetString()), return GRAPH_FAILED);
    auto ScaleDtype = context->GetDynamicInputDataType(GMM_INDEX_IN_SCALE, 0);
    OP_CHECK_IF(
        SCALE_TYPE_SUPPORT_SET.find(ScaleDtype) == SCALE_TYPE_SUPPORT_SET.end(),
        OP_LOGE(context->GetNodeName(), "Data type [%s] is not supported for scale.",
        ge::TypeUtils::DataTypeToAscendString(ScaleDtype).GetString()), return GRAPH_FAILED);
    ...
    OP_CHECK_IF(BIAS_TYPE_SUPPORT_SET.find(biasDtype) == BIAS_TYPE_SUPPORT_SET.end(),
              ...
              ge::TypeUtils::DataTypeToAscendString(biasDtype).GetString()), return GRAPH_FAILED);

    auto pertokenScaleDtype = context->GetDynamicInputDataType(GMM_INDEX_IN_PERTOKEN_SCALE, 0);
    if (pertokenScaleDtype != DT_UNDEFINED) {
        OP_CHECK_IF(PERTOEKN_SCALE_TYPE_SUPPORT_SET.find(pertokenScaleDtype) == PERTOEKN_SCALE_TYPE_SUPPORT_SET.end(),
                  ...
                  ge::TypeUtils::DataTypeToAscendString(pertokenScaleDtype).GetString()), return GRAPH_FAILED);
    }
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::CheckNotZeroValueForNoneSplitAxis(const gert::InferShapeContext *context,
...
nDimValue is [%ld].", xKDim_, eDimValue, weightNDim_), return GRAPH_FAILED);
    } else {
        OP_CHECK_IF(xMDim_ == 0 || weightNDim_ == 0,
                  OP_LOGE(context->GetNodeName(),
                            "Non split axis' dim value cannot be zero but mDimValue is [%ld], nDimValue is [%ld].",
                            xMDim_, weightNDim_), return GRAPH_FAILED);
    }
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::CheckScenarioValidForShape(const gert::InferShapeContext *context,
                                                                  const GMMAttrs &gmmAttrs) const
{
...
tensor of tensor list x is not empty."), return GRAPH_FAILED);
    OP_CHECK_IF(IsNonEmpty(weightSecondTensorShape),
              OP_LOGE(context->GetNodeName(), "Only support single/single/single scenario for now, but the second \
tensor of tensor list weight is not empty."), return GRAPH_FAILED);
    ...
    OP_CHECK_IF(gmmAttrs.splitItem != GMM_X_SEPARATED && gmmAttrs.splitItem != GMM_NO_SEPARATED,
              OP_LOGE(context->GetNodeName(), "Invalid splitItem, which can only be one of 2/3, but it is [%ld].",
                        gmmAttrs.splitItem), return GRAPH_FAILED);
    OP_CHECK_IF(gmmAttrs.groupType != GMM_SPLIT_M && gmmAttrs.groupType != GMM_SPLIT_K,
              OP_LOGE(context->GetNodeName(), "Invalid groupType, which can only be one of 0/2, but it is [%ld].",
                        gmmAttrs.groupType), return GRAPH_FAILED);
    ...
    OP_CHECK_IF(CheckNotZeroValueForNoneSplitAxis(context, gmmAttrs) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckNotZeroValueForNoneSplitAxis Failed."), return GRAPH_FAILED);

    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::CheckFormatValid(const gert::InferShapeContext *context) const
{
...
graphStatus GroupedMatmulQuantChecker::CheckShapeForBias(const gert::InferShapeContext *context) const
{
...
{   // pertoken dim num support 2 or 1 for now
    ...
    if (scaleShape->GetDimNum() == 2) { // dim num 2 with shape (e, 1)
        IsScaleMatchedDoubleScale =
            (scaleShape->GetDim(0) == groupNum_) && (scaleShape->GetDim(1) == 1);
    } else {
                                                   ...
                                                   const GMMAttrs &gmmAttrs) const
{
    ...
    auto perTokenMDim = gmmAttrs.transposeX ?
        perTokenScaleShape->GetDim(xdimNum_ - 1) : perTokenScaleShape->GetDim(xdimNum_ - PENULTIMATE_DIM);
...
graphStatus GroupedMatmulQuantChecker::CheckGroupedMatmulPerGroupDim(const gert::InferShapeContext *context,
                                                                     const GMMAttrs &gmmAttrs) const
{
    ...
    if (gmmAttrs.groupType == GMM_SPLIT_M) {
        OP_CHECK_IF(
            weightdimNum_ != GMM_SPLIT_M_SINGLE_WEIGHT_DIM,
            OP_LOGE(context->GetNodeName(), "The dim num of weight should be 3 when groupType is 0 (split M), but \
actual dim num is %zu.", weightdimNum_),
            return GRAPH_FAILED);
    } else if (gmmAttrs.groupType == GMM_SPLIT_K) {
        OP_CHECK_IF(
            weightdimNum_ != GMM_SPLIT_K_SINGLE_WEIGHT_DIM,
            OP_LOGE(context->GetNodeName(), "The dim num of weight should be 2 when groupType is 2 (split K), but \
actual dim num is %zu.", weightdimNum_),
            return GRAPH_FAILED);
    }
    OP_CHECK_IF(
        scaleDimNum != weightdimNum_,
        OP_LOGE(context->GetNodeName(), "The dim num of scale[%zu] should be equal to that of weight[%zu] when \
groupType is %ld.", scaleDimNum, weightdimNum_, gmmAttrs.groupType),
        return GRAPH_FAILED);
    OP_CHECK_IF(
        perTokenDimNum != xdimNum_,
        OP_LOGE(context->GetNodeName(), "The dim num of per_token_scale[%zu] should be equal to that of x[%zu].",
                  perTokenDimNum, xdimNum_),
        return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::CheckGroupedMatmulPerTileShape(const gert::InferShapeContext *context,
                                                                      const GMMAttrs &gmmAttrs) const
{
    auto scaleShape = context->GetDynamicInputShape(GMM_INDEX_IN_SCALE, 0);
    auto perTokenShape = context->GetDynamicInputShape(GMM_INDEX_IN_PERTOKEN_SCALE, 0);
    auto perTokenMDim = gmmAttrs.transposeX ?
        perTokenShape->GetDim(xdimNum_ - 1) : perTokenShape->GetDim(xdimNum_ - PENULTIMATE_DIM);
    auto perTokenKDim = gmmAttrs.transposeX ?
        perTokenShape->GetDim(xdimNum_ - PENULTIMATE_DIM) : perTokenShape->GetDim(xdimNum_ - 1);
    auto scaleKDim =
        gmmAttrs.transposeWeight ?
            scaleShape->GetDim(weightdimNum_ - 1) : scaleShape->GetDim(weightdimNum_ - PENULTIMATE_DIM);
    auto scaleNDim =
        gmmAttrs.transposeWeight ?
            scaleShape->GetDim(weightdimNum_ - PENULTIMATE_DIM) : scaleShape->GetDim(weightdimNum_ - 1);
    OP_CHECK_IF(
        perTokenMDim != xMDim_,
        OP_LOGE(context->GetNodeName(), "When quantification mode is G-B quantification, the M value in x[%ld] and \
per_token_scale[%ld] should be consistent.", xMDim_, perTokenMDim), return GRAPH_FAILED);
    OP_CHECK_IF(
        scaleNDim != (weightNDim_ + PERTILE_GROUP_SIZE - 1) / PERTILE_GROUP_SIZE,
        OP_LOGE(context->GetNodeName(), "When quantification mode is G-B quantification, the N value in scale [%ld] \
must be equal to the N value in weight [%ld] divided by 128.", scaleNDim, weightNDim_), return GRAPH_FAILED);
    if (gmmAttrs.groupType == GMM_SPLIT_M) {
...
dim of weight [%ld] divided by 128, rounded up to the next integer.",
                  perTokenKDim, scaleNDim, weightNDim_), return GRAPH_FAILED);
    } else {
...
weight [%ld] divided by 128, plus the groupSize [%ld].", perTokenKDim, scaleKDim, weightKDim_, groupNum_),
                  return GRAPH_FAILED);
    }
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::CheckShapeForPerGroupQuantParam(const gert::InferShapeContext *context,
                                                                       const GMMAttrs &gmmAttrs) const
{
    if (IsPerTileQuantMode(context, gmmAttrs)) {
        OP_CHECK_IF(CheckGroupedMatmulPerGroupDim(context, gmmAttrs) != GRAPH_SUCCESS ||
                  CheckGroupedMatmulPerTileShape(context, gmmAttrs) != GRAPH_SUCCESS,
                  OP_LOGE(context->GetNodeName(), "CheckShapeForPerGroupQuantParam failed"),
                  return GRAPH_FAILED);
    }
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::CheckPertokenShapeInNormalQuantMode(const gert::InferShapeContext *context,
                                                                           const GMMAttrs &gmmAttrs,
                                                                           const gert::Shape *perTokenScaleShape) const
{
...
perTokenScale is (m,) or (g,), which is (%ld,) or (%ld,), but the actual shape is (%ld,).",
                                xMDim_, groupNum_, perTokenScaleShape->GetDim(0)), return GRAPH_FAILED);
        } else {
...
perTokenScale is (g,1), which is (%ld,1), but the actual shape is (%ld, %ld).",
                      groupNum_, perTokenScaleShape->GetDim(0), perTokenScaleShape->GetDim(1)), return GRAPH_FAILED);
        }
    } else {
        // split k
        if (perTokenScaleShape->GetDimNum() == 1) {
...
perTokenScale is (g,), which is (%ld,), but the actual shape is (%ld,).", groupNum_, perTokenScaleShape->GetDim(0)),
                      return GRAPH_FAILED);
        } else {
            OP_CHECK_IF(
...
graphStatus GroupedMatmulQuantChecker::CheckShapeForQuantParam(const gert::InferShapeContext *context,
                                                               const GMMAttrs &gmmAttrs) const
{
    auto scaleShape = context->GetDynamicInputShape(GMM_INDEX_IN_SCALE, 0);
    auto perTokenScaleShape = context->GetDynamicInputShape(GMM_INDEX_IN_PERTOKEN_SCALE, 0);
    ...
    OP_CHECK_IF(scaleShape->GetDimNum() != 2 && scaleShape->GetDimNum() != 1,
              OP_LOGE(context->GetNodeName(), "The dim num of scale should be 1 or 2, but the actual is [%zu].",
                        scaleShape->GetDimNum()), return GRAPH_FAILED);
    if (scaleShape->GetDimNum() == 1) {
        OP_CHECK_IF(scaleShape->GetDim(0) != groupNum_,
                  OP_LOGE(context->GetNodeName(),
                            ...
                            groupNum_, scaleShape->GetDim(0)), return GRAPH_FAILED);
    } else {
        OP_CHECK_IF(
...
, but the actual 1st dim value is [%ld], and the 2nd dim value is [%ld].",
    ...
    if (IsNonEmpty(perTokenScaleShape)) {
        ...
        if(CheckPertokenShapeInNormalQuantMode(context, gmmAttrs, perTokenScaleShape) == GRAPH_FAILED) {
            return GRAPH_FAILED;
        }
    }
    return GRAPH_SUCCESS;
...
graphStatus GroupedMatmulQuantChecker::GetGroupNumValue(const gert::InferShapeContext *context)
{
...
graphStatus GroupedMatmulQuantChecker::CheckShapeForGrouplist(const gert::InferShapeContext *context) const
{
    OP_CHECK_IF(groupNum_ <= 0,
              OP_LOGE(context->GetNodeName(),
                        "The groupList 1st dim value should be greater than 0, but the actual is [%ld].",
                        groupNum_),
              return GRAPH_FAILED);
    OP_CHECK_IF(groupNum_ > 1024,
              OP_LOGE(context->GetNodeName(),
                        "Only support 1024 groups at MAX for now, but the actual group number is [%ld].",
                        groupNum_),
              return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::CheckShapeValid(const gert::InferShapeContext *context,
                                                       const GMMAttrs &gmmAttrs) const
{
    auto weightShape = context->GetDynamicInputShape(GMM_INDEX_IN_WEIGHT, 0);
    OP_CHECK_IF(xKDim_ != weightKDim_, OP_LOGE(context->GetNodeName(),
              ...
              xKDim_, weightKDim_), return GRAPH_FAILED);
    OP_CHECK_IF(CheckShapeForGrouplist(context) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeForGrouplist failed."), return GRAPH_FAILED);
    if (gmmAttrs.groupType == GMM_SPLIT_M) {
...
but the actual x dim num is [%zu], actual weight dim num is [%zu].", xdimNum_, weightdimNum_), return GRAPH_FAILED);
    }
    OP_CHECK_IF(CheckShapeForBias(context) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeForBias failed."), return GRAPH_FAILED);
    OP_CHECK_IF(!IsNonEmpty(context->GetDynamicInputShape(GMM_INDEX_IN_SCALE, 0)),
              OP_LOGE(context->GetNodeName(), "The 1st tensor of scale cannot be empty."), return GRAPH_FAILED);
    OP_CHECK_IF(CheckShapeForQuantParam(context, gmmAttrs) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeForQuantParam failed."), return GRAPH_FAILED);
...
graphStatus GroupedMatmulQuantChecker::CheckShape(const gert::InferShapeContext *context,
                                                  const GroupedMatmulCommonUtil &commonUtil) const
{
    ...
    OP_CHECK_IF(CheckScenarioValidForShape(context, commonUtil.attrsInfo) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckScenarioValidForShape failed."), return GRAPH_FAILED);
    OP_CHECK_IF(CheckShapeValid(context, commonUtil.attrsInfo) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckShapeValid failed."), return GRAPH_FAILED);
    OP_CHECK_IF(CheckFormatValid(context) != GRAPH_SUCCESS, OP_LOGE(context->GetNodeName(), "CheckFormatValid failed."),
              return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::UpdateShapeY(gert::InferShapeContext *context, size_t idxY,
...
graphStatus GroupedMatmulQuantChecker::InferOutShape(gert::InferShapeContext *context,
                                                     const GMMAttrs &gmmAttrs)
{
...
graphStatus GroupedMatmulQuantChecker::CheckScenarioValidForDtype(const gert::InferDataTypeContext *context,
                                                                  const GMMAttrs &gmmAttrs) const
{
    auto xDtype = context->GetDynamicInputDataType(GMM_INDEX_IN_X, 0);
    auto scaleDtype = context->GetDynamicInputDataType(GMM_INDEX_IN_SCALE, 0);
    ...
    if (gmmAttrs.groupType == GMM_SPLIT_K) {
        OP_CHECK_IF(xDtype == ge::DT_INT8 || scaleDtype == ge::DT_INT64 || scaleDtype == ge::DT_UINT64,
                  OP_LOGE(context->GetNodeName(),
                            "When split k, x with int8 dtype or scale with int64/uint64 dtype \
is not supported, but the acutal x dtype is [%s] and actual scale dtype is [%s].",
                            TypeUtils::DataTypeToAscendString(xDtype).GetString(),
                            TypeUtils::DataTypeToAscendString(scaleDtype).GetString()), return GRAPH_FAILED);
    }
    if ((xDtype == ge::DT_HIFLOAT8 || xDtype == ge::DT_FLOAT8_E5M2 ||
         xDtype == ge::DT_FLOAT8_E4M3FN) && perTokenScaleDtype != DT_UNDEFINED) {
        OP_CHECK_IF(
            scaleDtype != perTokenScaleDtype || (scaleDtype != DataType::DT_FLOAT && scaleDtype != DataType::DT_FLOAT8_E8M0),
            OP_LOGE(context->GetNodeName(), "When x data type is float8/hifloat8, data type of scale [%s] should be \
equal to per_token_scale's dtype [%s], and be float32/float8_e8m0.",
                    TypeUtils::DataTypeToAscendString(scaleDtype).GetString(),
                    TypeUtils::DataTypeToAscendString(perTokenScaleDtype).GetString()), return GRAPH_FAILED);
    }
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::CheckDtype(const gert::InferDataTypeContext *context,
                                                  const GroupedMatmulCommonUtil &commonUtil) const
{
    OP_CHECK_IF(CheckDtypeValid(context) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckDtypeValid failed."), return GRAPH_FAILED);
    OP_CHECK_IF(CheckScenarioValidForDtype(context, commonUtil.attrsInfo) != GRAPH_SUCCESS,
              OP_LOGE(context->GetNodeName(), "CheckScenarioValidForDtype failed."), return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}

graphStatus GroupedMatmulQuantChecker::InferOutDtype(gert::InferDataTypeContext *context)
{
    auto attrs = context->GetAttrs();
    const int64_t *outputDtype = attrs->GetInt(GMM_INDEX_ATTR_OUTPUT_DTYPE);
    DataType yDtype = context->GetDynamicInputDataType(GMM_INDEX_IN_X, 0);
...
, but the actual output dtype is [%s].",
              ge::TypeUtils::DataTypeToAscendString(yDtype).GetString()), return GRAPH_FAILED);
    yDtype = it->second;
