 ...
 * \brief
 */
#ifndef ASCENDC_GROUPED_MATMUL_KERNEL_H
#define ASCENDC_GROUPED_MATMUL_KERNEL_H
...
#define GMM_IMP(computeClass, processClass, transA, transB, sync, cfg)                                                 \
    do {                                                                                                               \
        ...
        matmulType::MT mm;                                                                                             \
        REGIST_MATMUL_OBJ(&tPipe, GetSysWorkSpacePtr(), mm, &tilingData.mmTilingData);                                 \
        ...
        op.Init(&tilingData.gmmBaseParams, &tilingData.mmTilingData, (__gm__ int32_t *)&tilingData.gmmArray,           \
                groupList);                                                                                            \
        op.Process();                                                                                                  \
    } while (0)
...
#define GMM_CUBE_STATIC_TILING_IMP(processClass, transA, transB, sync, cfg)                                            \
    do {                                                                                                               \
        ...
        computeOp.Init(x, weight, bias, scale, offset, antiquantScale, antiquantOffset, groupList, perTokenScale, y,   \
                       user1, &tilingData.gmmBaseParams, nullptr, &tPipe);                                             \
        processClass<decltype(computeOp)> op(computeOp);                                                               \
        op.Init(&tilingData.gmmBaseParams, nullptr, 0, groupList);                                                     \
...
#define GMM_CV_SPLIT_STATIC_TILING_IMP(computeClass, processClass, transA, transB, sync, cfg, aType, bType, cType)     \
    do {                                                                                                               \
        ...
        computeOp.Init(x, weight, bias, scale, offset, antiquantScale, antiquantOffset, groupList, perTokenScale, y,   \
                       user1, &tilingData.gmmBaseParams, nullptr, &tPipe);                                             \
        computeOp.InitStaticTiling(&tilingData.gmmBaseParams, user1, (cfg).baseM, (cfg).baseN);                        \
        processClass<decltype(computeOp)> op(computeOp);                                                               \
        op.Init(&tilingData.gmmBaseParams, nullptr, 0, groupList);                                                     \
        op.InitStaticTiling((cfg).baseM, (cfg).baseN);                                                                 \
        op.Process();                                                                                                  \
...
#define GMM_CV_SPLIT_IMP(computeClass, processClass, transA, transB, sync, cfg, aType, bType, cType)                   \
    do {                                                                                                               \
        ...
        computeOp.Init(x, weight, bias, scale, offset, antiquantScale, antiquantOffset, groupList, perTokenScale, y,   \
                       user1, &tilingData.gmmBaseParams, &tilingData.mmTilingData, &tPipe);                            \
        processClass<decltype(computeOp)> op(computeOp);
op.Init(&tilingData.gmmBaseParams, &tilingData.mmTilingData, (__gm__ int32_t *)&tilingData.gmmArray, groupList);
op.Process();
}
while (0)
#else
#define GMM_CV_SPLIT_IMP(computeClass, processClass, transA, transB, sync, cfg, aType, bType, cType)                   \
    do {                                                                                                               \
        ...
        preTiling.Init(groupList, tilingData.gmmBaseParams, tilingData.mmTilingData, &tPipe);                          \
        preTiling.Process(tilingData.gmmBaseParams, tilingData.mmTilingData);                                          \
        if ASCEND_IS_AIC {                                                                                             \
            mm.SetSubBlockIdx(0);                                                                                      \
            mm.Init(&tilingData.mmTilingData, &tPipe);                                                                 \
        }                                                                                                              \
...
#define GMM_A4W4_IMP(computeClass, transA, transB, cfg, aType, bType, cType)                                           \
    do {                                                                                                               \
        ...
        computeOp.Init(x, weight, scale, groupList, perTokenScale, y, user1, &tilingData.gmmBaseParams,                \
                       &tilingData.mmTilingData, &tPipe);                                                              \
        computeOp.Process();                                                                                           \
    } while (0)
...
#define GMM_CV_SPLIT_IMP_A8W4_MSD(computeClass, cfg)                                                                   \
    do {                                                                                                               \
        if ASCEND_IS_AIV {                                                                                             \
            ...
            op1.Init(x, x, groupList, user1, tilingData.gmmBaseParams, &tPipe);                                        \
            op1.Process();                                                                                             \
            tPipe.Reset();                                                                                             \
            tPipe.Destroy();                                                                                           \
            tPipe.Init();                                                                                              \
        ...
        op.Init(x, weight, bias, groupList, scale, perTokenScale, offset, nullptr, nullptr, nullptr, y, user1,         \
                &tilingData.gmmBaseParams, &tilingData.mmTilingData, &tPipe);                                          \
        op.Process();                                                                                                  \
    } while (0)
...
#define GMM_CV_SPLIT_IMP_A8W4(computeClass, cfg)                                                                       \
    do {                                                                                                               \
        if ASCEND_IS_AIV {                                                                                             \
            ...
            op1.Init(weight, y, groupList, user1, tilingData.gmmBaseParams, &tPipe);                                   \
            op1.Process();                                                                                             \
            tPipe.Reset();                                                                                             \
            tPipe.Destroy();                                                                                           \
            tPipe.Init();                                                                                              \
...
#define GMM_CV_SPLIT_IMP_A8W4_FAKEA8W8(computeClass, cfg)                                                              \
    do {                                                                                                               \
        if ASCEND_IS_AIV {                                                                                             \
            GMMA8W4FakeQuantPreProcess<wFormat> op1;                                                                   \
            op1.Init(weight, y, scale, user1, tilingData.gmmBaseParams, &tPipe);                                       \
            op1.Process();                                                                                             \
            tPipe.Reset();                                                                                             \
            tPipe.Destroy();                                                                                           \
            tPipe.Init();                                                                                              \
        ...
        computeOp.Init(x, user1, bias, user1, offset, antiquantScale, antiquantOffset, groupList, perTokenScale, y,    \
                       user1, &tilingData.gmmBaseParams, &tilingData.mmTilingData, &tPipe);                            \
        GMMProcess<decltype(computeOp)> op(computeOp);                                                                 \
        __gm__ int32_t *gmmArray_ = nullptr;                                                                           \
        op.Init(&tilingData.gmmBaseParams, &tilingData.mmTilingData, gmmArray_, groupList);                            \
        op.Process();                                                                                                  \
    }while (0)
        ...
        __aicore__ inline void GetGroupedMatmulTilingData(GM_ADDR tiling, GroupedMatmulTilingData &tilingDataObj)
        {
            // #if defined(__CCE_AICORE__) && __CCE_AICORE__ == 220
            // #if defined(GMM_ANTI_QUANT_A8W4_MSD)
            //     // ANTIQUANT_A8W4
            ...
            //                       A8W4_KERNEL_TEMPLATE == GROUPED_MATMUL_A8W4_KERNEL_TEMPLATE_MSD_VECTOR_DEQUANT ||
            //                       A8W4_KERNEL_TEMPLATE == GROUPED_MATMUL_A8W4_KERNEL_TEMPLATE_PERCHANNEL_ANTIQUANT ||
            //                       A8W4_KERNEL_TEMPLATE == GROUPED_MATMUL_A8W4_KERNEL_TEMPLATE_PERGROUP_ANTIQUANT) {
            //             GET_TILING_DATA_MEMBER(GroupedMatmulTilingData, gmmBaseParams, gmmBaseParams_, tiling);
            //             GET_TILING_DATA_MEMBER(GroupedMatmulTilingData, mmTilingData, mmTilingData_, tiling);
            //             GET_TILING_DATA_MEMBER_ADDR(GroupedMatmulTilingData, gmmArray, gmmArray_, tiling);
            //             tilingDataObj.gmmBaseParams = gmmBaseParams_;
            //             tilingDataObj.mmTilingData = mmTilingData_;
            //             tilingDataObj.gmmArray = gmmArray_;
            //         } else if constexpr (A8W4_KERNEL_TEMPLATE == GROUPED_MATMUL_A8W4_KERNEL_TEMPLATE_AUTOTILING) {
            //             GET_TILING_DATA_MEMBER(GroupedMatmulTilingData, hpTilingData, hpTilingData_, tiling);
            //             tilingDataObj.hpTilingData = hpTilingData_;
            //         }
            //     }
            ...
            //     // QUANT_A8W8O16
            //     if constexpr (D_T_A == GMM_TPL_INT8 && D_T_B == GMM_TPL_INT8 &&
            //                   (D_T_Y == GMM_TPL_BF16 || D_T_Y == GMM_TPL_FLOAT16) && TRANS_A == 0 &&
            ...
            //     if constexpr (D_T_A == GMM_TPL_INT8 && D_T_B == GMM_TPL_INT8 && (D_T_Y == GMM_TPL_INT8 || D_T_Y ==
            //     GMM_TPL_INT32) &&
            ...
            //             if ASCEND_IS_AIV {
            //                 GET_TILING_DATA(tilingDataObj, tiling);
            ...
            //             return;
            //         } else {
            //             GET_TILING_DATA_MEMBER(GroupedMatmulTilingData, gmmBaseParams, tilingDataObj.gmmBaseParams,
            //             tiling); GET_TILING_DATA_MEMBER(GroupedMatmulTilingData, mmTilingData,
            //             tilingDataObj.mmTilingData, tiling); GET_TILING_DATA_MEMBER_ADDR(GroupedMatmulTilingData,
            //             gmmArray, tilingDataObj.gmmArray, tiling);
            //         }
                                               ...
                                               GroupedMatmulTilingData &tilingData)
{
    ...
    //                                         const_cast<A8W4HPTiling *>(&tilingData.hpTilingData), &tPipe);
    //             op.Init();
    //             op.Process();
    //         }
    //     }
    ...
    //             GMM_CV_SPLIT_IMP(GMMA16W8MSDCompute, GMMA16W8MSDProcess, false, false, false, matmulCFG, xTypeMSD,
    //                              weightTypeMSD, yTypeMSD);
    //         } else if constexpr (TRANS_B == 1) {
    //             GMM_CV_SPLIT_IMP(GMMA16W8MSDCompute, GMMA16W8MSDProcess, false, true, false, matmulCFG, xTypeMSD,
    //                              weightTypeMSD, yTypeMSD);
    //         }
    //     }
    // #endif
    ...
    //                     GMM_CV_SPLIT_IMP(GMMQuantMixCoreCompute, GMMProcess, false, false, false, matmulCFG, xType,
    //                                      weightType, yType);
    //                 } else if constexpr (TRANS_B == 1 && GROUP_LIST_TYPE != GROUPED_MATMUL_GROUP_LIST_TYPE_SPARSEM) {
    //                     GMM_CV_SPLIT_IMP(GMMQuantMixCoreCompute, GMMProcess, false, true, false, matmulCFG, xType,
    //                                      weightType, yType);
    //                 } else if constexpr (TRANS_B == 0 && GROUP_LIST_TYPE == GROUPED_MATMUL_GROUP_LIST_TYPE_SPARSEM) {
    //                     GMM_CV_SPLIT_IMP(GMMQuantMixCoreCompute, GMMGroupMSparseProcess, false, false, false,
    //                     matmulCFG,
    //                                      xType, weightType, yType);
    //                 } else if constexpr (TRANS_B == 1 && GROUP_LIST_TYPE == GROUPED_MATMUL_GROUP_LIST_TYPE_SPARSEM) {
    //                     GMM_CV_SPLIT_IMP(GMMQuantMixCoreCompute, GMMGroupMSparseProcess, false, true, false,
    //                     matmulCFG,
    //                                      xType, weightType, yType);
    //                 }
    ...
    //         } else if (IS_STATIC_TILING_API == 1) {
    //             if constexpr (TRANS_B == 0 && GROUP_LIST_TYPE != GROUPED_MATMUL_GROUP_LIST_TYPE_SPARSEM) {
    //                 GMM_CV_SPLIT_STATIC_TILING_IMP(GMMQuantMixCoreCompute, GMMProcess, false, false, false,
    //                 staticCFG,
    //                                                xType, weightType, yType);
    //             } else if constexpr (TRANS_B == 1 && GROUP_LIST_TYPE != GROUPED_MATMUL_GROUP_LIST_TYPE_SPARSEM) {
    //                 GMM_CV_SPLIT_STATIC_TILING_IMP(GMMQuantMixCoreCompute, GMMProcess, false, true, false,
    //                 staticCFGtransB,
    //                                                xType, weightType, yType);
    //             } else if constexpr (TRANS_B == 0 && GROUP_LIST_TYPE == GROUPED_MATMUL_GROUP_LIST_TYPE_SPARSEM) {
    //                 GMM_CV_SPLIT_STATIC_TILING_IMP(GMMQuantMixCoreCompute, GMMGroupMSparseProcess, false, false,
    //                 false,
    //                                                staticCFG, xType, weightType, yType);
    //             } else if constexpr (TRANS_B == 1 && GROUP_LIST_TYPE == GROUPED_MATMUL_GROUP_LIST_TYPE_SPARSEM) {
    //                 GMM_CV_SPLIT_STATIC_TILING_IMP(GMMQuantMixCoreCompute, GMMGroupMSparseProcess, false, true,
    //                 false,
    //                                                staticCFGtransB, xType, weightType, yType);
    //             }
    //         }
    //     }
    // #elif defined(GMM_A4W4)
