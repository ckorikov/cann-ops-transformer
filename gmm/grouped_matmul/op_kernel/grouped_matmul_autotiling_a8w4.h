                                                  ...
                                                  TPipe *pipe)
{
    ...
    uint32_t E_M_length = ceilINT(total_M, group_num);
    int32_t UsageRate1024 = ceilINT(total_N, TOTAL_N_THRESHOLD_1024) * ceilINT(E_M_length, single_M) * group_num;
    int32_t virtualCoreNum = ceilINT((uint32_t)UsageRate1024, numAic) * numAic;
    if ((float)UsageRate1024/virtualCoreNum >= USAGE_RATE_THRESHOLD) {
