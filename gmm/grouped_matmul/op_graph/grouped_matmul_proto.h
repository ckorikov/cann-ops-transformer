...
/*!
 * \file grouped_matmul_proto.h
 * \brief
 */
#ifndef OPS_OP_PROTO_INC_GROUPED_MATMUL_H_
#define OPS_OP_PROTO_INC_GROUPED_MATMUL_H_
...
* @par Inputs:
* @li x: A Tensor List. Format supports ND. The type support float16, bfloat16, int8, float32, hifloat8, float8_e5m2, float8_e4m3fn, float4_e2m1, float4_e1m2.
* Maximum length of x is 128.
* @li weight: A Tensor List of weight. Format supports ND/NZ. The type support float16, bfloat16, int8, float32, int4, int32, hifloat8, float8_e5m2, float8_e4m3fn, float4_e2m1, float4_e1m2.
* Maximum length of weight is 128. When type is float32/int32 in fake-quantization modes which only supported in Ascend 910_95 AI Processor, the input is float4_e2m1/int4-packed data.
* @li bias: A Tensor List of bias. Format supports ND. The type support float16, float32, int32, bfloat16.
...
* @li tuning_config: A ListInt. The first element in the array indicates the expected number of tokens processed by each expert.
...
*\n
*\n
* - The following are the supported data formats and data types (for Ascend 910_95 AI Processor):
*\n
| Tensor    | x                         | weight                                  | bias                   | scale            | offset  | antiquant_scale | antiquant_offset | per_token_scale | y   |
| :-------: | :-----------------------: | :-------------------------------------: | :--------------------: | :--------------: | :-----: | :---------: | :-----: | :---------: | :----------------------: |
| Format1   | ND                        | ND                                      | ND                     | ND               | ND      | ND          | ND      | ND          | ND                       |
...
|           | float4_e2m1/float4_e1m2   | float4_e2m1/float4_e1m2                 | float32                | float8_e8m0      | float32 | float16     | float16 | float8_e8m0 | float16/bfloat16/float32 |
...
| Format2   | ND                        | FRACTAL_NZ                              | ND                     | ND               | ND      | ND          | ND      | ND          | ND                       |
| Data Type | int8                      | int4                                    | float32                | float32          | float32 | float16     | float16 | float32     | float16/bfloat16         |
|           | float16                   | float4_e2m1/float4_e1m2/float32         | float16                | uint64           | float32 | float8_e8m0 | float16 |      -      | float16                  |
|           | bfloat16                  | float4_e2m1/float4_e1m2/float32         | bfloat16/float32       | uint64           | float32 | float8_e8m0 | bfloat16|      -      | bfloat16                 |
...
*\n
* - The following are the supported data types and no quantization modes(for Ascend 910_95 AI Processor):
| x                         | weight                    | bias                   |   y                      | group_type |
| :-----------------------: | :-----------------------: | :--------------------: | :----------------------: | :--------: |
| float16                   | float16                   | float16/float32        | float16                  |-1/0/2      |
| bfloat16                  | bfloat16                  | bfloat16/float32       | bfloat16                 |-1/0/2      |
*\n
* - The following are the supported data types and quantization modes(for Ascend 910_95 AI Processor):
*\n
...
*\n
  ...
  | float4_e2m1/float4_e1m2   | float4_e2m1/float4_e1m2   | float8_e8m0          | float8_e8m0      | 0          |
*\n
perchannel：
*\n
...
*\n
  ...
  | ------------------------- | ------------------------------- | -------------------- | --------------------- | -------------------- | ---------- |
  | float16                   | float4_e2m1/float4_e1m2/float32 | float8_e8m0          | float16               |           -          | 0          |
  | bfloat16                  | float4_e2m1/float4_e1m2/float32 | float8_e8m0          | bfloat16              |           -          | 0          |
  | float8_e4m3fn             | float4_e2m1/float32             | float8_e8m0          | float16/bfloat16      | float8_e8m0          | 0          |
*\n
* - The following are the supported quantization modes, data types and shapes(for Ascend 910_95 AI Processor):
*\n
...
| mx                   | 0          | float4_e2m1/float4_e1m2            | float8_e8m0                   | [(M,K)]   | [(B,N,K)]           | [(M,N)]   | [(B,N,ceil(K/64),2)]                      | (M,ceil(K/64),2)      | -/[(B,N)]  |
...
| mx                                | 0          | float16       | float4_e2m1/float4_e1m2            | -                    | uint64     | float8_e8m0          | float16               | [(M,K)] | [(B,ceil(N/16),ceil(K/16),16,16)] | [(M,N)] | -                     | [(0)]       | [(B,ceil(K/32),N,)]         | [(0)]                  |
| mx                                | 0          | bfloat16      | float4_e2m1/float4_e1m2            | -                    | uint64     | float8_e8m0          | bfloat16              | [(M,K)] | [(B,ceil(N/16),ceil(K/16),16,16)] | [(M,N)] | -                     | [(0)]       | [(B,ceil(K/32),N,)]         | [(0)]                  |
...
* @li single-tensor: for tensor list type of input and output, tensors for different groups are not separated, and only one tensor in the tensor list.
...
* @li Quantization and fake-quantization are supported only when group_type is set to -1 or 0 for Atlas A2 Training Series Product/Atlas 800I A2 Inference Product/A200I A2 Box Heterogeneous Component and Atlas Inference Series Product.
* Quantization is supported only when group_type is set to 0 or 2 for Ascend 910_95 AI Processor.
* @li Quantization with y data type bfloat16 or float16 or float32 is only supported in single-tensor x, single-tensor weight, single-tensor y cases.
...
* @li In following tables, "S" stands for single-tensor, and "M" stands for multi-tensor, expressed in the sequence of x, weight, y.
...
*\n
*\n
* The following are the supported shapes and constrains for different scenarios(for for Ascend 910_95 AI Processor):
*\n
| group_type | supported scenario | x shape | weight shape | y shape | optional-dynamic inputs shape if needed | group_list shape if passed | per_token_scale shape if passed |    constrains    |
| :--------: | :----------------: | :-----: | :----------: | :-----: | :-------------------------------------: | :------------------------: | :-----------------------------: | :--------------: |
...
*\n
* The constrains of inputs' shape in quantization(for for Ascend 910_95 AI Processor) can be found in the table "The following are the supported quantization modes, data types and shapes(for Ascend 910_95 AI Processor)" above.
* @li Shape of offset and not needed optional-dynamic inputs is [(0)].
* @li Shape of weight indicated in above table corresponds to data format ND. Currently, only single-tensor x, single-tensor weight, single-tensor y with group_type 0 case supports weight with format NZ.
...
*\n
*\n
* @li The following are the supported group_type and constrains for different scenarios:
...
*\n
* @li Atlas Inference Series Product Constraints: \n
...
#endif  // OPS_OP_PROTO_INC_GROUPED_MATMUL_H_
