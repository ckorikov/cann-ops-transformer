...
AclnnGroupedMatmulParam::AclnnGroupedMatmulParam(std::vector<TensorList> inputs, Tensor groupList,
    std::vector<int64_t> groupListData, std::int32_t splitItem, int32_t dtype, bool transposeWeight,
    bool transposeX, int32_t groupType, int32_t groupListType, int32_t actType, std::vector<int64_t> tuningConfig, FunctionType functionType,
    AclnnGroupedMatmulVersion aclnnGMMVersion)
    : Param(std::move(inputs), Tensor(), std::move(groupList), std::move(groupListData), splitItem, dtype, transposeWeight,
    transposeX, groupType, groupListType, actType, std::move(tuningConfig)), mFunctionType(functionType), mAclnnGroupedMatmulVersion(aclnnGMMVersion)
{
}
...
bool AclnnGroupedMatmulParam::Init()
{
    ...
    auto ret_tuningConfig = InitTuningConfig();
    LOG_IF_EXPR(ret_tuningConfig == false, LOG_ERR("InitTuningConfig faild"), return false);
...
void AclnnGroupedMatmulParam::InitFunctionParams()
{
    if (mFunctionType == FunctionType::QUANT) {
        ...
        auto iter = mTensorLists.find("antiquant_offset");
        if (iter != mTensorLists.end()) {
            aclnnAntiquantOffset = ops::adv::tests::utils::AclnnTensorList(mTensorLists["antiquant_offset"]);
        }
    } else if (mFunctionType == FunctionType::QUANT_PERTOKEN || mFunctionType == FunctionType::MXFP || mFunctionType == FunctionType::PERTILE) {
        bool scaleTrans = (mFunctionType == FunctionType::MXFP || mFunctionType == FunctionType::PERTILE) ? mTransposeWeight : false;
        bool pertokenScaleTrans = (mFunctionType == FunctionType::MXFP || mFunctionType == FunctionType::PERTILE) ? mTransposeX : false;
        aclnnScale = ops::adv::tests::utils::AclnnTensorList(mTensorLists["scale"], scaleTrans);
        aclnnPerTokenScale =
            ops::adv::tests::utils::AclnnTensorList(mTensorLists["pertoken_scale"], pertokenScaleTrans);
        auto iter = mTensorLists.find("offset");
        if (iter != mTensorLists.end()) {
            aclnnOffset = ops::adv::tests::utils::AclnnTensorList(mTensorLists["offset"]);
        }
...
bool AclnnGroupedMatmulParam::InitGroupList()
{
    ...
    if (mAclnnGroupedMatmulVersion == AclnnGroupedMatmulVersion::V3 ||
        ...
        mAclnnGroupedMatmulVersion == AclnnGroupedMatmulVersion::V5 ||
        mAclnnGroupedMatmulVersion == AclnnGroupedMatmulVersion::WeightNz) {
...
bool AclnnGroupedMatmulParam::InitTuningConfig()
{
    if (mTuningConfig.size() == 0) {
        return true;
    }

    if (mAclnnGroupedMatmulVersion == AclnnGroupedMatmulVersion::V5 ||
        mAclnnGroupedMatmulVersion == AclnnGroupedMatmulVersion::WeightNz) {
        if (!InitAclIntArray(&aclnnTuningConfigIntAry, mTuningConfig)) {
            return false;
        }
    }
