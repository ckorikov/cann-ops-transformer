...
createOpapiLatestSoftlink() {
    ...
    if [ -d $(dirname $1)/latest/${architectureDir}/lib64 ]; then
        for file_so in $(ls -1 $1/${architectureDir}/lib64 | grep -E "libaclnn_|libopapi.so")
        do
...
latestSoftlinksRemove() {
    ...
    if [ -d $(dirname $targetdir)/latest/${architectureDir}/lib64 ]; then
        for file_so in $(ls -l "$(dirname $targetdir)/latest/${architectureDir}/lib64/"  | grep -E "libaclnn_|libopapi.so")
        do         
...
softlinksRemove() {
    ...
    if [ -d $targetdir/${architectureDir}/lib64 ]; then
        for file_so in $(ls -1 $targetdir/${architectureDir}/lib64 | grep -E "libaclnn_|libopapi.so")
        do
    ...
    if [ -d $targetdir/opp/lib64 ]; then
        for file_so in $(ls -l $targetdir/opp/lib64 | grep -E "libaclnn_|libopapi.so")
        do 
