...
opapi_transformer_lib_src_path=${version_install_dir}/${opp_platform_dir}/lib64/libopapi_transformer.so
opgraph_transformer_lib_src_path=${version_install_dir}/${opp_platform_dir}/lib64/libopgraph_transformer.so
ophost_transformer_lib_src_path=${version_install_dir}/${opp_platform_dir}/lib64/libophost_transformer.so
common_transformer_lib_src_path=${version_install_dir}/${opp_platform_dir}/lib64/libcommon_transformer.so
...
do
    ...
    lib_relative_path=$(realpath --relative-to="${!dst_path%/*}" "${!src_path}")

    chmod u+w "${!dst_path}"  2> /dev/null
    logandprint "[INFO]: Creating soft link from ("${!src_path}") to ("${!dst_path}")."
    createsoftlink "${lib_relative_path}" "${!dst_path}"
    logwitherrorlevel "$?" "warn" "[WARNING]: Create soft link for "${!dst_path}" failed. \
    That may cause some issues for ops_transformer "${lib_name}"."
done
...
lib_relative_path=$(realpath --relative-to="${aclnn_kernels_dst_dir%/*}" "${aclnn_kernels_src_dir}")

logandprint "[INFO]: Creating soft link from ("$aclnn_kernels_src_dir") to ("${aclnn_kernels_dst_dir}")."
createsoftlink "${lib_relative_path}" "${aclnn_kernels_dst_dir}"
logwitherrorlevel "$?" "warn" "[WARNING]: Create soft link for "${aclnn_kernels_dst_dir}" failed. That may cause some issues for ops_transformer aclnn."
chmod -R "${_BUILTIN_PERM}" "${aclnn_kernels_dst_dir}" 2> /dev/null

# create opp symbolic link in latest
opp_latest_src_dir=${version_install_dir}/opp
opp_latest_dst_dir=${_TARGET_INSTALL_PATH}/latest/opp
opp_relative_path=$(realpath --relative-to="${opp_latest_dst_dir%/*}" "${opp_latest_src_dir}")

logandprint "[INFO]: Creating soft link from ("$opp_latest_src_dir") to ("${opp_latest_dst_dir}")."
createsoftlink "${opp_relative_path}" "${opp_latest_dst_dir}"
logwitherrorlevel "$?" "warn" "[WARNING]: Create soft link for "${opp_latest_dst_dir}" failed. That may cause some issues for ops_transformer."
chmod -R "${_BUILTIN_PERM}" "${opp_latest_dst_dir}" 2> /dev/null
...
[ export ASCEND_OPS_TRANSFORMER_PATH=${install_version_dir}/${opp_platform_dir} ] to set the environment path."
fi
