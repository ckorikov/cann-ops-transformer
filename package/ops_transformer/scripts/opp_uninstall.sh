...
# remove ops_transformer symbolic link
lib_transformer_dir=${_TARGET_INSTALL_PATH}/latest/${platform_data}-linux/lib64/
...
chmod u+w "${lib_transformer_dir}" 2> /dev/null
...
do
    dst_link="${lib_name}_lib_dst_path"
    if [ -L "${!dst_link}" ]; then
        logandprint "[INFO]: Delete the ops_transformer soft link ("${!dst_link}")."
        rm -rf "${!dst_link}"
        logwitherrorlevel "$?" "warn" "[WARNING]Delete "${lib_name}" soft link failed, that may cause \
        some error to ops_transformer."
    fi
done
chmod u-w "${lib_transformer_dir}" 2> /dev/null

aclnn_kernels_dst_dir=${_TARGET_INSTALL_PATH}/latest/${platform_data}-linux/include/aclnn_kernels
if [ -L "${aclnn_kernels_dst_dir}" ]; then
    logandprint "[INFO]: Delete the ops_transformer aclnn_kernels soft link ("${aclnn_kernels_dst_dir}")."
    rm -rf "${aclnn_kernels_dst_dir}"
    logwitherrorlevel "$?" "warn" "[WARNING]Delete "${aclnn_kernels_dst_dir}" soft link failed, that may cause \
    some error to ops_transformer."
fi
