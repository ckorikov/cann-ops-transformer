...
sys.path.append(os.path.abspath(os.path.dirname(__file__)) + '/../scripts/common_log')
import comm_log as COMM_LOG
...
class Require(NamedTuple):
    ...
    @classmethod
    def _to_required_str(cls, versions: List) -> List[str]:
        """转换为版本需求字符串。"""
...
def gen_ubengine_version(full_path: str) -> str:
    format_month = f"{datetime.now().month:03d}"
    with open(full_path, 'r') as file:
        return file.readlines()[0].strip()+format_month


def read_version_conf(top_dir: str, version: str) -> str:
    full_path = os.path.join(top_dir, version)
    print(full_path)
    if os.path.isfile(full_path):
        if "ubengine" in full_path:
            return gen_ubengine_version(full_path)
        with open(full_path, 'r') as file:
            return file.readlines()[0].strip()
    COMM_LOG.cilog_error(THIS_FILE_NAME, "the version(%s) option is incorrectly configured !", version)
    sys.exit(-1)
...
def parse_version_conf(top_source_dir: str, version: str) -> str:
    if is_version_number(version):
        return version
    return read_version_conf(top_source_dir, version)
...
class VersionXml(NamedTuple):
    ...
    @classmethod
    def parse(cls, filepath: Path, top_dir: str):
        """解析版本号定义文件。"""
        tree = ET.parse(filepath)
        root_ele = tree.getroot()
        # 支持version.xml中version属性配置文件路径
        release_version = parse_version_conf(top_dir, root_ele.attrib['version'])
        cur_ver = ".".join(release_version.split(".")[:2])
        version_dir = root_ele.get('version_dir')
        if version_dir != None:
            version_dir = version_dir.replace("$(CUR_VER)", cur_ver)

        packages = {}
        for package_ele in root_ele.findall("./package"):
            pkg_name = package_ele.attrib['name']
            if pkg_name in packages:
                raise DuplicatedPkgConfig(pkg_name)

            compatible_ele = package_ele.find("./compatible")
            if compatible_ele is None:
                compatible = CompatibleElement(items=[])
            else:
                compatible = CompatibleElement.parse(compatible_ele, cur_ver)
            packages[pkg_name] = compatible

        return cls(release_version, version_dir, packages)
...
class VersionInfoFile(NamedTuple):
    ...
    def _get_content(self) -> str:
        ...
        if self.base_version:
            lines.append('base_version={0}'.format(self.base_version))
        if self.spc_version:
            lines.append('spc_version={0}'.format(self.spc_version))
        if self.timestamp:
        ...
        if self.requires:
            requires_strs = [require.to_required_full_str() for require in self.requires]
            lines.extend(requires_strs)
