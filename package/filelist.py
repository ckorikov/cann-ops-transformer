...
from typing import Callable, Iterator, List, NamedTuple, Set, Tuple, Union

from pkg_utils import (
    COMM_LOG, FilelistError, GenerateFilelistError, conditional_apply,
    pairwise, swap_args
)
from pkg_feature import config_feature_to_string
from utils.funcbase import (
    any_, constant, dispatch, identity, invoke, pipe, side_effect, star_apply
)
...
def create_fileitem(*args, **kwargs) -> FileItem:
    """创建文件条目。"""
    fileitem = FileItem(*args, **kwargs)

    if not isinstance(fileitem.feature, set):
        raise TypeError('The feature parameter should be a set.')
    if not isinstance(fileitem.chip, set):
        raise TypeError('The chip parameter should be a set.')
    if not isinstance(fileitem.softlink, list):
        raise TypeError('The softlink parameter should be a list.')
    if not isinstance(fileitem.pkg_inner_softlink, list):
        raise TypeError('The pkg_inner_softlink parameter should be a list.')

    return fileitem


def parse_fileitem(line: Union[str, List, Tuple], is_dir: bool = False) -> FileItem:
    """解析文件条目。"""
    ...
    if parts[8] == 'NA':
        ...
        softlink = parts[8].split(';')

    if parts[9] == 'all':
        feature = set()
    else:
        feature = set(parts[9].split(';'))

    if parts[14] == 'NA':
        pkg_inner_softlink = []
    else:
        pkg_inner_softlink = parts[14].split(';')

    if parts[15] == 'all':
        chip = set()
    else:
        chip = set(parts[15].split(';'))

    params = chain(
        parts[:8], (softlink, feature), parts[10:14], (pkg_inner_softlink, chip, is_dir)
    )
    return create_fileitem(*params)
...
def softlinks_to_string(softlinks: List[str]) -> str:
    """软链接转换为字符串。"""
    if not softlinks:
        return 'NA'
    return ';'.join(softlinks)


def fileitem_to_string(item: FileItem) -> str:
    """文件条目转换为字符串。"""
    return ','.join([
        ...
        softlinks_to_string(item.softlink), config_feature_to_string(item.feature),
        item.is_common_path, item.configurable, item.hash_value, item.block,
        softlinks_to_string(item.pkg_inner_softlink), config_feature_to_string(item.chip)
    ])
...
def fill_is_common_path(filelist: FileList, target_env: str) -> Iterator[FileItem]:
    ...
    def get_softlinks_not_in_common_paths():
        for fileitem in filelist:
            if fileitem.relative_install_path.startswith(target_env):
                for softlink in fileitem.softlink:
                    if not softlink.startswith(target_env):
                        yield softlink

    softlinks = set(get_softlinks_not_in_common_paths())
    for fileitem in filelist:
        if fileitem.relative_install_path.startswith(target_env):
            yield fileitem._replace(is_common_path='Y')
        else:
            is_softlinks_prefix = map(
                methodcaller('startswith', f'{fileitem.relative_install_path}/'), softlinks
            )
            if any(is_softlinks_prefix):
                yield fileitem._replace(is_common_path='YY')
            else:
                yield fileitem
...
def is_specific_operations(fileitem: FileItem, operations: List[str]) -> bool:
    """是否为特定的操作类型。"""
    if fileitem.operation in operations:
        return True
    return False


def is_specific_install_type(fileitem: FileItem, install_types: Set[str]) -> bool:
    """是否为特定的安装类型。"""
    item_install_types = set(fileitem.install_type.split(';'))
...
def get_missing_dir_set(filelist: FileList) -> Set[str]:
    ...
    mkdir_installs: Set[str] = {
        fileitem.relative_install_path
        for fileitem in filter(
            partial(is_specific_operations, operations={'mkdir'}),
            filelist
        )
        if is_relative_install_path(fileitem.relative_install_path)
    }
...
def print_missing_dir_set(missing_dir_set: Set[str], in_msg: str = None) -> Set[str]:
    ...
    for path in sorted(missing_dir_set):
        COMM_LOG.cilog_error(THIS_FILE_NAME, f'missing dir info path "{path}"{tail_msg}')
    return missing_dir_set
...
def print_unsafe_paths(unsafe_paths: Tuple[str, ...]) -> Tuple[str, ...]:
    ...
    for path in unsafe_paths:
        COMM_LOG.cilog_error(THIS_FILE_NAME, f'unsafe path "{path}" in move scene.')
    return unsafe_paths
    ...
    pipe(
        dispatch(
            pipe(
                ...
                partial(map, lambda x: set((x, 'comm'))),
                # 此时为feature集合序列
            ),
            repeat,  # 重复filelist
        ),
...
def commonpath(*args: List[str]) -> str:
    """公共路径前缀。"""
    try:
...
def is_nested_fileitem(item: FileItem, base_item: FileItem) -> FileItemRelation:
    """是否为嵌套的文件。"""
    ...
    common_install_path = commonpath(install_path, base_install_path)
...
def found_nested_fileitem(item: FileItem, base_item: FileItem):
    """发现嵌套元素。"""
...
def convert_nested_path_in_filelist(filelist: FileList):
    ...
    for item in filelist:
        ret = is_nested_fileitem(item, preitem)
...
check_nested_path_in_filelist = pipe(
    ...
    partial(
        map,
        conditional_apply(star_apply(is_nested_fileitem), star_apply(found_nested_fileitem))
    ),
    list,
)
...
def generate_filelist(filelist: FileList, delivery_dir: str, filename: str):
    """生成文件列表文件。"""
    content_list = list(
        itertools.chain(
            [get_filelist_header_string()],
            [fileitem_to_string(item) for item in filelist]
        )
    )
    content = '\n'.join(content_list)
