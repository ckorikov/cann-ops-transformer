...
算子功能：在快速恢复检测流程中，对一个通信域内的所有卡发送数据并写状态位，以供aclnnElasticReceivableInfoCollect检测通信链路是否正常。
...
- 示例代码如下，仅供参考
    ...
    int DetectionThreadFun(Args &args)
    {
        ...
        if (needClean) {
            ...
            ret = aclnnMoeDistributeBufferResetGetWorkspaceSize(resetTensor, hcomEpName, EP_WORLD_SIZE, &resetWorkspaceSize, &resetExecutor);
