...
- 算子功能：当存在TP域通信时，先进行ReduceScatterV通信，再进行AlltoAllV通信，最后将接收的数据整合（乘权重再相加）；当不存在TP域通信时，进行AlltoAllV通信，最后将接收的数据整合（乘权重再相加），之后完成Add + RmsNorm融合。支持特殊专家场景。
...
<colgroup>
 ...
 <tbody>
  ...
  <tr>
   <td>groupTp</td>
   <td>输入</td>
   <td>TP通信域名称（数据并行），字符串长度[0, 128)，不能和groupEp相同，仅在无TP域通信时支持传空字符串。</td>
   <td>STRING</td>
   <td>-</td>
  </tr>
  <tr>
