...
每个算子分为[两段式接口](common/两段式接口.md)，必须先调用“aclnnMatmulAllReduceAddRmsNormGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnMatmulAllReduceAddRmsNorm”接口执行计算。
...
* `aclnnStatus aclnnMatmulAllReduceAddRmsNorm(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor, const aclrtStream stream)`
...
- 使用场景同融合算子[aclnnMatmulAllReduce](../context/aclnnMatmulAllReduce.md)一致：增量场景不使能MC2，全量场景使能MC2。
