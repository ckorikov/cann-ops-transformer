 ...
 */
/*!
 * \file moe_distribute_combine_a2_layered_aicpu.h
 * \brief
 */
...
#define TemplateMC2TypeA2layeredAicpuFunc ExpandXType, ExpandIdxType
using namespace AscendC;
template <TemplateMC2TypeA2layeredAicpuClass>
class MoeDistributeCombineA2LayeredAicpu {
public:
...
private:
    ...
    TBuf<> countReduceInt16Buf_;    // k=1 高精度
...
template <TemplateMC2TypeA2layeredAicpuClass>
__aicore__ inline void MoeDistributeCombineA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::Init(GM_ADDR expandX, GM_ADDR expertIds,
...
__aicore__ inline void MoeDistributeCombineA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::BuffInit()
{
    ...
    tpipe_->InitBuffer(sendCountBuf_,
                       RoundUp(moeExpertNum_, B32_PER_BLOCK) * sizeof(int32_t)); // 保存累计出现次数，用于计算windows中的偏移
    ...
    uint32_t offsetReduceRange = axisK_ >= serverNum ? axisK_ : serverNum;  //在k较小的情况，防止DataCopy长度越界
    tpipe_->InitBuffer(offsetReduceBuf_, RoundUp(maxLocalBs * offsetReduceRange * 4, (uint32_t)UB_ALIGN));
    tpipe_->InitBuffer(countReduceBuf_, (maxLocalBs + 8) * 4); // 1k
    tpipe_->InitBuffer(countReduceInt16Buf_, (maxLocalBs + 8) * 4);
...
__aicore__ inline void MoeDistributeCombineA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::SumToWindow()
{
    ...
    if (coreIdx_ < serverNum) {
        ...
        DataCopy(countReduceLocal,
                 ...
                 RoundUp(maxLocalBs, (uint32_t)(UB_ALIGN / sizeof(int16_t))));
        SyncFunc<AscendC::HardEvent::MTE2_S>();
...
template <TemplateMC2TypeA2layeredAicpuClass>
__aicore__ inline void MoeDistributeCombineA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::AlltoAllServerDispatch()
{
...
template <TemplateMC2TypeA2layeredAicpuClass>
__aicore__ inline void MoeDistributeCombineA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::SetStatus()
{
    if (coreIdx_ != 0U) {
        SyncAll<true>();
        return;
...
template <TemplateMC2TypeA2layeredAicpuClass>
__aicore__ inline void MoeDistributeCombineA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::WaitDispatch()
{
    if (coreIdx_ < serverNum) {
...
template <TemplateMC2TypeA2layeredAicpuClass>
__aicore__ inline void MoeDistributeCombineA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::Preload()
{
...
template <TemplateMC2TypeA2layeredAicpuClass>
__aicore__ inline void MoeDistributeCombineA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::SumToServer()
{
    uint32_t reduceCore = 8U;
    if (coreIdx_ >= reduceCore) {
        SyncAll<true>();
    ...
    for (uint32_t i = startBs; i < endBs; i++) {
        ...
        for (int j = 0; j < copyNum; j++) {
            ...
            Cast(rowTmpFloatLocal_, tmpOtherUb_, AscendC::RoundMode::CAST_NONE, axisH_);
            PipeBarrier<PIPE_V>();
            // 将mulBufLocal累加到sumFloatBufLocal
            AscendC::Add(sumFloatLocal_, sumFloatLocal_, rowTmpFloatLocal_, axisH_);
            moeSumQueue_.FreeTensor<ExpandXType>(tmpOtherUb_);
            offsetIndex++;
        }
...
template <TemplateMC2TypeA2layeredAicpuClass>
__aicore__ inline void MoeDistributeCombineA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::Process()
{
    if ASCEND_IS_AIV {
        AlltoAllDispatch();
        SumToWindow();
...
#endif  // MOE_DISTRIBUTE_COMBINE_A2_LAYERED_AICPU_H
