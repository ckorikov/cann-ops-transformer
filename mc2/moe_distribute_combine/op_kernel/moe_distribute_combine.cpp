...
extern "C" __global__ __aicore__ void moe_distribute_combine(GM_ADDR expandX, GM_ADDR expertIds, GM_ADDR expandIdx,
                                                             ...
                                                             GM_ADDR groupList, GM_ADDR expandScales, GM_ADDR performanceInfo, GM_ADDR XOut, 
...

{
  ...
  if (TILING_KEY_IS(2000)) {
    ...
    op.Init(expandX, expertIds, expandIdx, epSendCount, scales, xActiveMask, performanceInfo, XOut, workspaceGM, &pipe, &tilingData);
    op.Process();
  }
  if (TILING_KEY_IS(3000)) {
    ...
    if (dataplaneMode == DataplaneMode::AICPU) {
      ...
      op.Init(expandX, expertIds, expandIdx, epSendCount, expandScales, performanceInfo, XOut, workspaceGM, &pipe, &tilingData,
        contextGM0);
      op.Process();
    }
