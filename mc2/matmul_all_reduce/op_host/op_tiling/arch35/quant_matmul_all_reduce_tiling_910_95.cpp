...
constexpr uint64_t PERTILE_FP32_WORKSPACE_CNT = 3;
...
ge::graphStatus QuantMatmulAllReduceTilingA5::GetDynamicQuantTempBuffSize()
{
    ...
    int64_t procRowsRaw = MutableTCubeTileTilingData().get_M() < aivNum ? 1 : MutableTCubeTileTilingData().get_M() / aivNum;
    ...
    OP_TILING_CHECK(
        tempBuffSize > ubSize,
        VECTOR_INNER_ERR_REPORT_TILING(
            opName_, "Dynamic Quant tempBuffSize=%lu is greater than total ubSize=%lu", tempBuffSize, ubSize),
        return ge::GRAPH_FAILED);
...
ge::graphStatus QuantMatmulAllReduceTilingA5::GetWorkspaceSize()
{
    ...
    if (isFp8) {
        // 存放Matmul输出+2次quant输出+1次alltoall输出+1次reduceSum输出+2次dequant输出+1次allgather输出
        myWorkSpaceSize_ = myWorkSpaceSize_ + PERTILE_FP8_WORKSPACE_CNT * commInt8WorkSpace +
                           ...
                           commFp32WorkSpace / args_.rankDim;
    } else {
