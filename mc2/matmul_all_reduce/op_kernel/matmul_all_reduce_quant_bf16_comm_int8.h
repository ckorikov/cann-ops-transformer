        ...
        if (isSendTileFlag_) { // 需要同步前一块
            ...
            if (isTailFlag && (i == 0U)) {
                MatmulAllReduceReduceSumInt8<yType>(
                    tempBuffGM_, commQuantScale1GM_, commQuantScale2GM_, tilePadM_, mmTiling->matmulTiling.N, tPipe_,
                    hccl_);
    ...
    xType, wType, fFormat, wFormat, scaleType, yType, commType, aTrans, bTrans>::PrepareInit()
{
    ...
    if (GetBlockIdx() == 0) {
        ...
        for (uint32_t i = 0U; i < numN; ++i) { // 按总核数下发
            allToAllHandleId_[nowAll2AllIdx] = hccl_.AlltoAll<false>(
                all2AllSendGM_[nowAll2AllIdx], allRecvOutGM_[nowAll2AllIdx], SendCountCheck(nowAll2AllIdx),
            ...
            allGatherHandleId_[nowAllGatherIdx] = hccl_.AllGather<false>(
                allGatherSendGM_[nowAllGatherIdx], allRecvOutGM_[nowAllGatherIdx], SendCountCheck(nowAllGatherIdx),
                AscendC::HCCL_DATA_TYPE_INT8, 0);
            nowAllGatherIdx++;
            allGatherHandleId_[nowAllGatherIdx] = hccl_.AllGather<false>(
                allGatherSendGM_[nowAllGatherIdx], allRecvOutGM_[nowAllGatherIdx], SendCountCheck(nowAllGatherIdx),
...
template <
    typename xType, typename wType, int fFormat, int wFormat, typename scaleType, typename yType, typename commType,
    bool aTrans, bool bTrans>
__aicore__ inline uint32_t MatmulAllReduceQuantBF16CommInt8<
...
template <
    ...
    bool aTrans, bool bTrans>
__aicore__ inline void
MatmulAllReduceQuantBF16CommInt8<xType, wType, fFormat, wFormat, scaleType, yType, commType, aTrans, bTrans>::Process(
    BmmDequantBf16<xType, wType, fFormat, wFormat, yType, yType, aTrans, bTrans, true>& opTile,
    BmmDequantBf16<xType, wType, fFormat, wFormat, yType, yType, aTrans, bTrans, true>& opTail)
{
    auto&& mc2Tiling = tilingData_->param;
