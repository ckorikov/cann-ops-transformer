...
public:
    ...
    __aicore__ inline void Init(TPipe* tPipe, uint32_t quantUbSize)
    {
        ...
        if (this->splitMode == SPLIT_M_MATMUL_ALLREDUCE_INT8) {
            ...
            pipe->InitBuffer(
                inQueueS, DOUBLE_BUFFER_MATMUL_ALLREDUCE_INT8,
        ...
        int64_t scaleAddrOffsetSplitMN, uint32_t blockCntSpiltMN)
    {
        ...
        mmOutGm.SetGlobalBuffer(
            reinterpret_cast<__gm__ T*>(this->mmOut) + curBlockAddrOffset, curBlockCnt); // 用于读入数据
        quantedOutGm.SetGlobalBuffer(
            reinterpret_cast<__gm__ int8_t*>(this->quantedOut) + curBlockAddrOffset, curBlockCnt); // 用于输出数据
        if (((loopIdx == 0) && (this->splitMode == SPLIT_M_MATMUL_ALLREDUCE_INT8)) ||
            (this->splitMode == SPLIT_MN_MATMUL_ALLREDUCE_INT8)) {
            quantScaleGm.SetGlobalBuffer(
                reinterpret_cast<__gm__ T*>(this->quantScale) + curScaleAddrOffset, curScaleCnt);
        }
    }
