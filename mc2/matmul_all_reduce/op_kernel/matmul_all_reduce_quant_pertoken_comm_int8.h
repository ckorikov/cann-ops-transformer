...
template <
    typename xType, typename wType, int fFormat, int wFormat, typename scaleType, typename yType, typename commType,
...
private:
    __aicore__ inline void InnerProcess(
        ...
        uint32_t tileCnt, uint32_t padM, Mc2QuantBatchMatmulV3TilingData* mmTiling, uint32_t isAdd, uint32_t needUbBuffer,
        bool isTailFlag);
...
MatmulAllReduceQuantPertokenInt8<xType, wType, fFormat, wFormat, scaleType, yType, commType, aTrans, bTrans>::
    InnerProcess(
        BmmDequantPertoken<xType, wType, fFormat, wFormat, scaleType, yType, aTrans, bTrans, true>& op,
        uint32_t tileCnt, uint32_t curPadM, Mc2QuantBatchMatmulV3TilingData* mmTiling, uint32_t isAdd,
        uint32_t needUbBuffer, bool isTailFlag)
{
    ...
    for (uint32_t i = 1U; i <= tileCnt; ++i) {
        ...
        MatmulAllReduceQuantPerchannelCommInt8<yType>(
            cGM_, commQuantScale1GM_, tempBuffWinOrGM_, tPipe_, mmTiling->matmulTiling.N, mmTiling->matmulTiling.M);
        SyncAll();
        ...
        if (isSendTileFlag_) {
            ...
            SyncAll();
            if (isTailFlag && i == 1U) {
                MatmulAllReduceReduceSumInt8<yType>(
                    tempBuffGM_, commQuantScale1GM_, commQuantScale2GM_, tilePadM, mmTiling->matmulTiling.N, tPipe_,
                    hccl_);
                tempBuffGM_ += tilePadM * mmTiling->matmulTiling.N;
            } else {
                MatmulAllReduceReduceSumInt8<yType>(
                    tempBuffGM_, commQuantScale1GM_, commQuantScale2GM_, curPadM, mmTiling->matmulTiling.N, tPipe_,
                    hccl_);
...
template <
    typename xType, typename wType, int fFormat, int wFormat, typename scaleType, typename yType, typename commType,
    bool aTrans, bool bTrans>
__aicore__ inline uint32_t MatmulAllReduceQuantPertokenInt8<
    xType, wType, fFormat, wFormat, scaleType, yType, commType, aTrans, bTrans>::SendCountCheck(uint32_t prepareIndex)
{
...
__aicore__ inline void MatmulAllReduceQuantPertokenInt8<
    xType, wType, fFormat, wFormat, scaleType, yType, commType, aTrans, bTrans>::PrepareInit()
{
    ...
    if (GetBlockIdx() == 0) { // V核的0核下发通信任务
        ...
        for (uint32_t i = 0; i < N; i++) { // 按总核数下发
            allToAllHandleId_[allToAllIdx_] = hccl_.AlltoAll<false>(
                allToAllsendGM_[allToAllIdx_], allRecvOutGM_[allToAllIdx_], SendCountCheck(allToAllIdx_),
                AscendC::HCCL_DATA_TYPE_INT8);
            allToAllIdx_++;
            allToAllHandleId_[allToAllIdx_] = hccl_.AlltoAll<false>(
                allToAllsendGM_[allToAllIdx_], allRecvOutGM_[allToAllIdx_], SendCountCheck(allToAllIdx_),
                AscendC::HCCL_DATA_TYPE_INT8);
            allToAllIdx_++;
            allGatherHandleId_[allGatherIdx_] = hccl_.AllGather<false>(
                allGatherSendGM_[allGatherIdx_], allRecvOutGM_[allGatherIdx_], SendCountCheck(allGatherIdx_),
    ...
    BmmDequantPertoken<xType, wType, fFormat, wFormat, scaleType, yType, aTrans, bTrans, true>& opTail)
{
    ...
    PrepareInit();
    InnerProcess(
        opTile, mc2Tiling.tileCnt, tilePadM, &tilingData_->tilematmulTiling, mc2Tiling.isAdd, mc2Tiling.needUbBuffer,
        false);
    if (mc2Tiling.tailM != 0U) {
        InnerProcess(
            opTail, mc2Tiling.tailCnt, tailPadM, &tilingData_->tailmatmulTiling, mc2Tiling.isAdd,
    ...
    MatmulAllReduceReduceSumInt8<yType>(
        tempBuffGM_, commQuantScale1GM_, commQuantScale2GM_, padM, lastN, tPipe_, hccl_);
    SyncAll();
    if (GetBlockIdx() == 0) {
        hccl_.Commit(allGatherHandleId_[allToAllWaitIdx_]);
        allToAllWaitIdx_++;
    ...
    for (uint32_t i = 0; i < mc2Tiling.tileCnt + mc2Tiling.tailCnt; i++) {
        ...
        if (i < mc2Tiling.tileCnt) { // dequant只传有效大小
            MatmulAllReduceDequantPerchannelCommInt8<yType>(
                tempBuffDequantGM_, commQuantScale2GM_, outGM_, tPipe_, tilingData_->tilematmulTiling.matmulTiling.N,
            ...
            MatmulAllReduceDequantPerchannelCommInt8<yType>(
                tempBuffDequantGM_, commQuantScale2GM_, outGM_, tPipe_, tilingData_->tailmatmulTiling.matmulTiling.N,
