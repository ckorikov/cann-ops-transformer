...
public:
    ...
    __aicore__ inline void Init(GM_ADDR dequantOut, GM_ADDR output, uint64_t dataCnt, uint64_t realCnt,
                                uint64_t rankNum, uint64_t coreNum, TPipe *tPipe)
    {
    ...
    __aicore__ inline void ProcessSum(uint64_t progress, uint64_t calCnt)
    {
        ...
        for (uint64_t i = 0; i < this->rankNum_; i++) {
            ...
            uint64_t realCalCnt = calCnt;
            bool enterPad = (globalOffset + calCnt) > this->realCnt_;
            if (enterPad) {
                realCalCnt = (this->realCnt_ <= globalOffset) ? 0 : (this->realCnt_ - globalOffset);
            }
            if (realCalCnt > 0) {
                DataCopyExtParams GtLCopyParams = {1, static_cast<uint32_t>(realCalCnt * sizeof(float)), 0, 0, 0};
...
__aicore__ inline void MatmulAllReduceReduceSumKernel(GM_ADDR dequantOut, GM_ADDR output, uint32_t dataCnt,
                                                      uint32_t realCnt, uint32_t rankNum, TPipe *tPipe)
{
    if ASCEND_IS_AIC {
        return;
    }
    ...
    op.Init(dequantOut, output, static_cast<uint64_t>(dataCnt), static_cast<uint64_t>(realCnt),
            static_cast<uint64_t>(rankNum), static_cast<uint64_t>(coreNum), tPipe);
    op.Process();
}
} // namespace MatmulAllReduceReduceSumImpl
