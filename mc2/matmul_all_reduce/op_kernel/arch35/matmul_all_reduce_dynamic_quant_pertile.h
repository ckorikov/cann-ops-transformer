...
#include "matmul_all_reduce_dynamic_quant_utils.h"
#include "../common.h"
...
public:
    ...
    TPipe* pipe_;
    TQue<QuePosition::VECIN, DOUBLE_BUFFER> tilesQ_;
    TQue<QuePosition::VECOUT, DOUBLE_BUFFER> outTilesQ_;
    TQue<QuePosition::VECOUT, DOUBLE_BUFFER> scalesQ_;
                                          ...
                                          uint64_t curRows, bool isFirstTail, bool isSecondTail)
    {
        ...
        pipe_->InitBuffer(scalesQ_, DOUBLE_BUFFER,
                                     ...
                                     DataCopyExtParams lToGNCopyParams, DataCopyExtParams lToGSCopyParams)
    {
        ...
        LocalTensor<float> curScale = scalesQ_.AllocTensor<float>();
        ...
        AscendC::DynamicQuant<T>(curScaleCnt, padCalCnt, tilesLocal, curOutTiles, curScale, tempWorkTiles, tempScale,
                                 tempMskBuf);
        scalesQ_.EnQue<float>(curScale);
        outTilesQ_.EnQue<T>(curOutTiles);
        LocalTensor<float> outScaleLocal = scalesQ_.DeQue<float>();
        ...
        scalesQ_.FreeTensor(outScaleLocal);
    ...
    __aicore__ inline void ProcessDequant(uint64_t innerIdx, uint64_t curRows, uint32_t tileN, uint32_t inputNandSLen)
    {
        ...
        LocalTensor<float> curScales = scalesQ_.AllocTensor<float>();
        ...
        scalesQ_.EnQue<float>(curScales);
                                       ...
                                       uint32_t curScaleCnt)
    {
        ...
        LocalTensor<float> scalesLocal = scalesQ_.DeQue<float>();
        ...
        tilesQ_.FreeTensor(tilesLocal);
        scalesQ_.FreeTensor(scalesLocal);
    }
};
} // namespace MatmulAllReduceDynamicQuantPertileImpl
