...
template <
    typename aType, typename bType, typename biasType, typename cType, typename commType, bool aTrans, bool bTrans>
__aicore__ inline void
MatmulAllReduceQuantFP16CommInt8<aType, bType, biasType, cType, commType, aTrans, bTrans>::PrepareInit()
{
    ...
    if (GetBlockIdx() == 0) {
        ...
        for (uint32_t i = 0U; i < N; ++i) { // 按总核数下发
            allToAllHandleId_[nowAll2AllIdx] = hccl_.AlltoAll<false>(
                allToAllSendGM_[nowAll2AllIdx], allRecvOutGM_[nowAll2AllIdx], SendCountCheck(nowAll2AllIdx),
...
template <
    typename aType, typename bType, typename biasType, typename cType, typename commType, bool aTrans, bool bTrans>
__aicore__ inline void
MatmulAllReduceQuantFP16CommInt8<aType, bType, biasType, cType, commType, aTrans, bTrans>::Process()
{
    ...
    if (g_coreType == AscendC::AIV) {
        ...
        MatmulAllReduceReduceSumInt8<cType>(
    ...
    if (g_coreType == AscendC::AIV) {
        ...
        const uint64_t outGmTileOffset =
            tilingData_->tilematmulTiling.matmulTiling.M * tilingData_->tilematmulTiling.matmulTiling.N * sizeof(cType);
        const uint64_t outGmTailOffset =
            tilingData_->tailmatmulTiling.matmulTiling.M * tilingData_->tailmatmulTiling.matmulTiling.N * sizeof(cType);
        for (uint32_t i = 0; i < cfg.tileCnt + cfg.tailCnt; i++) {
            ...
            if (i < cfg.tileCnt) { // dequant只传有效大小
                MatmulAllReduceDequantPerchannelCommInt8<cType>(
                    tempBuffDequantGM_, commQuantScale2GM_, outGM_, tPipe_,
    ...
    bool isTiletoTailFlag)
{
    ...
    for (uint32_t i = 0; i < tileCnt; i++) {
        ...
        SyncAll<false>();
        if (isAdd != 0) {
            Matmul_All_Reduce_Add_X3<cType>(
                cGM_, addGM_, cOffset / sizeof(cType), tilingData_->param.addX3UbCnt, tPipe_);
            SyncAll<false>();
        }
        MatmulAllReduceQuantPerchannelCommInt8<cType>(
            cGM_, commQuantScale1GM_, tempBuffWinOrGM_, tPipe_, quant_tiling.matmulTiling.N,
        ...
        if (g_coreType == AscendC::AIV && isSendTileFlag_) { // 从第二块all2all任务commit开始，等待前一块all2all任务做完
            ...
            if (isTiletoTailFlag && i == 0) {
                MatmulAllReduceReduceSumInt8<cType>(
                    tempBuffGM_, commQuantScale1GM_, commQuantScale2GM_, prePadM, quant_tiling.matmulTiling.N, tPipe_,
                    hccl_);
                tempBuffGM_ += prePadM * quant_tiling.matmulTiling.N;
            } else {
                MatmulAllReduceReduceSumInt8<cType>(
                    tempBuffGM_, commQuantScale1GM_, commQuantScale2GM_, padM, quant_tiling.matmulTiling.N, tPipe_,
                    hccl_);
