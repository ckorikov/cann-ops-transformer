...
public:
    ...
    __aicore__ inline void MatmulAllReduceReduceSumSplitM(
            ...
            if ((aivAddOneIndex < this->aivCoreNum + 1) && (vectorIndex >= aivAddOneIndex)) {
                ...
                blockAddrOffset = static_cast<int64_t>(aivAddOneIndex) * static_cast<int64_t>(singleAivM - 1) *
                                  ...
                                  static_cast<int64_t>(vectorIndex - aivAddOneIndex) *
                                      static_cast<int64_t>(singleAivM) * static_cast<int64_t>(this->N);
            } else {
                blockAddrOffset = static_cast<int64_t>(vectorIndex) * static_cast<int64_t>(singleAivM) *
                                  static_cast<int64_t>(this->N);
            }
    ...
    __aicore__ inline void MatmulAllReduceReduceSumSplitMAndN(
    ...
    __aicore__ inline void InitInner(
        uint32_t loopIdx, int64_t blockAddrOffsetSplitM, int64_t blockAddrOffsetSplitMN, int64_t scaleAddrOffsetSplitMN,
        uint32_t blockCntSpiltMN)
    {
        ...
        if (((loopIdx == 0) && (this->splitMode == SPLIT_M_MATMUL_ALLREDUCE_INT8)) ||
            ...
            quantScale1Gm_.SetGlobalBuffer(
                reinterpret_cast<__gm__ T*>(this->quantScale1GM) + curScaleAddrOffset, curScaleCnt);
            quantScale2Gm_.SetGlobalBuffer(
                reinterpret_cast<__gm__ T*>(this->quantScale2GM) + curScaleAddrOffset, curScaleCnt);
        }
    }
        ...
        uint32_t loopIdx, uint32_t aivLoopNum, uint32_t curBlockCntM, uint32_t blockCntSpiltMN)
    {
        ...
        for (uint32_t i = 1; i < this->rankCnt; i++) {
            ...
            DataCopyPad(
                allToAllAddLocal, tempBufferGm_[i * this->tileRankM * this->N], copyParamsCurBlock,
    ...
    __aicore__ inline void SetParams(
        GM_ADDR tempBufferGM, GM_ADDR quantScale1GM, GM_ADDR quantScale2GM, uint32_t M, uint32_t N,
        Hccl<HCCL_SERVER_TYPE_AICPU>& hccl_, uint32_t aivCoreNum, uint32_t& tileBlockCnt, uint32_t& tailBlockCnt,
        ...
        uint32_t& needAivCoreNum, uint32_t& blockNumPerRow, int32_t& nowQuantUbSize)
    {
        ...
        if (nowQuantUbSize >= nowAlginN) { // 分核切M， 不切N
            ...
            this->MatmulAllReduceReduceSumSplitM(nowQuantUbSize, blockAddrOffset, tileCalCntM, tailCalCntM, aivLoopNum);
        } else { // N超大，分核策略采取切MN
            this->splitMode = SPLIT_MN_MATMUL_ALLREDUCE_INT8;
            this->MatmulAllReduceReduceSumSplitMAndN(
                nowQuantUbSize, needAivCoreNum, tileBlockCnt, tailBlockCnt, aivLoopNum, blockNumPerRow);
        }
    }
...
template <class T>
__aicore__ inline void MatmulAllReduceReduceSumInt8(
    GM_ADDR tempBufferGM, GM_ADDR quantScale1GM, GM_ADDR quantScale2GM, uint32_t M, uint32_t N, TPipe* tPipe,
    Hccl<HCCL_SERVER_TYPE_AICPU>& hccl_)
{
    ...
    op.SetParams(
        tempBufferGM, quantScale1GM, quantScale2GM, M, N, hccl_, aivCoreNum, tileBlockCnt, tailBlockCnt, aivLoopNum,
