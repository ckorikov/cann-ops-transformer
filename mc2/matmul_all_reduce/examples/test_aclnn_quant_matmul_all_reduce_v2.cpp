...
/*!
 * \file test_aclnn_quant_matmul_all_reduce_v2.cpp
 * \brief
 */
...
int launchOneThreadQuantMatmulAllReduce(Args &args) {
    ...
    FreeTensor(x2);
    FreeTensor(bias);
    FreeTensor(dequantScale);
    FreeTensor(pertokenScale);
    FreeTensor(x3);
    FreeTensor(out);
    FreeDevice(x1DeviceAddr);
    FreeDevice(x2DeviceAddr);
    FreeDevice(biasDeviceAddr);
    FreeDevice(dequantScaleDeviceAddr);
    FreeDevice(pertokenScaleDeviceAddr);
