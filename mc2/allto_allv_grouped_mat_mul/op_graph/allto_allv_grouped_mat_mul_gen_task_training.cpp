...
static ge::Status Mc2TrainingCalcParamFunc(gert::ExeResGenerationContext* context)
{
...
ge::Status Mc2TrainingInsertHiddenInputForAicore(const gert::ExeResGenerationContext* context, const int32_t groupCnt,
...
ge::Status Mc2TrainingCreateAicpuTask(const gert::ExeResGenerationContext* context, domi::TaskDef& aicpuTask,
...
ge::Status Mc2TrainingInsertTask(const gert::ExeResGenerationContext* context, std::vector<domi::TaskDef>& tasks,
                                 int64_t& aicoreIndex, int32_t groupCnt, const char* opType)
{
    domi::TaskDef waitTask{};
    const char* nodeName = context->GetNodeName();
    GE_ASSERT_SUCCESS(Mc2GenTaskUtils::CreateNotifyTask(context, waitTask, RT_MODEL_TASK_NOTIFY_WAIT, true));
    OPS_ERR_IF(
        Mc2GenTaskUtils::CreateNotifyTask(context, waitTask, RT_MODEL_TASK_NOTIFY_WAIT, true) != ge::GRAPH_SUCCESS,
    ...
    OPS_LOG_D(nodeName, "Generate notify wait task for mc2 node successfully.");
    aicoreIndex += 1L;

    const std::string opTypeStr = opType;
    domi::TaskDef aicpuTask{};
    if (Mc2TrainingCreateAicpuTask(context, aicpuTask, groupCnt) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    ...
    GE_ASSERT_SUCCESS(Mc2GenTaskUtils::CreateNotifyTask(context, recordTask, RT_MODEL_TASK_NOTIFY_RECORD, false));
    OPS_ERR_IF(
        Mc2GenTaskUtils::CreateNotifyTask(context, recordTask, RT_MODEL_TASK_NOTIFY_RECORD, false) != ge::GRAPH_SUCCESS,
    ...
    OPS_LOG_D(nodeName, "Generate task for mc2 node successfully.");
    return ge::GRAPH_SUCCESS;
}

ge::Status Mc2TrainingGenTaskCallback(const gert::ExeResGenerationContext* context, std::vector<domi::TaskDef>& tasks)
{
    const char* nodeName = context->GetNodeName();
    ...
    if (Mc2TrainingInsertHiddenInputForAicore(context, groupCnt, tasks[static_cast<size_t>(aicoreIndex)]) !=
        ge::GRAPH_SUCCESS) {
    ...
    return Mc2TrainingInsertTask(context, tasks, aicoreIndex, groupCnt, opType);
}

ge::Status Mc2TrainingGenTaskFunc(const gert::ExeResGenerationContext* context,
                                  std::vector<std::vector<uint8_t>>& tasks)
{
    return Mc2GenTaskUtils::CommonKFCMc2GenTask(context, tasks, Mc2TrainingGenTaskCallback);
}

IMPL_OP_CT(AlltoAllvGroupedMatMul).CalcOpParam(Mc2TrainingCalcParamFunc).GenerateTask(Mc2TrainingGenTaskFunc);
