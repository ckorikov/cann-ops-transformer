...
constexpr uint64_t CYCLES_PER_US = 50U;
...
__aicore__ inline void DistributeBarrier<TemplateMC2TypeFunc>::TimeOutTest()
{
  ...
  while (curRank < targetNum) {
    DataCopyParams copyParams{(uint16_t)usedAivNum, 1, 3, 0U};
  ...
  DataCopyParams intriOutParams{static_cast<uint16_t>(usedAivNum), 1, 0U, 3};
