...
struct LocalResInfoV2 {
  ...
  HcclSignalInfo
      aicpuOpNotify[AICPU_OP_NOTIFY_MAX_NUM];  // 集合通信AICPU展开资源
...
struct HcclOpResParam {
  ...
  uint64_t
      winSize;  // 每个win大小，静态图时，可能是0，如果通信域内也有动态图，则可能为非0
  ...
  RemoteResPtr remoteRes
      [AICPU_MAX_RANK_NUM];  //数组指针，指向HcclRankRelationResV2，下标为remoteUserRankId
...
struct HcclAiRMAMemInfo {
  ...
  uint64_t memDetailPtr{
      0};  // MemDetails数组首地址, 个数: HcclAiRMAMemType::MAX_NUM
           // 可往后追加字段
};
...
struct HcclA2CombineOpParam {
  ...
  uint64_t
      windowsIn[AscendC::HCCL_MAX_RANK_NUM];  // windows address for input,
                                              ...
                                              // mapping addresses.
  uint64_t
      windowsOut[AscendC::HCCL_MAX_RANK_NUM];  // windows address for output,
                                               // windowsOut[rankId] corresponds
                                               // to the local card address,
                                               // and others are cross-card
                                               // mapping addresses.
  uint8_t res[8328];
  ...
  CombinedCapability* capability;  // address of the communication capability
                                   // information structure on the Device
  uint64_t capabilitySize;  // size of the communication capability information
                            // structure
};
...
__aicore__ inline DataplaneMode GetDataplaneMode(GM_ADDR contextGM0) {
  __gm__ HcclA2CombineOpParam* winContext_ =
      (__gm__ HcclA2CombineOpParam*)contextGM0;
