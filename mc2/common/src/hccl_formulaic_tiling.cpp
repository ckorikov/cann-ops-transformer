...
void FormPartition::AlignTileLarge() {
  if (cutRes.numLongTile >= tileArgs.mAlignLen) {
    OP_LOGW(
        "Common",
        "Invalid parameters mAlignLen %lu, maxTileCnt %lu, numLongTile %lu\n",
        tileArgs.mAlignLen, tileArgs.maxTileCnt, cutRes.numLongTile);
    return;
  }
  // Align cutRes.shortTileLen
  cutRes.shortTileLen = (cutRes.shortTileLen + tileArgs.mAlignLen - 1) /
                        tileArgs.mAlignLen * tileArgs.mAlignLen;
  cutRes.longTileLen =
      (totalLen - cutRes.shortTileLen + cutRes.numLongTile - 1) /
      cutRes.numLongTile;
...
void FormPartition::AlignTileSmall() {
  ...
  bool smallBackTile = (cutRes.shortTileLen < tileArgs.mAlignLen) ||
  ...
  // 校正检查：totalTileCnt均分后，所有主块加起来最多向尾块抽取 mAlignLen *
  // numLongTile 大小，尾块剩余大小为 (MPerRank/totalTileCnt) - mAlignLen *
  // numLongTile 校正检查：检查条件为 (MPerRank/totalTileCnt) > mAlignLen *
  // totalTileCnt, 确保尾块至少剩下mAlignLen大小
  bool largeBackTile = (cutRes.shortTileLen > cutRes.longTileLen) &&
                       ((totalLen / cutRes.totalTileCnt) >
                        (tileArgs.mAlignLen * cutRes.totalTileCnt));
  if (largeBackTile) {
    cutRes.longTileLen =
        (totalLen / cutRes.totalTileCnt + tileArgs.mAlignLen - 1) /
        tileArgs.mAlignLen * tileArgs.mAlignLen;
    cutRes.shortTileLen = totalLen - cutRes.numLongTile * cutRes.longTileLen;
  }
}
...
void FormPartition::GenerateInitialPartition(bool smallDimAlignUp) {
  ...
  cutRes.longTileLen =
      std::min(cutRes.longTileLen, totalLen - cutRes.shortTileLen);
  uint64_t newTileLen =
      cutRes.longTileLen / tileArgs.mAlignLen * tileArgs.mAlignLen;
  ...
  bool alignUpFlag =
      ((cutRes.longTileLen - newTileLen) > (tileArgs.mAlignLen * alignRatio)) ||
      smallDimAlignUp;
  if (alignUpFlag) {
  ...
  cutRes.numLongTile =
      (totalLen -
       static_cast<uint64_t>(static_cast<double>(cutRes.shortTileLen) *
                             backTileRatio)) /
      cutRes.longTileLen;
  ...
  OP_LOGD("Common", "Initial cut: longTileLen %lu, shortTileLen %lu.",
          cutRes.longTileLen, cutRes.shortTileLen);
}

void FormPartition::FitTileLengthDiscrete(bool smallDimAlignUp,
                                          bool goodLinearityShape,
                                          bool localAtFront) {
  // 流水配平，找到初始切分长度和个数
  ...
  bool largeBackTile =
      (resLen >= cutRes.shortTileLen) &&
      ((SMALL_TILECNT_PAR * cutRes.shortTileLen) >= cutRes.longTileLen);
  goodLinearityShape =
      (goodLinearityShape && (cutRes.shortTileLen >= tileArgs.mAlignLen)) ||
  ...
  if (balanceCut) {  // 重新均匀切分
    cutRes.longTileLen = cutRes.shortTileLen;
    cutRes.numLongTile =
        (totalLen - cutRes.shortTileLen) / cutRes.longTileLen;  // floor
    cutRes.numLongTile = std::max(cutRes.numLongTile, static_cast<uint64_t>(1));
    cutRes.totalTileCnt = cutRes.numShortTile + cutRes.numLongTile;
    largeBackTile = false;
  }
  ...
  bool moreFrontTile =
      largeBackTile && (cutRes.longTileLen <= cutRes.shortTileLen + resLen);
  ...
  // 检查切分轮次总数符合约束
  tileArgs.maxTileCnt =
      std::max(tileArgs.maxTileCnt, ONE);  // 确保maxTileCnt > 0
  if (cutRes.totalTileCnt > tileArgs.maxTileCnt) {
    uint64_t tileLen = totalLen / tileArgs.maxTileCnt;
    bool isDivisible = (tileLen * tileArgs.maxTileCnt == totalLen) &&
                       (tileLen % tileArgs.mAlignLen == 0U);
    if (isDivisible) {
...
void FormPartition::LargeBackTileCheck(bool soc310Flag,
                                       bool largeCalcCommRatio) {
  bool largeBackTile =
      cutRes.shortTileLen >= (cutRes.longTileLen + tileArgs.mAlignLen);
  if (soc310Flag) {
    largeBackTile =
        (cutRes.shortTileLen >= (cutRes.longTileLen + SOC310_BASE_M));
  }
  if (largeBackTile) {
    cutRes.longTileLen =
        (totalLen / cutRes.totalTileCnt + tileArgs.mAlignLen - 1) /
  ...
  if (largeCalcCommRatio) {
    ...
    largeBackTile =
        (cutRes.shortTileLen >= (cutRes.longTileLen + tileArgs.mAlignLen)) &&
        (cutRes.shortTileLen >=
             (moveLen + tileArgs.mAlignLen * LARGE_BACKTILE_COMPARE_PAR) &&
...
void FormPartition::CalcBackTile() {
  CalcShortTile();
  bool tinyBackTile = (cutRes.shortTileLen > 0) &&
                      (cutRes.shortTileLen < tileArgs.mAlignLen * 0.5);
...
void FormPartition::SmallFrontTileCheck(bool soc310Flag, bool kGreaterThanN) {
  ...
  bool smallFrontTile = (cutRes.longTileLen < optimalTileLen) &&
                        (totalLen > cutRes.longTileLen * OPT_TILE_CNT);
...
void FormPartition::RedistributeTile() {
  ...
  if (redistTileFlag) {
    uint64_t redistTileLen =
        (tmpBackTileLen - cutRes.shortTileLen) / cutRes.totalTileCnt;
...
void FormPartition::CommBoundShortAlign() {
  ...
  uint64_t alignDownLen =
      cutRes.shortTileLen / tileArgs.mAlignLen * tileArgs.mAlignLen;
  uint64_t alignDownDiff = cutRes.shortTileLen - alignDownLen;
  uint64_t alignUpLen = (cutRes.shortTileLen + tileArgs.mAlignLen - 1) /
                        tileArgs.mAlignLen * tileArgs.mAlignLen;
  bool alignUpShortFlag = (alignUpLen <= SMALL_SHORTTILE_BAR &&
                           cutRes.longTileLen > SMALL_SHORTTILE_BAR) ||
...
void FormPartition::FitTileLengthContinuous(bool kGreaterThanN,
                                            bool largeCalcCommRatio,
                                            bool soc310Flag) {
  cutRes.numLongTile =
      (totalLen -
...
double OneCalcOneCommBase::EstimateTotalMatmulTime() {
  ...
  if (clusterInfo_.socType == SocVersion::SOC910_93 &&
      rankDim_ > MIN_COMM_RANKDIM) {  // double ring
    tileM *= rankTileNum_;
  }
  OP_LOGD("Common", "Estimate mm time: mValue %lu, rankDim_ %lu, tileM %lu",
...
void OneCalcOneCommBase::GetTiling() {
  matmulPerf_.FindCubeUtil(tilingM_.GetMinLen(), rankTileNum_, inferFlag_,
