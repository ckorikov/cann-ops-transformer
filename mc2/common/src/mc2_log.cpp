...
static void PrintTCubeTilingDataSecondPartNewTiling(const std::string &opName,
                                                    ::TCubeTiling &tiling) {
  ...
  OP_LOGD(opName, " tiling.dbL0A %d", tiling.dbL0A);
  OP_LOGD(opName, " tiling.dbL0B %d", tiling.dbL0B);
  OP_LOGD(opName, " tiling.dbL0C %d", tiling.dbL0C);
  ...
  OP_LOGD(opName, " tiling.mxTypePara %d", tiling.mxTypePara);
}
...
void PrintTCubeTilingData(const std::string &opName, ::TCubeTiling &tiling)
{
    ...
    // for cleancode, make sure func less than 50 lines
    PrintTCubeTilingDataSecondPartNewTiling(opName,tiling);
}
