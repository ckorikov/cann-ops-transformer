...
constexpr uint32_t L1_SIZE =
    512 * KB_SIZE - 32;  // reserve 32Bit for allreduce mix
...
enum class KfcTaskType {
  ...
  KFC_TASK_HCC_TASK_PREPARE =
      3,  // 从参数获取通信任务, 完成准备, 待AIC通知消息再激活
  KFC_TASK_HCC_TASK_DELIVER =
      4,  // 从参数获取通信任务, 直接下发. AIC自己发Record激活
  KFC_TASK_COMMON_CMD_EXE =
      5,  // 预留, 通用任务执行, 具体任务信息可以在参数中描述
  KFC_TASK_TYPE_END
};
...
struct TilingArgs {
  ...
  uint64_t nValue;
  uint64_t
      baseMLimit;  // 新增字段，归一方案里，用于多卡多片数据一把传给mmv3做tiling时，表示单卡单片数据的大小
