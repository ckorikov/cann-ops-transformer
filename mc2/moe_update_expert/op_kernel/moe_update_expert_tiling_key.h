...
#define TILINGKEY_LOAD_BALANCING 0
#define TILINGKEY_EXPERT_PURNING 1
namespace Mc2Tiling {
ASCENDC_TPL_ARGS_DECL(MoeUpdateExpert, ASCENDC_TPL_UINT_DECL(TILINGKEY_PURNING, ASCENDC_TPL_2_BW, ASCENDC_TPL_UI_LIST,
                                                             TILINGKEY_LOAD_BALANCING, TILINGKEY_EXPERT_PURNING),
                      ASCENDC_TPL_DTYPE_DECL(TILINGKEY_DATA_TYPE, TILINGKEY_FLOAT, TILINGKEY_HALF, TILINGKEY_BFLOAT16),
);
ASCENDC_TPL_SEL(ASCENDC_TPL_ARGS_SEL(ASCENDC_TPL_UINT_SEL(TILINGKEY_PURNING,  ASCENDC_TPL_UI_LIST, 
                                                          TILINGKEY_LOAD_BALANCING, TILINGKEY_EXPERT_PURNING),
                                     ASCENDC_TPL_DTYPE_SEL(TILINGKEY_DATA_TYPE, TILINGKEY_FLOAT),
                                     ASCENDC_TPL_TILING_STRUCT_SEL(MoeUpdateExpertTilingData)), 
                ASCENDC_TPL_ARGS_SEL(ASCENDC_TPL_UINT_SEL(TILINGKEY_PURNING,  ASCENDC_TPL_UI_LIST, 
                                                          TILINGKEY_EXPERT_PURNING),
