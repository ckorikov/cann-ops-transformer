...
static ge::graphStatus GetAttrsHccl(const gert::TilingContext *context, const char *nodeName,  
...
static ge::graphStatus GetAttrExpert(const gert::TilingContext *context, const char *nodeName, 
...
static ge::graphStatus GetAttrSpecial(const gert::TilingContext *context, const char *nodeName, 
    ...
    MoeDistributeDispatchV2TilingData &tilingData, std::string &groupEp, std::string &groupTp, bool &isSetCommAlg)
{
    ...
    // 获取通信参数
    OP_TILING_CHECK(GetAttrsHccl(context, nodeName, *tilingData, groupEp, groupTp) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Get hccl attr and set tiling data failed."), return ge::GRAPH_FAILED);
    
    // 获取expert 
    OP_TILING_CHECK(GetAttrExpert(context, nodeName, *tilingData) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Get expert attr and set tiling data failed."), return ge::GRAPH_FAILED);

    // 获取特殊专家与commAlg
    OP_TILING_CHECK(GetAttrSpecial(context, nodeName, *tilingData, isSetCommAlg) != ge::GRAPH_SUCCESS,
...
static void CalTilingKey(gert::TilingContext *context, MoeDistributeDispatchV2TilingData *tilingData, const char *nodeName,
    const bool isScales, const uint32_t quantMode, const bool isSetCommAlg)
{
...
static ge::graphStatus GetParamsAndSetTilingData(const gert::TilingContext *context, const char *nodeName, MoeDistributeDispatchV2TilingData *tilingData,
    bool &isActiveMask, bool &isScales, bool &hasElasticInfo, uint32_t &quantMode)
{
    // 获取scales
    const gert::StorageShape *scalesStorageShape = context->GetOptionalInputShape(SCALES_INDEX);
...
static ge::graphStatus MoeDistributeDispatchA3TilingFuncImpl(gert::TilingContext *context)
{
    ...
    OP_TILING_CHECK(GetParamsAndSetTilingData(context, nodeName, tilingData, isActiveMask, isScales, hasElasticInfo, quantMode) != ge::GRAPH_SUCCESS,
    ...
    CalTilingKey(context, tilingData, nodeName, isScales, quantMode, isSetCommAlg);
