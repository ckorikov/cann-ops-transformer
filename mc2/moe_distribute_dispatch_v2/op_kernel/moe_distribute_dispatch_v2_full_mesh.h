...
private:
    ...
    __aicore__ inline void CalAndSendToken(TQue<QuePosition::VECIN, 1> inQueue, TBuf<> outBuf, TBuf<> expertMaskBuf,
                                           uint32_t delCurExpertGroupNum, uint32_t groupIdx);
    ...
    __aicore__ inline void CopyInAndOut(GM_ADDR wAddr, uint32_t index, uint32_t dstPosition, uint32_t arriveCount);
...
{   
    ...
    if (sharedExpertRankNum_ != 0U) {
        sharedUsedAivNum_ = ((aivNum_ - 1) * sharedExpertNum_) / (axisK_ + sharedExpertNum_);
        if (sharedUsedAivNum_ == 0) {
            sharedUsedAivNum_ = 1;
        }
    }
    moeUsedAivNum_ = (aivNum_ - 1) - sharedUsedAivNum_;
    ...
    uint32_t expertIndex)
{
    ...
    if constexpr (DynamicQuant) {
        ...
        dynamicScale = static_cast<float>(127.0) / floatLocalAbsTemp_.GetValue(0);
    ...
    floatTempTensor.SetValue(tokenQuantAlign_, static_cast<float>(1.0) / dynamicScale);
}
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFunc>::SendToSharedExpert(TQue<QuePosition::VECIN, 1> inQueue, TBuf<> outBuf)
{
    ...
    Duplicate<float>(outTensorFp32, static_cast<float>(1.0), hCommuSize_ * BUFFER_NUM / sizeof(float));
    PipeBarrier<PIPE_V>();
    // 分核
    ...
    isFront_ = false;
    SplitToCore(curSendCnt, sharedUsedAivNum_, startTokenId, endTokenId, sendTokenNum);
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFunc>::CalAndSendToken(TQue<QuePosition::VECIN, 1> inQueue, TBuf<> outBuf,
    TBuf<> expertMaskBuf, uint32_t delCurExpertGroupNum, uint32_t groupIdx)
{
    CalExpertSendNum(outBuf, expertMaskBuf);
    int32_t expertIndex, dstExpertId;
    ...
    TBuf<> expertMaskBuf, TBuf<> outBuf)
{
    ...
    // 计算专家发送数据量 && 发送
    CalAndSendToken(inQueue, outBuf, expertMaskBuf, delCurExpertGroupNum, groupIdx);
}
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFunc>::AlltoAllDispatch()
{
    ...
    if (isSendShared) {
        ...
        uint32_t expertIdCntAlign256 = Ceil(expertIdsCnt_, SIZE_ALIGN_256) * SIZE_ALIGN_256;
        maskSizePerExpert_ = expertIdCntAlign256 / 8; // 8 is 1byte->8bit
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFunc>::CalTokenSendExpertCnt(uint32_t dstExpertId, int32_t calCnt, int32_t &curExpertCnt)
{
    ...
    PipeBarrier<PIPE_V>();
    Sub(subExpIdTensor_, validExpertIdsTensor_, dstExpIdTensor_, calCnt);
    PipeBarrier<PIPE_V>();
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFunc>::CalAndSendCnt()
{
    ...
    for (uint32_t curExpertId = 0; curExpertId < totalExpertNum_; ++curExpertId) {
        int32_t curExpertCnt = 0;
        int32_t cntPosIndex = curExpertId * 8 + 1;                 // 一个block有8个int32的元素，第一个元素为flag位，第二个为发送token数
        ...
        DataCopy<int32_t>(rankGMTensor, statusTensor_[curExpertId * 8], 8UL);  // 8 = UB_ALIGN / sizeof(int32_t)
    }
    SyncFunc<AscendC::HardEvent::MTE3_S>();
}
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFunc>::WaitCumSumFlag()
{
    ...
    while (true) {
        ...
        if (cumSumFlag == static_cast<float>(1.0)) {
            break;
        }
    }
    ...
    int32_t beginIdx, int32_t copyCnt)
{
    ...
    PipeBarrier<PIPE_V>();
    CompareScalar(flagCompResultU8_, flagGatherOutTensor_, static_cast<float>(1.0), AscendC::CMPMODE::EQ, compareCount);
    SyncFunc<AscendC::HardEvent::V_S>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFunc>::CopyInAndOut(GM_ADDR wAddr, uint32_t index, 
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFunc>::WaitAndFormatOutput(TBuf<> tBuf, uint32_t validNum)
{
    ...
    while (true) {
        ...
        if (arriveCount == copyCnt) {
            ...
            CopyInAndOut(wAddr, index, dstPosition, arriveCount);
            // finish更新并clean
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFunc>::LocalWindowCopy()
{
    ...
    isFront_ = true;
    SplitToCore(rscvStatusNum_, aivNum_, startId_, endId_, sendNum_);
    // 软同步
    WaitCumSumFlag();
    if (sendNum_ == 0) {
        return;
...
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::CalValidExpIdx(LocalTensor<bool> maskInputTensor)
{
    ...
    PipeBarrier<PIPE_V>();
    Duplicate<uint32_t>(gatherMaskTensor_, 0, Ceil(expertIdsCnt_ * sizeof(int32_t), SIZE_ALIGN_256) * SIZE_ALIGN_256 / sizeof(uint32_t));
    PipeBarrier<PIPE_V>();
    CompareScalar(gatherMaskTensorInt8, tempTensor, static_cast<half>(1), AscendC::CMPMODE::EQ, calCnt);
    PipeBarrier<PIPE_V>();
    GatherMask(expertsIndexTensor, expertsIndexTensor, gatherMaskTensor_, true, mask, {1, 1, 0, 0}, sendToMoeExpTokenCnt_);
}
...
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::MaskZeroComputeExpert(uint32_t maskCnt)
{
    ...
    PipeBarrier<PIPE_V>();
    PipeBarrier<PIPE_V>();
    GatherMask(expertIdsTensor_, expertIdsTensor_, gatherMaskTensor_, true, maskCnt, {1, 1, 0, 0}, sendToMoeExpTokenCnt_);
    SyncFunc<AscendC::HardEvent::V_S>();
}
...
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::GenerateGatherMaskTensor(uint32_t maskCnt)
{
    ...
    PipeBarrier<PIPE_V>();
    uint32_t cnt = maskCnt / UB_ALIGN;
    uint32_t remain = maskCnt % UB_ALIGN;
    if (cnt != 0) {
        Duplicate<uint32_t>(gatherMaskTensor_, 0xFFFFFFFF, cnt);
    }

    if (remain != 0) {
        gatherMaskTensor_(cnt) = (0xffffffff) >> (UB_ALIGN - remain);
    }
    PipeBarrier<PIPE_V>();
}
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::activeMaskProc()
{
    ...
    tpipe_->InitBuffer(dstExpBuf_, expertIdsBufSize_);           // BS * K * 4
    tpipe_->InitBuffer(subExpBuf_, expertIdsBufSize_);           // BS * K * 4
    ...
    if (isExpertMaskFlag_ || (zeroComputeExpertNum_ != 0)) {
        ...
        tpipe_->InitBuffer(validExpertIndexBuf_, expertIdsSize); // BS * K * 4
    }
    ...
    Duplicate<int32_t>(validExpertIdsTensor_, -1, int32_t(expertIdsBufSize_ / sizeof(int32_t))); // 0x3F800000为float的1,0xBF800000 为float的1
    PipeBarrier<PIPE_V>();
    if (isExpertMaskFlag_) {
        ...
        DataCopyExtParams expertIdsCntParams1 = {1U, static_cast<uint32_t>(expertIdsCnt_ * sizeof(uint32_t)), 0U, 0U, 0U};
        DataCopyPadExtParams<int32_t> expertIdsCntCopyPadParams1{false, 0U, 0U, 0U};
        DataCopyPad(tmpExpertIdsTensor, expertIdsGMTensor_, expertIdsCntParams1, expertIdsCntCopyPadParams1);
        SyncFunc<AscendC::HardEvent::MTE2_V>();
        LocalTensor<float> validExpertIdsFloat = expertIdsBuf_.Get<float>();
        Select(validExpertIdsFloat, gatherMaskTensorInt8, tmpExpertIdsTensorFloat, static_cast<float>(-1), SELMODE::VSEL_TENSOR_SCALAR_MODE, expertIdsBufSize_);
    } else {
...
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::Process()
{
    if ASCEND_IS_AIV {          // 全aiv处理
        activeMaskProc();
        if (aivId_ == lastCore_) {
            CalCumSum();        // 最后一个核发送当前卡给每个专家的tokenCnt，输出epRecvCnt/exportTokenNums
        } else {
            AlltoAllDispatch(); // all2all发送
        }
