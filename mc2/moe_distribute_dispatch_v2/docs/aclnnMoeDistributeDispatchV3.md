...
- 新增支持零专家场景：
  ...
  - **copyExpert**：`Moe(x) = x`，通过传入大于0的`copyExpertNum`参数使能。
  - **constExpert**：`Moe(x) = alpha * x + alpha2 * v`，通过传入大于0的`constExpertNum`参数使能。
...
<colgroup>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>输入</td>
   <td>每个专家的量化平滑参数，2D Tensor，shape <code>sharedExpertNum + moeExpertNum, H</code>：<br>非量化场景传空指针；动态量化可传有效数据或空指针；<br><term>Atlas A2系列产品</term>：commAlg="hierarchy"或特定环境变量配置时需传nullptr。</td>
   <td>FLOAT32</td>
   <td>ND（支持非连续Tensor）</td>
  </tr>
  <tr>
   ...
   <td>输入</td>
   <td>标识token是否参与通信：<br><term>Atlas A2系列产品</term>：不支持，传空指针；<br><term>Atlas A3系列产品</term>：1D（shape <code>BS,</code>）或2D（shape <code>BS, K</code>）Tensor，BOOL类型，true需排在false前。</td>
   <td>BOOL</td>
   <td>ND（支持非连续Tensor）</td>
  </tr>
  <tr>
   ...
   <td>输入</td>
   <td>每个token的topK个专家权重：<br><term>Atlas A2系列产品</term>：2D Tensor，shape <code>Bs, K</code>；<br><term>Atlas A3系列产品</term>：不支持，传空指针。</td>
   <td>FLOAT32</td>
   <td>ND（支持非连续Tensor）</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>输入</td>
   <td>输出<code>expertTokenNums</code>的语义类型：0=前缀和，1=实际token数。</td>
   <td>INT64</td>
   <td>-</td>
  </tr>
  <tr>
  ...
  <tr>
   <td>expandXOut</td>
   <td>输出</td>
   <td>根据expertIds扩展的token特征，2D Tensor，shape为 <code>max(tpWorldSize, 1) * A, H</code>。</td>
  ...
  <tr>
   ...
   <td>输出</td>
   <td>给同一专家发送的token个数（供CombineV3系列算子使用），1D Tensor，shape为 <code>A*128,</code>。</td>
   <td>INT32</td>
   <td>ND（支持非连续Tensor）</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>输出</td>
   <td>从EP通信域各卡接收的token数（供CombineV3系列算子使用），1D Tensor：<br><term>Atlas A2系列产品</term>：shape为 <code>moeExpertNum + 2 * globalBs * K * serverNum,</code>；<br><term>Atlas A3系列产品</term>：shape为 <code>epWorldSize * max(tpWorldSize, 1) * localExpertNum,</code>。</td>
   <td>INT32</td>
   <td>ND（支持非连续Tensor）</td>
  </tr>
  <tr>
   ...
   <td>输出</td>
   <td>从TP通信域各卡接收的token数（供CombineV3系列算子使用）：<br><term>Atlas A2系列产品</term>：不支持TP通信，无输出；<br><term>Atlas A3系列产品</term>：有TP通信时为1D Tensor，shape为 <code>tpWorldSize,</code>。</td>
   <td>INT32</td>
   <td>ND（支持非连续Tensor）</td>
  </tr>
  <tr>
   ...
   <td>输出</td>
   <td>本卡输出token的权重（供CombineV3系列算子使用）：<br><term>Atlas A2系列产品</term>：1D Tensor，shape <code>A,</code>；<br><term>Atlas A3系列产品</term>：不支持，无输出。</td>
   <td>FLOAT32</td>
   <td>ND（支持非连续Tensor）</td>
  </tr>
  <tr>
...
- **环境变量约束**：
  - **HCCL_BUFFSIZE**（单位MB，默认200MB）：
    ...
    - <term>Atlas A3系列产品</term>：≥2且≥2*(localExpertNum*maxBs*epWorldSize*Align512(Align32(2*H)+64)+(K+sharedExpertNum)*maxBs*Align512(2*H))（Align512(x)=((x+511)/512)*512，Align32(x)=((x+31)/32)*32）。
  - **HCCL_INTRA_PCIE_ENABLE/HCCL_INTRA_ROCE_ENABLE**：<term>Atlas A2系列产品</term>不再推荐使用，建议通过commAlg="hierarchy"配置。

- **通信域使用约束**：
  - 一个模型中，`aclnnMoeDistributeDispatchV3`与CombineV3系列算子仅支持相同EP通信域，且该域不允许有其他算子。
