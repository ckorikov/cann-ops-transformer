...
class MatMulV3BaseTiling : public MatMulBaseTiling {
 public:
  MatMulV3BaseTiling(gert::TilingContext *context, MatMulTilingCfg &cfg)
      : MatMulBaseTiling(context, cfg),
        compileInfo_(
            *static_cast<const MatmulV3CompileInfo *>(cfg.compileInfo)),
 ...
 protected:
  ge::graphStatus GetShapeAttrsInfo() override {
    ...
    if (!isValidDimValue(args_.mValue) || !isValidDimValue(args_.kValue) ||
        !isValidDimValue(args_.nValue)) {
      OP_LOGE(args_.opName, "illegal value: m[%lu], k[%lu], n[%lu]",
              args_.mValue, args_.kValue, args_.nValue);
      return ge::GRAPH_FAILED;
    }
  ...
  ge::graphStatus AdjustOpTiling() override {
    if (args_.hasBias) {
      runInfo_.baseN = std::min(BASIC_BLOCK_SIZE_256,
                                runInfo_.baseN);  // 有bias时 baseN 小于256
    }
    return ge::GRAPH_SUCCESS;
  };
  ...
  uint64_t GetBlockDim() const override { return runInfo_.usedCoreNum; };

  bool CheckBasicApiTilingKey(uint64_t tilingkey) const {
    return MatMulV3TilingKey().GetApiLevel(tilingkey) ==
  ...
  bool CheckIterBatchBasicApi(uint64_t tilingkey) const {
    return (MatMulV3TilingKey().GetApiLevel(tilingkey) ==
                MatMulV3ApiLevel::BASIC_LEVEL &&
            MatMulV3TilingKey().GetModel(tilingkey) ==
                MatMulV3Model::ITER_BATCH_BATCH_BIAS);
  }
  ...
  ge::graphStatus SetTilingData(const TilingResult &tiling) {
    if ((strcmp(context_->GetNodeType(), "MatMulV3") == 0) &&
        (tiling.tilingDataSize <= TILINGDATA_OFFSET) &&
        (!CheckBasicApiTilingKey(tiling.tilingKey)) &&
        (!CheckIterBatchBasicApi(tiling.tilingKey))) {
      for (uint64_t i = 0; i < TILINGDATA_SPLIT_NUM; ++i) {
        errno_t ret = memcpy_s(
            (uint8_t *)context_->GetRawTilingData()->GetData() +
                i * TILINGDATA_OFFSET,
      ...
      context_->GetRawTilingData()->SetDataSize(
          ops::CeilAlign(tiling.tilingDataSize, TILINGDATA_OFFSET) +
  ...
  ge::graphStatus InitTCubeTilingData(::TCubeTiling &tCubeTiling) const {
    ...
    auto aFormat = args_.aFormat == ge::FORMAT_ND
                       ? matmul_tiling::CubeFormat::ND
                       : matmul_tiling::CubeFormat::NZ;
    auto bFormat = args_.bFormat == ge::FORMAT_ND
                       ? matmul_tiling::CubeFormat::ND
                       : matmul_tiling::CubeFormat::NZ;
    auto cFormat = args_.outFormat == ge::FORMAT_ND
                       ? matmul_tiling::CubeFormat::ND
                       : matmul_tiling::CubeFormat::NZ;
    try {
      mm.SetAType(matmul_tiling::TPosition::GM, aFormat,
                  dtypeMap_.at(args_.aType), args_.isATrans);
      mm.SetBType(matmul_tiling::TPosition::GM, bFormat,
                  dtypeMap_.at(args_.bType), args_.isBTrans);
      mm.SetCType(matmul_tiling::TPosition::GM, cFormat,
                  dtypeMap_.at(args_.cType));
      ...
      if (args_.hasBias) {
        mm.SetBias(true);
        mm.SetBiasType(matmul_tiling::TPosition::GM,
                       matmul_tiling::CubeFormat::ND,
    ...
    mm.SetBufferSpace(compileInfo_.l1Size, compileInfo_.l0CSize,
                      compileInfo_.ubSize);
  ...
  virtual ge::graphStatus GetTilingData(MatMulV3TilingData &tilingData) const {
    ...
    tilingData.mBaseTailCnt = runInfo_.tailInfo.mBaseTailCnt;
    tilingData.nBaseTailCnt = runInfo_.tailInfo.nBaseTailCnt;
    tilingData.isHf32 = args_.isHf32;
    tilingData.tCubeTiling.BatchNum = runInfo_.bmmRunInfo.iterBatch;
    return ret;
  };

  virtual ge::graphStatus GetTilingData(
      BatchMatMulV3TilingData &tilingData) const {
  ...
  virtual ge::graphStatus GetTilingData(
      BatchMatMulV3BasicTilingData &tilingData) const {
    tilingData.batchDimAll = batchInfo_->batchA;
    return GetTilingData(tilingData.matMulTilingData);
  };

  virtual ge::graphStatus GetTilingData(
      MatMulV3BasicTilingData &tilingData) const {
    ...
    tilingData.nL1 = std::min(ops::CeilAlign(args_.nValue, BASIC_BLOCK_SIZE_16),
                              runInfo_.baseN * runInfo_.stepN);
  ...
  virtual ge::graphStatus GetTilingData(
      BatchMatMulV3IterBatchBasicTilingData &iterbatchTilingBasicData) const {
