...
class MatmulV3BaseTiling : public TilingBaseClass {
 ...
 public:
  ...
  MatmulV3BaseTiling(gert::TilingContext *context, MatmulTilingData *tilingData,
 ...
 protected:
  ...
  void DebugLog(
      const std::shared_ptr<tuningtiling::MatMulV3InputArgs> &inputArgs);
  ...
  void CalcNd2NzTiling(uint64_t dtypeSize, uint64_t nValue, uint64_t dValue,
                       uint64_t &baseN, uint64_t &baseD);
  bool CheckUbOverFlow(uint64_t nAligned16, uint64_t nValue,
                       const uint64_t &baseN, const uint64_t &baseD,
                       uint64_t dtypeSize);
  ...
  void CalBaseMBaseN(uint64_t &baseM, uint64_t &baseN,
                     uint64_t maxBaseM = BASIC_BLOCK_SIZE_128,
  ...
  bool CheckAoeTilingEnable(uint32_t aoeTilingEnable,
                            const std::string &opName);
  void SetBasicBlockOf24(MatmulV3RunInfo &runInfo, const uint64_t &mTile,
                         const uint64_t &nTile) const;
  bool IsMixNd2nz();
  void IsGmToL1ByShape();
  void InitL2SplitParams(MatmulV3L2SplitParams &l2SplitParams) const;
  bool IsTailSmall(MatmulV3L2SplitParams &l2SplitParams, uint64_t outL2Split,
                   uint64_t innerL2Split, uint64_t innerMaxConflict) const;
  bool CalcTile(uint64_t &outTile, uint64_t &innerTile, uint64_t &outL2Split,
                uint64_t &innerL2Split, const bool isInnerBad) const;
  uint64_t GetTotalSize(uint64_t m, uint64_t k, uint64_t n, uint64_t aDtype,
                        uint64_t bDtype) const;
  void DoIncreTiling();
  void GetV2Tiling();
  ...
  bool NeedNd2NzVnchw(uint64_t outerSize, uint64_t innerSize,
                      bool supportNd2NzOnTheWay, uint64_t dtypeSize,
