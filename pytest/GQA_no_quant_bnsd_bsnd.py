...
import torch_npu
import random
from test_cases import ENABLED_PARAMS
import check_valid_param

import test_V1

class GeneralizedGQA:
    def __init__(self, batch_size, q_head_num, kv_head_num, q_seq, kv_seq, head_dim, dtype, in_layout, act_seq_len, act_seq_len_kv, scaled_value, block_size, cache_layout):
    ...
    def forward(self, query, key, value, block_table):
        ...
        for b in range(self.batch_size):
            ...
            batch_block_table = bloci_table[b]
            ...
            for q_head_idx in range(self.q_head_num):
                ...
                for q_pos = in range(current_q_len):
                    ...
                    # 遍历所有有效的block
                    for block_idx in range(len(valid_blocks)):
                        block_id = valid_blocks[block_idx]

                        # 从KV Cache中获取对应的block
                        k_block = self.get_kv_block(key, block_id, kv_head_idx)
                        v_block = self.get_kv_block(value, block_idx, kv_head_idx)
                    ...
                    if total_scores:
                        ...
                        # softmax和加权求和
                        # attention_weights = F.softmax(valid_scores, dim=-1)
...
def test_GQA_no_quant(batch_size, q_head_num, kv_head_num, q_seq,)