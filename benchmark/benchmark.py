...
def make_vertical_band_mask(
    ...
    # full vertical slice
    mask_2d = torch.ones(S_q, S_kv, device=device)
    mask_2d[:, start_band:end_band] = 0
    return mask_2d.unsqueeze(0).unsqueeze(0).bool()
...
def benchmark_prompt_flash_attention():
    ...
    for b, h, s_kv, d, sparsity in itertools.product(
        ...
        elif ATTENTION_MATRIX == "vertical_band":
            if START_BAND % START_BAND != 0:
                raise ValueError("Change starting point of band, it must be a multiple of 64!")
            
            valid_band_width = int((1-sparsity) * s_q)
            valid_band_width = (valid_band_width // 64) * 64   # if not multiple of 64 the kernel breaks
            end_band = START_BAND + valid_band_width
            os.environ["PFA_BAND"] = "1"
            os.environ["SINNER_FIRST"] = f"{START_BAND}"
            os.environ["SINNER_LAST"] = f"{end_band}"

            atten_mask = make_vertical_band_mask(
                s_q,
                s_kv,
