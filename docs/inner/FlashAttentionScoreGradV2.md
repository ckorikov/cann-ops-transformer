...
- **参数说明：**
  ...
  - dy（aclTensor\*，计算输入）：Device侧的aclTensor，公式中的输入dY，数据类型支持FLOAT16、BFLOAT16、FLOAT32，[数据格式](common/数据格式.md)支持ND，综合约束请见[约束说明](#约束说明)。

  - pseShiftOptional（aclTensor\*，计算输入）：Device侧的aclTensor，公式中的输入pse，表示位置编码，数据类型支持FLOAT16、BFLOAT16、FLOAT32，[数据格式](common/数据格式.md)支持ND，支持shape范围为\[B,N,S,S\]、\[B,N,1,S\]、\[1,N,S,S\]、\[B,N,H,S\]、\[1,N,H,S\]，H固定为1024。alibi位置编码场景，preTokens和nextTokens必须配置下三角。后续章节如无特殊说明，S表示query或key、value的sequence length, Sq表示query的sequence length, Skv表示key、value的sequence length, SS表示Sq*Skv。如果psetype为2或3的时候，数据类型需为FLOAT32, 对应shape支持范围是\[B,N],\[N]。

  - dropMaskOptional（aclTensor\*，计算输入）：Device侧的aclTensor，数据类型支持UINT8，[数据格式](common/数据格式.md)支持ND，如不使用该参数，可传入nullptr。其shape和数据排布可表示为：
  ...
  - qStartIdxOptional（aclIntArray\*，计算输入）：Host侧的aclIntArray。数据类型支持INT64，代表外切场景，当前分块的Q的sequence在全局中的起始索引，[数据格式](common/数据格式.md)支持ND。如不使用该参数，可传入nullptr。

  - kvStartIdxOptional（aclIntArray\*，计算输入）：Host侧的aclIntArray，数据类型支持INT64，代表外切场景，当前分块的Q的sequence在全局中的起始索引，[数据格式](common/数据格式.md)支持ND。如不使用该参数，可传入nullptr。

  - attenMaskOptional（aclTensor\*，计算输入）：Device侧的aclTensor，0代表保留，1代表掩掉的mask矩阵，数据类型支持BOOL\(8bit的BOOL\)、UINT8，[数据格式](common/数据格式.md)支持ND，支持shape范围为\[B,N,S,S\]、\[B,1,S,S\]、\[1,1,S,S\]、\[S,S\]；综合约束请见[约束说明](#约束说明)。如不使用该参数，可传入nullptr.
  ...
  - keepProb（double，计算输入）：Host侧的double，代表dropMaskOptional中1的比例，数据类型支持DOUBLE；综合约束请见[约束说明](#约束说明)。

  - preTokens（int64\_t，计算输入）：Host侧的int64\_t，用于稀疏计算的参数，数据类型支持INT64。

  - nextTokens（int64\_t，计算输入）：Host侧的int64\_t，用于稀疏计算的参数，数据类型支持INT64。
  ...
  - innerPrecise（int64\_t，计算输入）：**保留参数，暂未使用**。

  - sparseMode（int64\_t，计算输入）：Host侧的int，表示sparse的模式。数据类型支持INT64。

    -   sparseMode为0时，代表defaultMask模式，如果attenMaskOptional未传入则不做mask操作，忽略preTokens和nextTokens\(内部赋值为INT\_MAX\)；如果传入，则需要传入完整的attenMaskOptional矩阵（S1 \* S2），表示preTokens和nextTokens之间的部分需要计算。
  ...
  - pseType （int64\_t，计算输入）：Host侧的int64_t。数据类型支持INT64。用户不特意指定时可传入1，跟当前[FlashAttentionScoreGrad](./FlashAttentionScoreGrad.md)实现一致，支持配置值为0、1、2、3。
    | pseType     | 含义                              |      备注   |
    | ----------- | --------------------------------- | ----------|
    | 0           | 外部传入pse 先mul再add              | - |
