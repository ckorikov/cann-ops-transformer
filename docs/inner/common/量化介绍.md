...
- mx量化：本质属于pergroup-pergroup量化模式（简称G-G量化模式），实际是对于量化参数的数据类型为FLOAT8_E8M0且group size为32的特例。
