...
-   算子功能：适配增量&全量推理场景的FlashAttention算子，既可以支持全量计算场景（PromptFlashAttention），也可支持增量计算场景（IncreFlashAttention）。相比于FusedInferAttentionScoreV3，本接口新增dequantScaleQueryOptional、queryQuantMode参数。
...
-   **参数说明：**
    ...
    - queryQuantMode（int64，计算输入）：query 的伪量化的方式，模式编号与keyAntiquantMode一致。当前版本仅支持传入3，代表模式3：per-token叠加per-head模式。
...
-   **参数说明：**
    ...
    -   workspaceSize（uint64\_t，入参）：在Device侧申请的workspace大小，由第一段接口aclnnFusedInferAttentionScoreV4GetWorkspaceSize获取。
    ...
    - 输入queryRope和keyRope时，仅支持如下特性：
      ...
      -  <term>Atlas A2 训练系列产品/Atlas 800I A2 推理产品/A200I A2 Box 异构组件</term>：
            ...
            - 当query的d等于512时：
                ...
                - key&value&keyRope支持ND和NZ输入，当输入NZ时，数据类型为FLOAT16或BFLOAT16时，输入格式为[blockNum, N, D/16, blockSize, 16]， 数据类型为INT8时，输入格式为[blockNum, N, D/32, blockSize, 32]；
                ...
                - 不支持开启SoftMaxLse、左padding、tensorlist、pse、prefix、伪量化、后量化。
                - 支持全量化场景，即输入query/key/value全为INT8，queryRope和keyRope为BFLOAT16，输出为BFLOAT16的场景：
                    - 入参dequantScaleQuery、keyAntiquantScale、valueAntiquantScale需要同时存在，且其数据类型仅支持FP32。
