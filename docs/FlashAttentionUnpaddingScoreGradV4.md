...
# FlashAttentionUnpaddingScoreGradV4
...
- 算子功能：训练场景下计算注意力的反向输出，即[FlashAttentionVarLenScoreV4](./FlashAttentionVarLenScoreV4.md)的反向计算。该接口相较于[FlashAttentionUnpaddingScoreGrad](./FlashAttentionUnpaddingScoreGrad.md)接口，新增softmaxInLayout参数。
  ...
  - （原有FlashAttentionVarLenScore接口的softmaxSumOut和softmaxMaxOut的输出格式为TND（实际数据按照NTD进行排布），FlashAttentionVarLenScoreV4接口允许传入字符串类型参数softmaxOutLayout，来控制是否输出Shape和数据排布均为TND格式的softmaxSumOut和softmaxMaxOut）。
...
每个算子分为[两段式接口](common/两段式接口.md)，必须先调用“aclnnFlashAttentionUnpaddingScoreGradV4GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnFlashAttentionUnpaddingScoreGradV4”接口执行计算。
...
- **参数说明：**
  - query（aclTensor\*，计算输入）：Device侧的aclTensor，公式中的输入Q，数据类型支持FLOAT16、BFLOAT16、FLOAT32，[数据格式](common/数据格式.md)支持ND。综合约束请见[约束说明](#约束说明)。

  - keyIn（aclTensor\*，计算输入）：Device侧的aclTensor，公式中的输入K，数据类型支持FLOAT16、BFLOAT16、FLOAT32，[数据格式](common/数据格式.md)支持ND。综合约束请见[约束说明](#约束说明)。

  - value（aclTensor\*，计算输入）：Device侧的aclTensor，公式中的输入V，数据类型支持FLOAT16、BFLOAT16、FLOAT32，[数据格式](common/数据格式.md)支持ND。综合约束请见[约束说明](#约束说明)。

  - dy（aclTensor\*，计算输入）：Device侧的aclTensor，公式中的输入dY，数据类型支持、FLOAT16、BFLOAT16、FLOAT32，[数据格式](common/数据格式.md)支持ND。综合约束请见[约束说明](#约束说明)。

  - pseShiftOptional（aclTensor\*，计算输入）：Device侧的aclTensor，公式中的输入pse，表示位置编码。数据类型支持FLOAT16、BFLOAT16、FLOAT32，[数据格式](common/数据格式.md)支持ND，支持shape范围为\[B,N,H,S\]、\[1,N,H,S\]，H固定为1024。alibi位置编码场景，preTokens和nextTokens必须配置下三角，且Sq和Skv是等长的。

  - dropMaskOptional（aclTensor\*，计算输入）：Device侧的aclTensor，数据类型支持UINT8，[数据格式](common/数据格式.md)支持ND，其shape和数据排布可表示为：
  ...
  - attenMaskOptional（aclTensor\*，计算输入）：Device侧的aclTensor，数据类型支持BOOL\(8bit的BOOL\)、UINT8，[数据格式](common/数据格式.md)支持ND，支持shape范围为\[S1Max,S2Max\]。综合约束请见[约束说明](#约束说明)。如不使用该参数，可传入nullptr。
  ...
  - keepProb（double，计算输入）：Host侧的double，代表dropMaskOptional中1的比例，数据类型支持DOUBLE。综合约束请见[约束说明](#约束说明)。一般设置为1.0。
  ...
  - sparseMode（int64\_t，计算输入）：Host侧的int，表示sparse的模式。数据类型支持INT64。
    ...
    **说明：**
    当所有的attenMaskOptional的shape小于2048且相同的时候，建议使用default模式，来减少内存使用量；sparseMode配置为1、2、3、5、6时，用户配置的preTokens、nextTokens不会生效；sparse\_mode配置为0、4时，须保证attenMaskOptional与preTokens、nextTokens的范围一致。

    - softmaxInLayout（string\*，计算输入）：代表输入张量softmaxMax、softmaxSum的实际数据排布，实际数据排布为TND时需要传入"same_as_input"、实际数据排布为NTD时需要传入""。
...
- **返回值：**
  ...
  - 返回161002（ACLNN_ERR_PARAM_INVALID）：query、keyIn、value、dy、pseShiftOptional、dropMaskOptional、paddingMaskOptional、attenMaskOptional、softmaxMaxOptional、softmaxSumOptional、softmaxInOptional、attentionInOptional、dqOut、dkOut、dvOut、softmaxInLayout的数据类型和数据格式不在支持的范围内。
  ```
...
-   **参数说明：**
    ...
    -   workspaceSize（uint64\_t，入参）：在Device侧申请的workspace大小，由第一段接口aclnnFlashAttentionUnpaddingScoreGradV4GetWorkspaceSize获取。
