...
- pergroup量化（简称G量化）：量化对象既可以是左矩阵，也可以是右矩阵，在reduce轴上对数据分组，每组使用独立的量化参数。
  - 假设左矩阵shape为(m, k)，k为reduce轴，在k轴上分组，gs为group size，生成量化参数的shape为(m, k/gs)。
  - 假设右矩阵shape为(k, n)，k为reduce轴，在k轴上分组，gs为group size，生成量化参数的shape为(k/gs, n)。
...
- 全量化：一般是指对左、右矩阵均进行量化的模式，包括
  ...
  - pertensor-perchannel-pergroup量化模式（简称T-C-G量化模式）
