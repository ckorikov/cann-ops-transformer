...
constexpr uint32_t CombineOutMulti = 2;
...
private:
    ...
    __aicore__ inline void SelectTopKExpertScore(int64_t row, bool isTail=false);
    __aicore__ inline void CopyOut(int64_t progress, bool isTail=false);
...
private:
    ...
    uint32_t syncLen_;
    int64_t offsetAddrOut_;
    int64_t offsetAddrIn_;

    const MoeGatingTopKTilingData *tilingData_;

    bool needCalTail_ = false;
};
...
__aicore__ inline void MoeGatingTopKEKFullload<T>::CopyInX(int64_t row)
{
    ...
    DataCopy(xInLocalTensor, xGm_[row * expertCount_ + offsetAddrIn_], expertCount_);
...
template <typename T>
__aicore__ inline void MoeGatingTopKEKFullload<T>::SelectTopKExpertScore(int64_t row, bool isTail)
{
    ...
    LocalTensor<float> yTensorFp32 = outBuffer_[(row % CombineOutMulti) * k_];
    PipeBarrier<PIPE_V>();
    ...
    if (row % CombineOutMulti == 1 || isTail) {
      Cast(yTensor, outBuffer_, RoundMode::CAST_NONE, k_ * 2);
    }
...
template <typename T>
__aicore__ inline void MoeGatingTopKEKFullload<T>::CopyOut(int64_t row, bool isTail)
{
    ...
    if (row % CombineOutMulti == 1 || isTail) {
        int64_t offset = (row - 1) * k_ + offsetAddrOut_;
        if (isTail) {
            offset = row * k_ + offsetAddrOut_;
        }
        DataCopy(yGm_[offset], yOutTensor, k_ * 2);
    }
    DataCopy(expertIdxGm_[row * k_ + offsetAddrOut_], expertIdxTensor, (k_ + 7) / 8 * 8);
                                                        ...
                                                        const MoeGatingTopKTilingData *tilingData, TPipe *tPipe)
{
    ...
    int64_t lastCoreRowCount = tilingData_->lastCoreRowCount;
    ...
    offsetAddrIn_ = blockIdx_ < lastCoreRowCount ? 2 * expertCount_ * blockIdx_ : 2 * expertCount_ * lastCoreRowCount;
    offsetAddrOut_ = blockIdx_ < lastCoreRowCount ? 2 * k_ * blockIdx_ : 2 * k_ * lastCoreRowCount;
    curCoreRowCount_ = blockIdx_ < lastCoreRowCount ? perCoreRowCount_  + 2 : perCoreRowCount_;
    if (blockIdx_ == (GetBlockNum() - 1)) {
        needCalTail_ = rowCount_ - (perCoreRowCount_ * GetBlockNum() + lastCoreRowCount * 2) != 0;
        if (needCalTail_) {
            curCoreRowCount_ = curCoreRowCount_ + 1;
        }
    }
    // init output gm buf
    // bias
    ...
    outBuffer_ = indexTensor_[expertCount_].template ReinterpretCast<float>();
    // 专门为选8个专家的场景做的优化，凑够32B再输出
    int64_t alignVal = 32 / sizeof(half);
    sharedTmpBuffer_ = outBuffer_[(k_ + alignVal - 1) / alignVal * alignVal].template ReinterpretCast<uint8_t>();
...
__aicore__ inline void MoeGatingTopKEKFullload<T>::Process()
{
    ...
    for (int64_t row = 0; row < curCoreRowCount_; row++) {
        bool needOutput = needCalTail_ && (row == (curCoreRowCount_ - 1));
        CopyInX(row);
        ComputeX();
        SortInGroup();
        ...
        SelectTopKExpertScore(row, needOutput);
        CopyOut(row, needOutput);
    }
