...
private:
        ...
        const int32_t curRowsNum, const uint32_t colBytesAlign, const uint32_t col)
    {
        ...
        if constexpr (colRange == ColRangeEnum::SMALLER_THAN_8 || renorm == 1) {
            ...
            Max(dst, dst, src[(loopTimes - 1) * REPEAT_B32_SIZE], col - (loopTimes - 1) * REPEAT_B32_SIZE, curRowsNum,
                intriParams);
            PipeBarrier<PIPE_V>();
            AscendCUtils::SetMaskCount<float>();
        ...
        const int32_t curRowsNum, const uint32_t colBytesAlign, const uint32_t col)
    {
        ...
        if constexpr (colRange == ColRangeEnum::SMALLER_THAN_8 || renorm == 1) {
            ...
            Add(dst, dst, src[(loopTimes - 1) * REPEAT_B32_SIZE], col - (loopTimes - 1) * REPEAT_B32_SIZE, curRowsNum,
                intriParams);
            PipeBarrier<PIPE_V>();
            AscendCUtils::SetMaskCount<float>();
            SetVectorMask<float, MaskMode::COUNTER>(0, curRowsNum * REPEAT_B32_SIZE);
        ...
        const float value)
    {
        if (colBytesAlign - col != 0) {
            uint64_t mask[2] = {
                (((uint64_t)1 << (colBytesAlign - col)) - 1) << (CONSTANT_EIGHT - (colBytesAlign - col)), 0};
