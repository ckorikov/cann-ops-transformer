...
/*!
 * \file moe_finalize_routing_fp_db_all_bias.h
 * \brief
 */
#ifndef MOE_FINALIZE_ROUTING_FP_DB_ALL_BIAS
#define MOE_FINALIZE_ROUTING_FP_DB_ALL_BIAS
...
template <typename T>
class MoeFinalizeRoutingFpDbAllBias {
public:
    __aicore__ inline MoeFinalizeRoutingFpDbAllBias(){};
...
template <typename T>
__aicore__ inline int64_t MoeFinalizeRoutingFpDbAllBias<T>::AlignmentProcess(int64_t param)
{
    return (param + onceAlgnNum_ - 1) / onceAlgnNum_ * onceAlgnNum_;
}

template <typename T>
__aicore__ inline int64_t MoeFinalizeRoutingFpDbAllBias<T>::PadProcessT(int64_t param)
{
    return onceAlgnNum_ - param % onceAlgnNum_;
}

template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpDbAllBias<T>::ParseTilingData(const MoeFinalizeRoutingTilingData *tilingData)
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpDbAllBias<T>::CalcPaddingPara()
{
...
MoeFinalizeRoutingFpDbAllBias<T>::Init(GM_ADDR expandedPermutedRows, GM_ADDR skip1, GM_ADDR skip2, GM_ADDR bias,
                                       ...
                                       GM_ADDR out, GM_ADDR workspace, const MoeFinalizeRoutingTilingData *tilingData)
{
    ...
    pipe.InitBuffer(expandedPermutedRowsBufDb1_, AlignmentProcess(H_) * sizeof(T));
    pipe.InitBuffer(biasBufDb1_, AlignmentProcess(H_) * sizeof(T));
    pipe.InitBuffer(tmpBuf_, AlignmentProcess(H_) * sizeof(T));

    pipe.InitBuffer(expandedSrcToDstRowBuff_, Int32AlignmentProcess(curCoreHandleNum_) * K_ * sizeof(int32_t));
}

template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpDbAllBias<T>::PrepareData()
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpDbAllBias<T>::CopyIn(int64_t nLoopIdx, int64_t curRepeatTimes)
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpDbAllBias<T>::Compute(int64_t nLoopIdx, int64_t curRepeatTimes)
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpDbAllBias<T>::CopyOut(int64_t nLoopIdx, int64_t curRepeatTimes)
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpDbAllBias<T>::Process()
{
...
#endif // MOE_FINALIZE_ROUTING_FP_DB_ALL_BIAS
