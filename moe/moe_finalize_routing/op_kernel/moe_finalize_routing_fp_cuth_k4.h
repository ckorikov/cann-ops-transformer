...
/*!
 * \file moe_finalize_routing_fp_cuth_k4.h
 * \brief
 */
#ifndef MOE_FINALIZE_ROUTING_FP_CUTH_K_FOUR
#define MOE_FINALIZE_ROUTING_FP_CUTH_K_FOUR
...
template <typename T>
class MoeFinalizeRoutingFpCuthK4 {
public:
    __aicore__ inline MoeFinalizeRoutingFpCuthK4(){};
...
template <typename T>
__aicore__ inline int64_t MoeFinalizeRoutingFpCuthK4<T>::AlignmentProcess(int64_t param)
{
    return (param + onceAlgnNum_ - 1) / onceAlgnNum_ * onceAlgnNum_;
}

template <typename T>
__aicore__ inline int64_t MoeFinalizeRoutingFpCuthK4<T>::PadProcessT(int64_t param)
{
    return onceAlgnNum_ - param % onceAlgnNum_;
}

template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuthK4<T>::ParseTilingData(const MoeFinalizeRoutingTilingData *tilingData)
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuthK4<T>::CkechColAlignment()
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuthK4<T>::Init(GM_ADDR expandedPermutedRows, GM_ADDR skip1, GM_ADDR skip2,
                                                           ...
                                                           const MoeFinalizeRoutingTilingData *tilingData)
{
    ...
    pipe.InitBuffer(expandedPermutedRowsBufDb2_, AlignmentProcess(normalH_) * sizeof(T));
    pipe.InitBuffer(biasBufDb2_, AlignmentProcess(normalH_) * sizeof(T));

    pipe.InitBuffer(expandedPermutedRowsBufDb3_, AlignmentProcess(normalH_) * sizeof(T));
    pipe.InitBuffer(biasBufDb3_, AlignmentProcess(normalH_) * sizeof(T));
}

template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuthK4<T>::CopyIn(int64_t nLoopIdx, int64_t bias, int64_t dataLen,
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuthK4<T>::Compute(int64_t nLoopIdx, int64_t bias, int64_t dataLen,
                                                              bool isPadH, int64_t rightPaddingH)
{
    ...
    /*******************************乓***********************************************/
    int64_t biasIndexDb2 = expertForSourceRowLocal.GetValue(2);
    int64_t biasIndexDb3 = expertForSourceRowLocal.GetValue(3);
    T scalesValDb2 = scalesLocal.GetValue(2);
    T scalesValDb3 = scalesLocal.GetValue(3);
    SetFlag<HardEvent::S_MTE2>(EVENT_ID1);

    int64_t expandedSrcToDstRowIndexDb2 = nLoopIdx / (cutNumH_ + 1) + 2 * BS_ + GetBlockIdx() * normalCoreHandleNum_;
    int64_t expandedSrcToDstRowIndexDb3 = nLoopIdx / (cutNumH_ + 1) + 3 * BS_ + GetBlockIdx() * normalCoreHandleNum_;
    ...
    DataCopyPad(biasTmpUbDb2, gmBias_[biasIndexDb2 * H_ + bias], copyParams, padParams);
    DataCopyPad(biasTmpUbDb3, gmBias_[biasIndexDb3 * H_ + bias], copyParams, padParams);
    SetFlag<HardEvent::MTE2_V>(EVENT_ID1);

    WaitFlag<HardEvent::S_MTE2>(EVENT_ID3);
    DataCopyPad(expandedPermutedTmpUbDb2, gmExpandedPermutedRows_[expandedPermutedRowsIndexDb2 * H_ + bias], copyParams,
                padParams);
    DataCopyPad(expandedPermutedTmpUbDb3, gmExpandedPermutedRows_[expandedPermutedRowsIndexDb3 * H_ + bias], copyParams,
                padParams);
    ...
    PipeBarrier<PIPE_V>();
    Muls(expandedPermutedTmpUbDb0, expandedPermutedTmpUbDb0, scalesValDb0, dataLen);
    Muls(expandedPermutedTmpUbDb1, expandedPermutedTmpUbDb1, scalesValDb1, dataLen);
    PipeBarrier<PIPE_V>();
    Add(outLocal[0], outLocal[0], expandedPermutedTmpUbDb0, dataLen);
    PipeBarrier<PIPE_V>();
    Add(outLocal[0], outLocal[0], expandedPermutedTmpUbDb1, dataLen);
    ...
    PipeBarrier<PIPE_V>();
    Muls(expandedPermutedTmpUbDb2, expandedPermutedTmpUbDb2, scalesValDb2, dataLen);
    Muls(expandedPermutedTmpUbDb3, expandedPermutedTmpUbDb3, scalesValDb3, dataLen);
    PipeBarrier<PIPE_V>();
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuthK4<T>::CopyOut(int64_t nLoopIdx, int64_t bias, int64_t dataLen)
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuthK4<T>::Process()
{
...
#endif // MOE_FINALIZE_ROUTING_FP_CUTH_K_FOUR
