...
/*!
 * \file moe_finalize_routing_bf16_cuth_k4.h
 * \brief
 */
#ifndef MOE_FINALIZE_ROUTING_BF16_CUTH_K_FOUR
#define MOE_FINALIZE_ROUTING_BF16_CUTH_K_FOUR
...
template <typename T>
class MoeFinalizeRoutingBf16CuthK4 {
public:
    __aicore__ inline MoeFinalizeRoutingBf16CuthK4(){};
...
template <typename T>
__aicore__ inline int64_t MoeFinalizeRoutingBf16CuthK4<T>::AlignmentProcess(int64_t param)
{
    return (param + onceAlgnNum_ - 1) / onceAlgnNum_ * onceAlgnNum_;
}

template <typename T>
__aicore__ inline int64_t MoeFinalizeRoutingBf16CuthK4<T>::PadProcessT(int64_t param)
{
    return onceAlgnNum_ - param % onceAlgnNum_;
}

template <typename T>
__aicore__ inline void MoeFinalizeRoutingBf16CuthK4<T>::ParseTilingData(const MoeFinalizeRoutingTilingData *tilingData)
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingBf16CuthK4<T>::CkechColAlignment()
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingBf16CuthK4<T>::Init(GM_ADDR expandedPermutedRows, GM_ADDR skip1, GM_ADDR skip2,
                                                             ...
                                                             const MoeFinalizeRoutingTilingData *tilingData)
{
    ...
    pipe.InitBuffer(biasCastBuf2_, AlignmentProcess(normalH_) * sizeof(float));
    pipe.InitBuffer(expandedPermutedRowsCastBuf2_, AlignmentProcess(normalH_) * sizeof(float));

    pipe.InitBuffer(biasCastBuf3_, AlignmentProcess(normalH_) * sizeof(float));
    pipe.InitBuffer(expandedPermutedRowsCastBuf3_, AlignmentProcess(normalH_) * sizeof(float));
}

template <typename T>
__aicore__ inline void MoeFinalizeRoutingBf16CuthK4<T>::CopyIn(int64_t nLoopIdx, int64_t bias, int64_t dataLen,
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingBf16CuthK4<T>::Compute(int64_t nLoopIdx, int64_t bias, int64_t dataLen,
                                                                bool isPadH, int64_t rightPaddingH)
{
    ...
    LocalTensor<float> biasCastUb2 = biasCastBuf2_.Get<float>();

    LocalTensor<float> expandedPermutedRowsCastUb3 = expandedPermutedRowsCastBuf3_.Get<float>();
    LocalTensor<float> biasCastUb3 = biasCastBuf3_.Get<float>();
    ...
    float scalesValDb2 = ToFloat(scalesLocal.GetValue(2));
    int64_t biasIndexDb3 = expertForSourceRowLocal.GetValue(3);
    float scalesValDb3 = ToFloat(scalesLocal.GetValue(3));
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingBf16CuthK4<T>::CopyOut(int64_t nLoopIdx, int64_t bias, int64_t dataLen)
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingBf16CuthK4<T>::Process()
{
...
#endif // MOE_FINALIZE_ROUTING_BF16_CUTH_K_FOUR
