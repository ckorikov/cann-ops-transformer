...
/*!
 * \file moe_finalize_routing_v2_fp_db.h
 * \brief
 */
#ifndef MOE_FINALIZE_ROUTING_V2_FP_DB
#define MOE_FINALIZE_ROUTING_V2_FP_DB
...
class MoeFinalizeRoutingV2FpDb
{
public:
    __aicore__ inline MoeFinalizeRoutingV2FpDb(const MoeFinalizeRoutingV2TilingData& tilingData, TPipe& pipe)
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline int64_t MoeFinalizeRoutingV2FpDb<T, ISBIASEXIST>::AlignmentProcess(int64_t param)
{
    return (param + onceAlgnNum_ - 1) / onceAlgnNum_ * onceAlgnNum_;
}

template <typename T, const bool ISBIASEXIST>
__aicore__ inline int64_t MoeFinalizeRoutingV2FpDb<T, ISBIASEXIST>::PadProcessT(int64_t param)
{
    return onceAlgnNum_ - param % onceAlgnNum_;
}

template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2FpDb<T, ISBIASEXIST>::Init(
    GM_ADDR expandedPermutedRows, GM_ADDR expandedSrcToDstRow, GM_ADDR skip1, GM_ADDR skip2, GM_ADDR bias,
    GM_ADDR scales, GM_ADDR expertForSourceRow, GM_ADDR out, GM_ADDR workspace)
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2FpDb<T, ISBIASEXIST>::CopyIn(int64_t nLoopIdx, int64_t curRepeatTimes)
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2FpDb<T, ISBIASEXIST>::Compute(int64_t nLoopIdx, int64_t curRepeatTimes)
{
    ...
    for (int64_t i = 0; i < curRepeatTimes; i++) {
        ...
        for (int64_t j = 0; j < tilingData_.K / PARALLEL_NUM; j++) {
            ...
            if (tilingData_.dropPadMode == MODE_VALUE_0 || tilingData_.dropPadMode == MODE_VALUE_1) {
                expandedSrcToDstRowIndexDb0 = nLoopIdx * curCoreHandleNumPerLoop_ + i +
                                              PARALLEL_NUM * j * tilingData_.totalRowNum +
            ...
            if (tilingData_.dropPadMode == MODE_VALUE_0 || tilingData_.dropPadMode == MODE_VALUE_1) {
                expandedSrcToDstRowIndexDb1 = nLoopIdx * curCoreHandleNumPerLoop_ + i +
                                              (PARALLEL_NUM * j + 1) * tilingData_.totalRowNum +
                                              GetBlockIdx() * tilingData_.normalCoreHandleNum;
            } else if (tilingData_.dropPadMode == MODE_VALUE_2 || tilingData_.dropPadMode == MODE_VALUE_3) {
            ...
            if constexpr (ISBIASEXIST) {
#if defined(__CCE_AICORE__) && __CCE_AICORE__ == 200
                DataCopyPadCustom(biasTmpUbDb0, gmBias_[biasIndexDb0 * tilingData_.H], copyParams, padParams);
#else
            ...
            if constexpr (ISBIASEXIST) {
#if defined(__CCE_AICORE__) && __CCE_AICORE__ == 200
                DataCopyPadCustom(biasTmpUbDb1, gmBias_[biasIndexDb1 * tilingData_.H], copyParams, padParams);
#else
        ...
        if (tilingData_.K % PARALLEL_NUM != 0) {
            ...
            if (tilingData_.dropPadMode == MODE_VALUE_0 || tilingData_.dropPadMode == MODE_VALUE_1) {
                expandedSrcToDstRowIndexDb0 = nLoopIdx * curCoreHandleNumPerLoop_ + i +
                                              (tilingData_.K - 1) * tilingData_.totalRowNum +
                                              GetBlockIdx() * tilingData_.normalCoreHandleNum;
            } else if (tilingData_.dropPadMode == MODE_VALUE_2 || tilingData_.dropPadMode == MODE_VALUE_3) {
            ...
            if (expandedPermutedRowsIndexDb0 == INVALID_ROW_INDEX &&
                (tilingData_.dropPadMode == MODE_VALUE_1 || tilingData_.dropPadMode == MODE_VALUE_3)) {
                Duplicate(expandedPermutedTmpUbDb0, (T)0, tilingData_.H);
                Duplicate(skip1Local, (T)0, tilingData_.H);
                PipeBarrier<PIPE_V>();
            } else {
                if constexpr (ISBIASEXIST) {
                    Add(skip1Local, expandedPermutedTmpUbDb0, biasTmpUbDb0, tilingData_.H);
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2FpDb<T, ISBIASEXIST>::CopyOut(int64_t nLoopIdx, int64_t curRepeatTimes)
{
    ...
    DataCopyCustom<T, true, false>(
        gmOut_[nLoopIdx * tilingData_.H * curCoreHandleNumPerLoop_], outLocal,
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2FpDb<T, ISBIASEXIST>::Process()
{
...
#endif // MOE_FINALIZE_ROUTING_V2_FP_DB
