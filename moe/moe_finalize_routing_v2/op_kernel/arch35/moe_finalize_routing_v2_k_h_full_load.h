...
public:
        ...
        TPipe* pipePtr)
    {
        ...
        pipe->InitBuffer(expandedXQue, DOUBLE_BUFFER, tilingData->k * tilingData->hAligned * sizeof(T));
        pipe->InitBuffer(yQue, DOUBLE_BUFFER, rowFactorHAlignedFloat * sizeof(float));
        if (hasBiasAndExpertIdx) {
            pipe->InitBuffer(biasQue, DOUBLE_BUFFER, tilingData->k * tilingData->hAligned * sizeof(T));
        }
        if (hasX1) {
