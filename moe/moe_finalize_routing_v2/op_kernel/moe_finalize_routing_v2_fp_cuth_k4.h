    ...
    int64_t nLoopIdx, int64_t bias, int64_t dataLen, bool isPadH, int64_t rightPaddingH)
{
    ...
    if constexpr (ISBIASEXIST) {
#if defined(__CCE_AICORE__) && __CCE_AICORE__ == 200
        DataCopyPadCustom(biasTmpUbDb2, gmBias_[biasIndexDb2 * tilingData_.H + bias], copyParams, padParams);
        DataCopyPadCustom(biasTmpUbDb3, gmBias_[biasIndexDb3 * tilingData_.H + bias], copyParams, padParams);
#else
