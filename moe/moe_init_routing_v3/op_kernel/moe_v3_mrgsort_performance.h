...
/*!
 * \file moe_v3_mrgsort_performance.h
 * \brief
 */
#ifndef MOE_V3_MRGSORT_PERFORMANCE_H
#define MOE_V3_MRGSORT_PERFORMANCE_H
...
private:
    ...
    int64_t outOffset{0};
    int64_t offsets[4];            // Input Gm 偏移
    int64_t listRemainElements[4]; // 总排序长度
    int64_t lengths[4];            // 单次排序长度
    int64_t allRemainElements{0};
    int64_t curLoopSortedNum{0};

    // for MrgSort
    uint16_t validBitTail{0};
    uint16_t elementCountListTail[4];  // 过滤后的单次排序长度
    uint32_t listSortedNums[4];        // 输出：单次排好序的个数
    LocalTensor<float> tmpUbInputs[4]; // 过滤后的输入
};
...
#endif // MOE_V3_MRGSORT_PERFORMANCE_H