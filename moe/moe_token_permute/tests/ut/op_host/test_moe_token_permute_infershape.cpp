...
TEST_F(MoeTokenPermute, infer_shape_01)
{
    gert::StorageShape tokens_shape = {{2, 5}, {2, 5}};
    gert::StorageShape indices_shape = {{2, 3}, {2, 3}};
    gert::StorageShape permuted_tokens_shape = {{6, 5}, {6, 5}};
    gert::StorageShape sorted_indices_shape = {{6}, {6}};

    auto holder = gert::InferShapeContextFaker()
                      .SetOpType("MoeTokenPermute")
                      .NodeIoNum(2, 2)
                      .IrInstanceNum({
                          1,
                          1,
                      })
                      .InputShapes({&tokens_shape, &indices_shape})
                      .OutputShapes({&permuted_tokens_shape, &sorted_indices_shape})
    ...
    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("MoeTokenPermute")->infer_shape;
    ge::graphStatus ret = infer_shape_func(context);
    EXPECT_EQ(ret, ge::GRAPH_SUCCESS);

    std::vector<int64_t> expected_out_shape_0 = {6, 5};
...
TEST_F(MoeTokenPermute, infer_shape_02)
{
    ASSERT_NE(gert::OpImplRegistry::GetInstance().GetOpImpl("MoeTokenPermute"), nullptr);
    auto data_type_func = gert::OpImplRegistry::GetInstance().GetOpImpl("MoeTokenPermute")->infer_datatype;
    ...
    if (data_type_func != nullptr) {
        ...
        auto context_holder = gert::InferDataTypeContextFaker()
                                  ...
                                  .OutputDataTypes({&output_ref, &output_indices_ref})
                                  .Build();
