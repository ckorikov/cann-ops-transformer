...
- **参数说明：**
  ...
  - tokens（aclTensor\*，计算输入）：输入token，要求为一个维度为2D的Tensor，shape为（num\_tokens，hidden\_size），数据类型支持FLOAT16、BFLOAT16、FLOAT32、INT8，支持[非连续的Tensor](common/非连续的Tensor.md)，[数据格式](common/数据格式.md)要求为ND。
  ...
  - permuteTokensOut（aclTensor\*，计算输出）：根据indices进行扩展并排序过的tokens，要求是一个2D的Tensor，shape为（min\(num\_tokens \* topK, num_out_tokens\), hidden\_size）。数据类型同tokens，支持FLOAT16、BFLOAT16、FLOAT32、INT8，不支持[非连续的Tensor](common/非连续的Tensor.md)，[数据格式](common/数据格式.md)要求为ND。
