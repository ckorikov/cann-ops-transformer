...
#include "transformer/moe_token_unpermute_with_ep/op_host/moe_token_unpermute_with_ep_tiling.h"
...
{
    dlog_setlevel(0, 0, 0);
    std::vector<int64_t> range({1, 6144});
    gert::StorageShape permuted_tokens_shape = {{6143, 20480}, {6143, 20480}};
    gert::StorageShape sorted_indices_shape = {{6144}, {6144}};
    // output
    gert::StorageShape unpermuted_shape = {{768, 20480}, {768, 20480}};
    ...
    MoeTokenUnpermuteWithEpCompileInfo compile_info;

    std::string op_type("MoeTokenUnpermuteWithEp");
    ...
    auto holder = gert::TilingContextFaker()
                      .SetOpType("MoeTokenUnpermuteWithEp")
                      .NodeIoNum(3, 1)
                      .IrInstanceNum({1, 1, 1})
                      .InputShapes({&permuted_tokens_shape, &sorted_indices_shape, nullptr})
                      .OutputShapes({&unpermuted_shape})
                      ...
                      .NodeAttrs(
                          {{"num_topk", ge::AnyValue::CreateFrom<int64_t>(8)},
                           ...
                           {"restore_shape", ge::AnyValue::CreateFrom<std::vector<int64_t>>(restore_shape)}})
...
{
    dlog_setlevel(0, 0, 0);
    std::vector<int64_t> range({1, 49151});
    gert::StorageShape permuted_tokens_shape = {{49150, 5120}, {49150, 5120}};
    gert::StorageShape sorted_indices_shape = {{49152}, {49152}};
    gert::StorageShape probs_shape = {{6144, 8}, {6144, 8}};
    // output
    gert::StorageShape unpermuted_shape = {{6144, 5120}, {6144, 5120}};
    std::vector<int64_t> restore_shape({});
    string compile_info_string = R"({
        "hardware_info": {"BT_SIZE": 0, "load3d_constraints": "1",
                          "Intrinsic_fix_pipe_l0c2out": false, "Intrinsic_data_move_l12ub": true, "Intrinsic_data_move_l0c2ub": true, "Intrinsic_data_move_out2l1_nd2nz": false,
