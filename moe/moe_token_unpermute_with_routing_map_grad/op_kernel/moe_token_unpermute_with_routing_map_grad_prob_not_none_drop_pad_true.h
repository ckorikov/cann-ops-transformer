...
__aicore__ inline void MoeTokenUnpermuteWithRoutingMapGradProbNotNoneDropPadTrue<PermutedTokenT, IdxT, ProbsT>::Process()
{
    ...
    for (int64_t indicesLoopTime = 0; indicesLoopTime < outNumCurrentCore;
        ...
        float prob = 0.0;
        if constexpr (IsSameType<PermutedTokenT, bfloat16_t>::value) {
            prob = AscendC::ToFloat(probTemp);
        } else if constexpr (IsSameType<PermutedTokenT, half>::value) {
        ...
        for (int64_t hiddenLoop = 0; hiddenLoop < this->hiddenSizeLoopTimes; hiddenLoop++) {
            ...
            ReduceSumFunc(probGradReduceSumLocal[hiddenLoop], permutedTokensLocalFp32, hiddenLoopNum);
            VToSSync();
        ...
        ReduceSumFunc(probGradReduceSumLocal, probGradReduceSumLocal, this->hiddenSizeLoopTimes);
        VToSSync();
        ...
        if constexpr (IsSameType<ProbsT, bfloat16_t>::value) {
            ...
            probGrad = static_cast<PermutedTokenT>(probGradTemp);
        } else {
            probGrad = probTemp;
        }
