...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::InitExpertTokensGlobalMemory()
{
    if (this->blockIdx < this->needInitExpertCore) {
        if (this->expertTokensCountOrCumsumFlag > EXERPT_TOKENS_NONE) {
            InitOutput(expertTokensCountOrCumsumGm, currentCoreExpert, 0);
        }
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::VBSCopyIn(int64_t progress, int64_t size, int64_t sortNum)
{
    ...
    LocalTensor<int32_t> rowIdxLocal = inLocal[sortNum].template ReinterpretCast<int32_t>();
    int64_t startValue = this->blockIdx * this->vbsTilingData->perCoreElements + inOffset;
    SetWaitFlag<HardEvent::MTE3_S>(HardEvent::MTE3_S);
    ArithProgressionPerf(rowIdxLocal, startValue, 1, size);
    sortDataCopyInQueue.EnQue(inLocal);
}

template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::UBSortCompute(int64_t progress, int64_t size, int64_t sortNum)
{
...
#else
    concatLocal = expertForSourceRowLocalFp32;
#endif
    ...
    sourceRowLocal = inLocal[sortNum].template ReinterpretCast<uint32_t>();
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::VBSCopyOut(int64_t progress, int64_t size, int64_t sortNum)
{
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::InitMoeMrgSort(MoeV2Mrgsort *sorter, int64_t listNum, int64_t coreOffset,
                                                             int64_t loopOffset)
{
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::InitMoeMrgSortOut(MoeV2MrgsortOut *sorter, int64_t listNum,
                                                                int64_t coreOffset)
{
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::OneCoreVMSProcess(int64_t listNum, int64_t perListElements,
                                                                int64_t lastListElements)
{
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::UBSortProcess(int64_t progress, int64_t size, int64_t sortNum)
{
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::VBSProcess()
{
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::VMSProcess()
{
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::SortOutProcess()
{
    if (this->blockIdx < 1) {
        mrgsortParam.perListElements = perListElements;
        mrgsortParam.lastListElements = lastListElements;
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::Init(GM_ADDR expertIdx, GM_ADDR expertTokensCountOrCumsum,
                                                   GM_ADDR expertTokensBeforeCapacity, GM_ADDR workspace,
                                                   const MoeInitRoutingV2TilingData *tilingData, TPipe *tPipe)
{
...
template <typename T>
__aicore__ inline void MoeV2SortMultiCore<T>::Process()
{
