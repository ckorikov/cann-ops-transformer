...
template <typename T>
__aicore__ inline void MoeV2SortOneCore<T>::CopyIn()
{
    ...
    LocalTensor<int32_t> rowIdxLocal = inLocal[this->sortNum].template ReinterpretCast<int32_t>();
    ArithProgressionPerf(rowIdxLocal, 0, 1, this->sortNum);
    sortDataCopyInQueue.EnQue(inLocal);
}

template <typename T>
__aicore__ inline void MoeV2SortOneCore<T>::SortCompute()
{
    LocalTensor<T> inLocal = sortDataCopyInQueue.DeQue<T>();
    LocalTensor<T> expertForSourceRowLocal = inLocal[0];
    LocalTensor<float> expertForSourceRowLocalFp32 = expertForSourceRowLocal.template ReinterpretCast<float>();
    ...
    sourceRowLocal = inLocal[this->sortNum].template ReinterpretCast<uint32_t>();
...
template <typename T>
__aicore__ inline void MoeV2SortOneCore<T>::CopyOut()
{
...
template <typename T>
__aicore__ inline void MoeV2SortOneCore<T>::Init(GM_ADDR expertIdx, GM_ADDR expertTokensCountOrCumsum,
                                                 GM_ADDR expertTokensBeforeCapacity, GM_ADDR workspace,
                                                 const MoeInitRoutingV2TilingData *tilingData, TPipe *tPipe)
{
    ...
    expertIdxGm.SetGlobalBuffer((__gm__ T *)expertIdx, this->tileLength);
    ...
    if (this->blockIdx == this->coreNum - 1) {
        if (this->expertTokensCountOrCumsumFlag > 0) {
            ...
            InitOutput(expertTokensCountOrCumsumGm, this->expertNum, 0);
        }
        if (this->expertTokensBeforeCapacityFlag == 1) {
            expertTokensBeforeCapacityGm.SetGlobalBuffer((__gm__ int32_t *)expertTokensBeforeCapacity,
                                                         Align(this->expertNum, sizeof(int32_t)));
            InitOutput(expertTokensBeforeCapacityGm, this->expertNum, 0);
        }
    }
    // key and value
    int64_t kvFactor = 2;
    ...
    if constexpr (IsSameType<T, int64_t>::value) {
        pipe->InitBuffer(sortDataCopyInQueue, bufferNum, buffSize + this->sortNum * sizeof(int32_t));
    } else {
...
template <typename T>
__aicore__ inline void MoeV2SortOneCore<T>::Process()
{
    if (GetBlockIdx() < 1) {
        CopyIn();
        SortCompute();
