...
| [附录](docs/README.md#附录)                   | 介绍项目中关键的工程脚本或文件、FAQ等，如build.sh参数说明。      
...
- 算子bug修复
  ...
  您可以按照[提交Issue/处理Issue任务](https://gitcode.com/cann/community#提交Issue处理Issue任务)指引新建 `Bug-Report|缺陷反馈` 类Issue对Bug进行描述，
...
- 贡献新算子
  ...
  如果您有全新的算子想基于NPU芯片进行设计实现，欢迎您在Issue中提出新的想法和设计。

  您可以按照[提交Issue/处理Issue任务](https://gitcode.com/cann/community#提交Issue处理Issue任务)指引新建 `Requirement|需求建议` 类Issue提供您的新算子说明和设计方案，
  项目成员会与您进行沟通确认，并为您的算子提供一个合适的`contrib`目录分类，您可以将您的新算子贡献到对应分类目录下。

  同时，您需要在提交的Issue中评论“/assign”或“/assign @yourself”，认领该Issue并在后续完成新算子上库。

  新增算子时提交的算子文件通常比较多，您可以参考如下列表检查最小交付件集合，其中`${op_name}`表示您新增的算子名称：
  ```
  ${op_name}                                              # 使用示例:算子分类目录
  ├── add_example
  │   ├── CMakeLists.txt                                  # 算子CMake构建配置文件
  │   ├── examples                                        # 算子使用示例
  │   │   ├── eager_mode                                  # eager_mode用例，即aclnn调用模式
  │   │   │   ├── CMakeLists.txt
  │   │   │   ├── README.md
  │   │   │   ├── run.sh
  │   │   │   └── test_aclnn_${op_name}.cpp
  │   │   └── graph_mode                                  # graph_mode用例，即图调用模式
  │   │       ├── CMakeLists.txt
  │   │       ├── README.md
  │   │       ├── run.sh
  ...
  │   │   ├── ${op_name}_infershape.cpp                   # 算子InferShape相关实现文件
  │   │   ├── ${op_name}_tiling.cpp                       # 算子Tiling实现文件
  │   │   ├── ${op_name}_tiling.h                         # 算子Tiling头文件
  │   │   ├── CMakeLists.txt                              # 算子CMake构建配置文件
  │   │   └── config                                      # 算子二进制配置文件所有目录
  │   │   │   └── ${soc_version}                          # NPU芯片型号
  │   │   │       ├── ${op_name}_binary.json              # 算子二进制配置文件
  │   │   │       └── ${op_name}_simplified_key.ini       # 算子simplified_key配置文件
  │   │   └──op_api                                       # 算子aclnn接口实现目录
  │   ├── op_kernel                                       # 算子kernel目录
  │   │   ├── ${op_name}.cpp                              # 算子kernel实现代码
  │   │   └── ${op_name}.h                                # 算子kernel实现代码头文件
  │   └── README.md                                       # 算子说明介绍文档
...
- 文档纠错
  ...
  您可以按照[提交Issue/处理Issue任务](https://gitcode.com/cann/community#提交Issue处理Issue任务)指引新建 `Documentation|文档反馈` 类Issue指出对应文档中的问题，
