...
KernelRunContextFaker &KernelRunContextFaker::SetOpType(const std::string op_type) {
  optype_ = op_type;
  return *this;
}
...
ExeResGenerationContextFaker &ExeResGenerationContextFaker::NodeIoNum(size_t input_num, size_t output_num) {
  base_faker_.KernelIONum(input_num + kInputsAppendEnd, output_num);
  base_faker_.NodeIoNum(input_num, output_num);
  return *this;
}
ExeResGenerationContextFaker &ExeResGenerationContextFaker::InputShapes(std::vector<void *> input_shapes) {
  std::vector<void *> inputs(std::move(input_shapes));
  inputs_ = inputs;
  base_faker_.Inputs(std::move(inputs));
  return *this;
}
ExeResGenerationContextFaker &ExeResGenerationContextFaker::OutputShapes(std::vector<void *> output_shapes) {
  outputs_ = output_shapes;
  base_faker_.Outputs(std::move(output_shapes));
  return *this;
}
KernelRunContextHolder ExeResGenerationContextFaker::Build() {
...
TilingContextFaker &TilingContextFaker::DeterministicInfo(void *deterministic_info) {
  deterministic_info_ = deterministic_info;
  UpdateInputs();
  return *this;
}
...
TilingContextFaker &TilingContextFaker::ConstInput(std::vector<std::pair<size_t,
  ...
  inputs.push_back(deterministic_info_); // kInputsDeterministic
  base_faker_.Inputs(std::move(inputs));
  return *this;
}
