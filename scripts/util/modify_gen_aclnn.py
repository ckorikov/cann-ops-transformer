    ...
    for aclnn_cpp in auto_gen_cpps:
        ...
        with open(aclnn_cpp, 'r') as f:
            for line in f.readlines():
                ...
                if "AUTO_GEN_OP_RESOURCER" in line or "namespace op {" in line:
                    break
        if op_type != "":
            shell_exec(["bash", "-c", f"""sed -i 's/{op_type}_op_resource.h/op_resource.h/g' {aclnn_cpp}"""], shell=False)
            shell_exec(["bash", "-c", f"""sed -i 's/{op_type}_RESOURCES/AUTO_GEN_OP_RESOURCE({op_type})/g' {aclnn_cpp}"""], shell=False)
