                                          ...
                                          GM_ADDR workspaceGM, GM_ADDR contextGM, AllGatherAddTilingData *tilingData, TPipe *tPipe)
{
    ...
    coresPerRank_ = AscendC::GetBlockDim / hccl_.GetRankDim();
...
{
    uint64_t strideCount = tilingData_->gatherTileElemNum * hccl_.GetRankDim(); // 通信多轮切分，多张卡的数据拼接到gatherOutGM时，相邻数据块的起始地址偏移
    // 下发通信任务
...
__aicore__ inline void AllGatherAdd::Process()
{
    ...
    for (int i = 0; i < tilingData_->commTurn; i++) {
        ...
        // 根据通信轮次和blockIdx_计算本核需要搬运的起始地址偏移
        int commOffset = i * tilingData_->gatherTileElemNum * sizeof(half);
        int blockOffset = blockIdx_ / coresPerRank_ * tilingData_->gatherTileElemNum * hccl_.GetRankDim() * sizeof(half);
        gatherOutGM.SetGlobalBuffer((__gm__ half*)gatherGM_ + commOffset + blockOffset + (blockIdx_ % coresPerRank_) * blockElemNum_, blockElemNum_);
        inputBGM.SetGlobalBuffer((__gm__ half*)bGM_ + commOffset + blockOffset + (blockIdx_ % coresPerRank_) * blockElemNum_, blockElemNum_);
        outputCGM.SetGlobalBuffer((__gm__ half*)cGM_ + commOffset + blockOffset + (blockIdx_ % coresPerRank_) * blockElemNum_, blockElemNum_);
