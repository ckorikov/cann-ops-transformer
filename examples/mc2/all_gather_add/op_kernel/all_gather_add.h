                                          ...
                                          GM_ADDR workspaceGM, GM_ADDR contextGM, AllGatherAddTilingData *tilingData, TPipe *tPipe)
{
    ...
    strideCount_ = tilingData_->gatherTileElemNum * hccl_.GetRankDim(); // 通信多轮切分，多张卡的数据拼接到gatherOutGM时，相邻数据块的起始地址偏移
    ...
    // 根据rankId确定搬运的起始地址偏移
    gatherOutGM.SetGlobalBuffer((__gm__ half*)gatherGM + rankId_ * strideCount_ + blockElemNum_ * AscendC::GetBlockIdx(), blockElemNum_);
    inputBGM.SetGlobalBuffer((__gm__ half*)bGM + rankId_ * strideCount_ + blockElemNum_ * AscendC::GetBlockIdx(), blockElemNum_);
    outputCGM.SetGlobalBuffer((__gm__ half*)cGM + rankId_ * strideCount_ + blockElemNum_ * AscendC::GetBlockIdx(), blockElemNum_);
