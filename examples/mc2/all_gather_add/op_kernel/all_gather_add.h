                                          ...
                                          GM_ADDR workspaceGM, GM_ADDR contextGM, AllGatherAddTilingData *tilingData, TPipe *tPipe)
{
    ...
    // 传入全局数据的指针，并设置存储大小
    inputAGM.SetGlobalBuffer((__gm__ half*)aGM, tilingData->gatherTileElemNum); // 多轮切分AllGather场景，每张卡参与Gather的数据大小为{140，256}
...
{
    uint32_t rankDim = hccl_.GetRankDim();
    uint64_t strideCount = tilingData_->gatherTileElemNum * rankDim; // 通信多轮切分时，多张卡的数据拼接到gatherOutGM时，相邻数据块的起始地址偏移
    // 下发通信任务
