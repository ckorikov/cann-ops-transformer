...
__aicore__ inline void AllGatherAdd::CalcAddGmAddr(int32_t commTurn)
{
    ...
    uint32_t blockOffset = blockIdx_ / tilingData_->addCoresPerRank * strideCount_;
    uint32_t totalOffset = commOffset + blockOffset + (blockIdx_ % tilingData_->addCoresPerRank) * blockElemNum_;
