...
- **注意事项**
    - 本示例算子仅接受固定shape的tensor为参数，且仅支持在2个昇腾AI处理器上执行。例如，若当前可用Device数量为8，Device ID分别为：0、1、2、3、4、5、6、7，本示例仅支持在其中两个Device上执行，其中Device ID为昇腾AI处理器的逻辑ID。
      ...
      本算子执行代码[test_aclnn_all_gather_add.cpp](examples/test_aclnn_all_gather_add.cpp)中的rankId为逻辑ID，代码中的rankId被赋值为0，1使得算子在Device ID为：0、1的昇腾AI处理器上执行算子，实际执行过程中可能存在多个进程竞争Device 0、1的问题，导致如下错误：
        ```bash
        [ERROR] HcclCommInit failed. ret = 4
        ```

      此时需要设置[ASCEND_RT_VISIBLE_DEVICES](https://www.hiascend.com/document/detail/zh/canncommercial/82RC1/maintenref/envvar/envref_07_0028.html)环境变量来指定实际执行算子的Device：
      
        ```bash
        # 查看各Device运行情况
        npu-smi info
        # 使用环境变量指定Device，若Device 6，7空闲，则指定仅Device 6，7对当前进程可见：
        export ASCEND_RT_VISIBLE_DEVICES=6,7 
        ```
        此时当前进程只可见Device 6，7两个处理器，则Device 6，7对于当前进程的逻辑ID为0，1。
        重新执行算子即可。