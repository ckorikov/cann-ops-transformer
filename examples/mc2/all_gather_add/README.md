...
- **执行算子样例**
    ...
    - 本示例算子目前仅支持基于单算子API执行方式调用算子，通常分为[两段式aclnn接口](../../../docs/context/两段式接口.md)调用，本示例的aclnn接口定义在[op_api](../all_gather_add/op_host/op_api)路径下。
      如需执行该样例算子，首先需要将自定义算子包安装在默认路径下，然后需要编写算子执行代码，通过调用两段式接口来完成算子调用。此处需要注意，算子执行代码需要放在算子的[examples](../all_gather_add/examples/)路径下，且名称需要严格按照"test_aclnn_[算子名称].cpp"来命名，这样[测试工程运行脚本](../../../build.sh)才能够根据算子名称找到对应的执行文件。
...
- **注意事项**
    - 本示例算子为固定shape，仅支持在2个卡上执行，算子样例代码[test_aclnn_all_gather_add.cpp](examples/test_aclnn_all_gather_add.cpp)中的rankId为逻辑卡号，为：0，1，实际执行中可能存在抢卡问题，也就是环境中的实际卡0和1被占用，可能报如下错误：
        ```bash
        [ERROR] HcclCommInitAll failed. ret = 4
        ```
        此时需要设置环境变量指定执行的空闲物理卡：
        ```bash
        # 首先查看各卡运行情况
        npu-smi info
        # 然后使用环境变量指定空闲卡，若6，7卡空闲：
