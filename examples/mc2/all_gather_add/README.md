...
- **执行算子样例**
    ...
    - 本示例算子目前仅支持基于单算子API执行方式调用算子，通常分为[两段式接口](../../../docs/context/两段式接口.md)调用，本示例的aclnn接口定义文件在[op_api](../all_gather_add/op_host/op_api)路径下。如需执行该样例算子，首先需要将自定义算子包安装在默认路径下，然后需要编写算子执行代码，通过调用两段式接口来完成算子调用。此处需要注意，算子执行代码需要放在算子的[examples](../all_gather_add/examples/)路径下，且名称需要严格按照"test_aclnn_[算子名称].cpp"来命名，这样[测试工程运行脚本](../../../build.sh)才能够根据算子名称找到对应执行的文件。
