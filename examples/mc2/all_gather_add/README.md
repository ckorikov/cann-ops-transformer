...
- **执行算子样例**
    ...
    - 完成自定义算子包安装后，执行命令如下：
        ...
        如需执行算子样例，需将自定义算子包安装在默认路径下。执行算子样例代码[test_aclnn_all_gather_add.cpp](examples/test_aclnn_all_gather_add.cpp)，该测试样例代码会按照固定shape随机生成测试数据作为输入，同时在cpu侧对输入数据进行拼接和相加来模拟AllGather、Add的运算并生成golden数据，最后通过**aclnnAllGatherAdd**接口方式调用算子。算子的执行结束后，测试样例会将算子的执行结果拷贝到主机侧与golden进行精度对比，本示例算子的精度要求为千分之一。
        运行结果示例如下：
